@echo off
REM This batchfile byte-compiles the PHW lisp files.

REM Copyright (C) 2000 - 2005, 2015 Jesper Nordenberg,
REM                           Klaus Berndl,
REM                           Kevin A. Burton,
...
REM Make sure you byte-compile PHW with the semantic, eieio and
...
REM Call "make" to byte-compile the PHW.
...
echo Byte-compiling PHW with LOADPATH= %SEMANTIC% %EIEIO% %SPEEDBAR%

if exist phw-compile-script-init del phw-compile-script-init
if exist phw.elc del *.elc

echo (add-to-list 'load-path nil) > phw-compile-script-init
echo (load-file "%CEDET%/common/cedet.el") >> phw-compile-script-init
echo (require 'phw) >> phw-compile-script-init
echo (setq debug-on-error t) >> phw-compile-script-init

%EMACS% -batch -no-site-file -l phw-compile-script-init --eval "(phw-byte-compile t)"

del phw-compile-script-init
