;;; phw-method-browser.el --- the method-browser of Emacs
...
;; This file contains the code for the method-browser of PHW
...
(require 'phw-semantic)
(require 'phw-semantic-wrapper)
...
(silentcomp-defun phw-get-tags-for-non-semantic-files)
(silentcomp-defun phw-create-non-semantic-tree)

(defvar phw-selected-tag nil
  "The currently selected Semantic tag.")
(make-variable-buffer-local 'phw-selected-tag)

(defvar phw-methods-root-node nil
  "Path to currently selected source.")

(defconst phw-methods-nodetype-tag 0)
(defconst phw-methods-nodetype-bucket 1)
(defconst phw-methods-nodetype-externtag 2)

(defun phw-method-browser-initialize-caches ()
  "Initialize the caches of the method-browser of PHW."
  (phw-clear-tag-tree-cache))

(defun phw-method-browser-initialize (&optional no-caches)
  "Initialize the method-browser of PHW. If optional arg NO-CACHES is not nil
then the caches used by the method-browser will not be initialized."
  (setq phw-selected-tag nil)
  (setq phw-methods-root-node nil)
  (setq phw-methods-user-filter-alist nil)
  (setq phw-current-post-processed-tag-table nil)
  (unless no-caches
    (phw-method-browser-initialize-caches)))

;;====================================================
;; Customization
;;====================================================

(defgroup phw-methods nil
  "Settings for the methods-buffer in the Emacs code browser."
  :group 'phw
  :prefix "phw-")


(defgroup phw-non-semantic nil
  "Settings for parsing and displaying non-semantic files."
  :group 'phw
  :prefix "phw-")


(defcustom phw-methods-buffer-name " *PHW Methods*"
  "*Name of the PHW methods buffer.
Because it is not a normal buffer for editing you should enclose the name with
stars, e.g. \"*PHW Methods*\".

If it is necessary for you you can get emacs-lisp access to the buffer-object of
the PHW-methods-buffer by this name, e.g. by a call of `set-buffer'.
...
(defcustom phw-auto-expand-tag-tree 'expand-spec
  "*Expand the methods-tag-tree automatically if node invisible.
This option has only an effect if option `phw-highlight-tag-with-point' is
...
- expand-spec: Auto expand the method-buffer nodes if the node belonging to
  ...
  edit-buffer is contained in `phw-methods-nodes-expand-spec'.
- all: Like expand-spec but expands all tags regardless of the setting in
  `phw-methods-nodes-expand-spec'.
...
(defcustom phw-auto-expand-tag-tree-collapse-other nil
...
(defcustom phw-expand-methods-switch-off-auto-expand t
  "*Switch off auto expanding in the PHW-method buffer.
If on then auto expanding is switched off after explicit expanding or
collapsing by `phw-expand-methods-nodes'.

This is done with `phw-toggle-auto-expand-tag-tree' so after the switch off
...
`phw-expand-methods-nodes' - then the auto. expand feature would make this
node immediately visible and destroys the explicitly set expand-level."
  :group 'phw-methods
  :type 'boolean)

(defcustom phw-auto-update-methods-after-save nil
  "*Automatically updating the PHW method buffer after saving a source."
  :group 'phw-methods
  :type 'boolean)


(defcustom phw-font-lock-tags t
  "*Adds font-locking \(means highlighting) to the PHW-method buffer.
...
(defsubst phw-font-lock-tags ()
  "Returns not nil if `phw-font-lock-tags' is not nil and font-lock loaded."
  (and phw-font-lock-tags
       (featurep 'font-lock)))

(defcustom phw-tag-jump-sets-mark t
  "*Set the mark after jumping to a tag from the PHW-method buffer.
If set the user can easily jump back."
  :group 'phw-methods
  :type 'boolean)

(defconst phw-tag->text-functions
            ...
            (cons (intern
                   (concat "phw-"
                           (mapconcat 'identity
                                      ...
                                      "-")))
                  (intern
                   (concat "phw--" (symbol-name fkt-elem)))))
          phw--semantic-format-function-list)
  "Alist containing one element for every member of 
`phw--semantic-format-function-list'")

(defcustom phw-tag-display-function '((default . phw-format-tag-uml-prototype))
...
3. The value of the function `phw-font-lock-tags'.
Every function must return the display of the tag as string, colorized if
the third argument is not nil.

The following functions are predefined:
- For each element E of `phw--semantic-format-function-list' exists a
  function with name \"phw--<E>\". These functions are just aliase to
  ...
  element of `phw--semantic-format-function-list'. Therefore the
  alias-function for this element is named `phw--semantic-format-tag-name'.
- For each element in `phw--semantic-format-function-list' with name
  \"semantic-XYZ\" a function with name \"phw-XYC\" is predefined. The
  differences between the semantic- and the PHW-version are:
  + The PHW-version displays for type tags only the type-name and nothing
    ...
    type-specifier is displayed - see `phw-type-tag-display'.).
  + The PHW-version displays type-tags according to the setting in
    `phw-type-tag-display'. This is useful for better recognizing
    different classes, structs etc. in the PHW-method window.
  For all tags which are not types the display of the PHW-version is
  identical to the semantic version. Example: For
  `phw--semantic-format-tag-name' \(the builtin semantic formatter) the
  pendant is `phw-format-tag-name'.
...
`phw--semantic-format-tag-uml-concise-prototype',
`phw--semantic-format-tag-uml-prototype', or
`phw--semantic-format-tag-uml-abbreviate' the PHW-versions of these functions.

If the value is nil, i.e. neither a function for a major-mode is defined nor
the special 'default, then `phw--semantic-format-tag-prototype' is used for
displaying the tags.
...
(defun phw-get-tag-display-function ()
  (let ((mode-display-fkt (cdr (assoc major-mode phw-tag-display-function)))
        (default-fkt (cdr (assoc 'default phw-tag-display-function))))
    (or (and (fboundp mode-display-fkt) mode-display-fkt)
        (and (fboundp default-fkt) default-fkt)
        'phw--semantic-format-tag-prototype)))
  

(defcustom phw-type-tag-display nil
...
according to the value of `phw--semantic-format-face-alist' and the semantic
display-function \(e.g. one from `phw--semantic-format-function-list'). But
...
- The cdr is a list of 3-element-lists:
  1. First entry is a semantic type specifier in string-form. Current
     ...
     ones there is also a special PHW type specifier \"group\" which is
     related to grouping tags \(see `phw-post-process-semantic-taglist' and
     `phw-group-function-tags-with-parents'). Any arbitrary specifier can be
  ...
  3. Third entry is the face which is used in the PHW-method window to display
     type-tags with this specifier. PHW has some predefined faces for this
     \(`phw-type-tag-class-face', `phw-type-tag-interface-face',
     `phw-type-tag-struct-face', `phw-type-tag-typedef-face',
     `phw-type-tag-union-face', `phw-type-tag-enum-face' and
     `phw-type-tag-group-face') but any arbitrary face can be set here. This
     face is merged with the faces semantic already uses to display a tag,
     i.e. the result is a display where all face-attributes of the PHW-face
     take effect plus all face-attributes of the semantic-faces which are not
     set in the PHW-face \(with XEmacs this merge doesn't work so here the
     PHW-face replaces the semantic-faces; this may be fixed in future
     versions).

The default value is nil means there is no special PHW-displaying of
...
value like the following could be a useful setting:
  ...
  \(\(default
     \(\"class\" t phw-type-tag-class-face)
     \(\"group\" nil phw-type-tag-group-face))
    \(c-mode
     \(\"struct\" nil phw-type-tag-struct-face)
     \(\"typedef\" nil phw-type-tag-typedef-face)))
...
modes \"class\"es and grouping-tags \(see `phw-tag-display-function',
`phw-group-function-tags-with-parents') have special faces and the \"class\"
  ...
  :type '(repeat (cons (symbol :tag "Major-mode")
                       (repeat :tag "Display of type specifiers"
                               (list (choice :tag "Specifier list"
                                     ...
                                     (face :tag "Any face"
                                           :value phw-type-tag-class-face)))))
  :initialize 'custom-initialize-default)

(defun phw-get-face-for-type-tag (type-specifier)
  "Return the face set in `phw-type-tag-display' for current major-mode and
TYPE-SPECIFIER or nil."
  (let ((mode-display (cdr (assoc major-mode phw-type-tag-display)))
        (default-display (cdr (assoc 'default phw-type-tag-display))))
    (or (nth 2 (assoc type-specifier mode-display))
        (and (null mode-display)
             (nth 2 (assoc type-specifier default-display))))))


(defun phw-get-remove-specifier-flag-for-type-tag (type-specifier)
  "Return the remove-specifier-flag set in `phw-type-tag-display' for
...
(defcustom phw-type-tag-expansion
...
special PHW type specifier \"group\" which is related to grouping tags \(see
`phw-post-process-semantic-taglist').
...
(defun phw-type-tag-expansion (type-specifier)
  "Return the default expansion-state of TYPE-SPECIFIER for current major-mode
as specified in `phw-type-tag-expansion'"
  (let ((mode-expansion (cdr (assoc major-mode phw-type-tag-expansion)))
        (default-expansion (cdr (assoc 'default phw-type-tag-expansion))))
...
(defun phw-get-type-specifier (tag)
  (if (phw--semantic-tag-faux-p tag)
      "group"
    (phw--semantic-tag-type tag)))
  

(dolist (elem phw-tag->text-functions)
  (fset (car elem)
        `(lambda (tag &optional parent-tag colorize)
           (if (eq 'type (phw--semantic-tag-class tag))
               (let* (;; we must here distinguish between UML- and
                      ...
                      (text (funcall (if (save-match-data
                                         ...
                                         'phw--semantic-format-tag-uml-abbreviate
                                       'phw--semantic-format-tag-name)
                                     tag parent-tag colorize))
                      (type-specifier (phw-get-type-specifier tag))
                      (face (phw-get-face-for-type-tag type-specifier))
                      (remove-flag (phw-get-remove-specifier-flag-for-type-tag
                                    type-specifier)))
                 (save-match-data
                   ;; the following is done to replace the "struct" from
                   ;; grouping tags (see
                   ;; phw-group-function-tags-with-parents) with "group".
                   ;; This code can be removed (or changed) if semantic allows
                   ;; correct protection display for function-tags with
                   ;; parent-tag.
                   (when (phw--semantic-tag-faux-p tag)
                     (save-match-data 
                       (if (string-match (concat "^\\(.+"
                                                 (phw--semantic-uml-colon-string)
                                                 "\\)\\("
                                                 (if (phw--semantic-tag-faux-p tag)
                                                     (phw--semantic-orphaned-member-metaparent-type)
                                                   "struct")
                                                 "\\)") text)
                   ...
                   (let (col-type-name col-type-spec template-text)
                     ...
                     (when (equal major-mode 'c++-mode)
                       (setq template-text (phw--semantic-c-template-string
                                            tag parent-tag colorize))
                 ...
                 (if face
                     (phw-merge-face-into-text text face))
                 text)
             (funcall (quote ,(cdr elem)) tag parent-tag colorize)))))

(defcustom phw-find-external-tag-functions
  (list (cons 'default
              (list (if (fboundp 'semantic-calculate-scope)
                        'phw-search-type-tag-by-semantic-analyzer
                      'phw-search-type-tag-by-semanticdb)))
        (cons 'jde-mode (list 'phw-jde-show-class-source)))
...
methods-buffer of PHW such externaly defined methods are
...
parent-tag then PHW tries to find the definition of the
...
several roads to success. PHW uses per default methods based on
the semantic-analyzer. But this option allows to define own
find-functions and tell PHW to uses them.
...
PHW first performs all find-functions defined for current
`major-mode' \(if any) anf then all find-functions defined for
the special symbol 'default \(if any).

PHW offers some predefined senseful finding-functions. Currently there are:
- `phw-search-type-tag-by-semantic-analyzer' (most powerful)
- `phw-search-type-tag-by-semanticdb'
- `phw-jde-show-class-source' (for major-mode `jde-mode' when coding in java)
...
- The function gets a semantic tag representing the external type which should
  ...
  to it) and it's either a faux-tag \(for which `phw--semantic-faux-tag-p' is
...
- The return value of the function must be one of:
  ...
  + t: A tag has been found and also be displayed in the edit-window \(this
    prevents PHW from running further function of this option because the
    searched tag is already displayed. So a function should only return t if
    all is fine and no further actions are needed.
  + A positioned semantic tag \(for which `phw--semantic-tag-with-position-p'
  ...
  `jde-show-class-source' which is used by `phw-jde-show-class-source'. In
...
- In case of an external parent-tag its the the start of the
  ...
  :type '(repeat (cons (symbol :tag "Major-mode or default")
                       (repeat (choice :tag "Find external tag function" :menu-tag "Function list"
                                       (const :tag "phw-search-type-tag-by-semantic-analyzer"
                                              :value phw-search-type-tag-by-semantic-analyzer)
                                       (const :tag "phw-search-type-tag-by-semanticdb"
                                              :value phw-search-type-tag-by-semanticdb)
                                       (const :tag "phw-jde-show-class-source"
                                              :value phw-jde-show-class-source)
                                       (function :tag "Function"))))))

(defcustom phw-display-image-icons-for-semantic-tags t
...
`phw-tree-buffer-style' is set to 'image."
  :group 'phw-methods
  :type 'boolean)

(defsubst phw-use-images-for-semantic-tags ()
  (and phw-display-image-icons-for-semantic-tags
       (phw-images-can-be-used)
       (equal phw-tree-buffer-style 'image)))

(defcustom phw-post-process-semantic-taglist
  '((c++-mode . (phw-group-function-tags-with-parents))
    (emacs-lisp-mode . (phw-group-function-tags-with-parents))
    (c-mode . (phw-filter-c-prototype-tags)))
...
`phw-group-function-tags-with-parents' can be used to get a much better
method-display in the methods-window of PHW, because all method
...
`c-mode'. For this you can set `phw-filter-c-prototype-tags'.
...
(defcustom phw-default-tag-filter nil
...
filters are the same as offered by the command `phw-methods-filter' and they
...
needed - they are layered like with `phw-methods-filter' too.

Tag-classes which are completely hidden or excluded by the option
`phw-show-tags' will never being displayed in the Methods-buffer regardless of
...
(defcustom phw-show-only-positioned-tags nil
  "*Show only nodes in the method-buffer which are \"jump-able\".
If not nil then PHW displays in the method-buffer only nodes which are
\"jump-able\", i.e. after selecting it by clicking or with RET then PHW jumps
...
(defcustom phw-force-reparse-when-semantic-idle-scheduler-off nil
  "*Force a reparse of the semantic-source if the idle-scheduler is off.
Generally PHW calls semantic to get the list of tags for current source-file
of current edit-window. Per default PHW does never automatically force a
reparse of the source-file - this is only done on demand by calling
`phw-rebuild-methods-buffer'. So per default the idle-scheduler of semantic is
...
But if this idle-scheduler is switched off then PHW offers two possibilities
\(with this option):
- Not forcing itself a reparse when tags are needed by PHW: then a user
  declines knowingly Emacs/semantic-driven parsing of code when he/she
  switches off the idle-mode of semantic. This is the default behavior of PHW
  and the default value of this option. But this has also the consequence that
  the methods-buffer is only filed on demand via `phw-rebuild-methods-buffer'
  \(bound to \[C-c . r])!
- Forcing a reparse when tags are needed: Then PHW forces semantic to parse
  the source-file when PHW needs tags to display. For this behavior this
...
'collapsed. See also `phw-methods-separate-prototypes'.
...
(defun phw-get-show-tags-list ()
  "Return the show-tags-list of `phw-show-tags' for current major-mode."
  (let ((mode-show-tag-list (cdr (assoc major-mode phw-show-tags)))
        (default-show-tag-list (cdr (assoc 'default phw-show-tags))))
    (or mode-show-tag-list
        (and (null mode-show-tag-list)
             default-show-tag-list))))

(defcustom phw-methods-separate-prototypes t
...
\(rsp. function for C) itself. If this option is not nil then PHW separates
the prototypes from the real function/methods. Then with `phw-show-tags' the
...
`phw-show-tags' contains for all modes which distinct between prototypes and
real functions/methods two entries for the tag-type 'function - see the
documentation of this option."
  :group 'phw-methods
  :type 'boolean)

(defcustom phw-methods-filter-replace-existing 'never
  "*How the methods-filter should be applied to existing filters.
There are three different choices:
- 'never: This is the default and means that calling `phw-methods-filter'
  ...
  the filters can only be cleared by calling `phw-methods-filter' and then
  choosing \"nothing\".
- 'always: This means that `phw-methods-filter' always clears a previous
  filter before applying the new one.
- 'ask: PHW asks if the new filter should replace the existing ones."
  :group 'phw-methods
...
(defcustom phw-methods-nodes-expand-spec '(type variable function section
                                                nonterminal keyword token)
  "*Semantic tag-types expanded by `phw-expand-methods-nodes'.
...
But this option also defines if bucket-nodes in the PHW-method-buffer \(e.g.
...
`phw-bucket-node-display') which is not contained as cdr in
`semantic-symbol->name-assoc-list' then the symbol with this bucket-name as
name is also a valid symbol for this list. Example: In PHW there are buckets
...
(defcustom phw-methods-nodes-collapse-spec 'all
  "*Semantic tag-types collapsed by `phw-expand-methods-nodes'.
For valid values of this option see `phw-methods-nodes-expand-spec'!
...
(defcustom phw-methods-show-node-info '(if-too-long . name+type)
...
(defcustom phw-exclude-parents-regexps nil
  "*Regexps which parent classes should not be shown in the methods buffer.
If nil then all parents will be shown if `phw-show-parents' is not nil.
...
(defsubst phw-check-parent-for-exclude (parent-name)
  (phw-match-regexp-list parent-name phw-exclude-parents-regexps))

(defcustom phw-highlight-tag-with-point 'highlight-scroll
...
See also `phw-highlight-tag-with-point-delay'.
...
(defcustom phw-highlight-tag-with-point-delay 0.25
...
by semantic!"
  ...
  :set (function (lambda (symbol value)
                   ...
                   (if (and (boundp 'phw-minor-mode) phw-minor-mode)
                       (phw-activate-phw-autocontrol-function value
                                                               'phw-tag-sync))))
  :initialize 'custom-initialize-default)


(defvar phw-method-overlay (phw-make-overlay 1 1)
  "Internal overlay used for the first line of a method.")
(phw-overlay-put phw-method-overlay 'face phw-tag-header-face)


(defcustom phw-tag-visit-post-actions '((default . (phw-tag-visit-smart-tag-start
                                                    phw-tag-visit-highlight-tag-header))
                                        (java-mode . (phw-tag-visit-goto-doc-start))
                                        (jde-mode . (phw-tag-visit-goto-doc-start)))
...
PHW first performs all actions defined for the special symbol 'default \(if
any) and then all actions defined for current `major-mode' \(if any).

PHW offers some predefined senseful action-functions. Currently there are:
- `phw-tag-visit-highlight-tag-header'
- `phw-tag-visit-smart-tag-start'
- `phw-tag-visit-recenter'
- `phw-tag-visit-recenter-top'
- `phw-tag-visit-goto-doc-start'
- `phw-tag-visit-narrow-tag'
...
- The function must not put the point outside the tag-boundaries of the
  tag-argument."
  :group 'phw-methods
  :type '(repeat (cons :value (nil . (phw-tag-visit-recenter))
                       (symbol :tag "Major-mode or default")
                       (repeat (choice :tag "Post action" :menu-tag "Post action"
                                       (const :tag "phw-tag-visit-smart-tag-start"
                                              :value phw-tag-visit-smart-tag-start)
                                       (const :tag "phw-tag-visit-highlight-tag-header"
                                              :value phw-tag-visit-highlight-tag-header)
                                       (const :tag "phw-tag-visit-goto-doc-start"
                                              :value phw-tag-visit-goto-doc-start)
                                       (const :tag "phw-tag-visit-narrow-tag"
                                              :value phw-tag-visit-narrow-tag)
                                       (const :tag "phw-tag-visit-recenter-top"
                                              :value phw-tag-visit-recenter-top)
                                       (const :tag "phw-tag-visit-recenter"
                                              :value phw-tag-visit-recenter)
                                       (function :tag "Function"))))))


(defun phw-tag-visit-function-member-p (fnc)
  (or (member fnc (cdr (assoc 'default phw-tag-visit-post-actions)))
      (member fnc (cdr (assoc major-mode phw-tag-visit-post-actions)))))

(defcustom phw-methods-menu-user-extension nil
...
built-in menu-entries of `phw-methods-menu' but the whole menu can be
re-arranged with `phw-methods-menu-sorter'."
  :group 'phw-methods
  :type (phw-create-menu-user-ext-type 1 phw-max-submenu-depth))


(defcustom phw-methods-menu-user-extension-function 'ignore
...
`phw-methods-menu-user-extension'. This function is called when the user opens
the popup-menu for the methods buffer. For an example how such a function can
be programmed see `phw-methods-menu-editwin-entries'.
...
extensions of `phw-methods-menu-user-extension' but the whole menu can be
re-arranged with `phw-methods-menu-sorter'."
  :group 'phw-methods
  :type 'function)

(defcustom phw-methods-menu-sorter nil
...
`phw-methods-menu-user-extension' and the built-in-menu
`phw-methods-menu'. If nil then no special sorting will be done and the
...
;; (defcustom phw-disable-semantic-threshold-alist nil
...
;; (defun phw-get-max-buffer-size-for-parsing ()
;;   "Threshold set in `phw-disable-semantic-threshold-alist' for
;; current major-mode"
;;   (let ((mode-threshold (cdr (assoc major-mode phw-disable-semantic-threshold-alist)))
;;         (default-threshold (cdr (assoc 'default phw-disable-semantic-threshold-alist))))
;;     (or mode-threshold default-threshold)))

;; (defun phw-prevent-from-parsing-if-exceeding-threshold ()
;;   "Prevents from parsing current buffer if exceeding the threshold
;; defined in `phw-disable-semantic-threshold-alist'."
;;   (if (and (boundp 'phw-minor-mode)
;;            phw-minor-mode
;;            (phw--semantic-active-p))
;;       (let ((threshold (phw-get-max-buffer-size-for-parsing)))
;;         (not (and threshold (> (buffer-size) threshold))))
;;     t))

(defcustom phw-methods-buffer-after-create-hook nil
...
creating the methods-buffer of PHW and it's local key-map. So for example a
function could be added which performs calls of `local-set-key' to define new
key-bindings only for the methods-buffer of PHW."
  :group 'phw-methods
  :type 'hook)


(defcustom phw-process-non-semantic-files (if (locate-library "speedbar")
                                              t)
  "*Display contents of non-semantic-files in the PHW-methods-buffer.
See also `phw-non-semantic-parsing-function'."
  :group 'phw-general
  :group 'phw-non-semantic
  :group 'phw-most-important
  :type 'boolean)


(defcustom phw-non-semantic-parsing-function nil
...
This option takes only effect if `phw-process-non-semantic-files' is not nil:
Then PHW checks for non-semantic buffers if current `major-mode' is contained
...
(defcustom phw-non-semantic-methods-initial-expand nil
...
(defcustom phw-auto-save-before-etags-methods-rebuild t
...
etags \(see `phw-process-non-semantic-files'). Because etags is an external
tool a source-buffer can only be reparsed if the buffer is saved to disk. So
the command `phw-rebuild-methods-buffer' checks for sources which are not
supported by semantic or imenu if either this option is t or if the major-mode
of the source-buffer is contained in this list: In both cases PHW saves the
...
(defcustom phw-non-semantic-exclude-modes '(sh-mode fundamental-mode text-mode)
  "*Exclude modes from parsing with imenu or etags.
Per default, PHW tries to parse all file-types not supported by semantic with
imenu or etags or some other method \(for details see the option
`phw-non-semantic-parsing-function'). If a file-type can not be parsed by
...
(defcustom phw-rebuild-non-semantic-methods-before-hook nil
  "*Hook at beginning of `phw-rebuild-methods-buffer-for-non-semantic'.
So this function is always called by the command `phw-rebuild-methods-buffer'
...
`phw-rebuild-methods-buffer-for-non-semantic' if either the hook contains no
...
;; phw-common-browser.el
(defun phw-enter-debugger (&rest error-args)
  "If `phw-debug-mode' is not nil then enter the Emacs-debugger and signal an
error with ERROR-ARGS."
  (when phw-debug-mode
    (let ((debug-on-error t))
      (apply 'error error-args))))

;; encapsulation all semantic-functions PHW uses if they operate with the
...
(defun phw-semantic-assert-valid-tag (tag &optional no-reparse)
  "Assert that TAG is a valid tag. If not valid then `phw-enter-debugger'
...
check the result if `phw-debug-mode' is nil in which case the function
`phw-enter-debugger' is a no-op."
  (if (phw--semantic-tag-p tag)
      (if (phw--semantic-tag-with-position-p tag)
          (let ((o  (phw--semantic-tag-overlay tag)))
            (if (and (phw--semantic-overlay-p o)
                     (not (phw--semantic-overlay-live-p o)))
                (progn
                  (when (not no-reparse)
                    ;; we need this because:
                    ;; 1. After every jump to a tag X via the method-buffer of
                    ;;    PHW this tag X is added to the navigation history list
                    ;;    as new phw-nav-tag-history-item.
                    ;; 2. Before any select of a source in the sources- or
                    ;;    history-buffer or of a node in the method-buffer
                    ;;    `phw-nav-save-current' is called which operates onto
                    ;;    the last saved history-item which is often a
                    ;;    tag-history-item (see 1.): `phw-nav-save-current'
                    ...
                    ;; During step 2. `phw-nav-save-current' gets the tag
                    ;; from the last tag-history-item and calls for this
                    ;; tag `phw--semantic-tag-start' which fails now because
                    ...
                    ;; PHW-method-buffer PHW is in correct state again!
                    (phw-nav-initialize)
                    (phw--semantic-clear-toplevel-cache)
                    (phw-update-methods-buffer--internal))
                  (phw-enter-debugger "Tag %S is invalid!" tag)
                  nil)
              ;; else, tag is OK.
              t))
    ...
    (phw-enter-debugger "Not a semantic tag: %S" tag)
    nil))
...
;; (defun phw-semantic-tag-buffer-indirect-buffer-check (tag)
...
;; is sufficient for the needs of PHW.
...
;;   (let ((source-buffer (phw-path-selected-source 'buffer))
;;         (buffer-of-tag (phw--semantic-tag-buffer tag)))
;;     (if (and (buffer-base-buffer source-buffer)
;;              (not (equal source-buffer buffer-of-tag))
;;              (equal (phw-path-selected-source 'file)
;;                     (phw-fix-filename (phw-buffer-file-name buffer-of-tag))))
;;         source-buffer
;;       buffer-of-tag)))

(defun phw-semantic-tag-buffer (tag)
  ;; if phw-debug-mode is not nil then the TAG is valid if we pass the
  ;; assert. If phw-debug-mode is nil then we call simply the semantic
  ;; function and see what happens.
  (phw-semantic-assert-valid-tag tag)
  (phw--semantic-tag-buffer tag))

(defun phw-semantic-tag-start (tag)
  ;; if phw-debug-mode is not nil then the TAG is valid if we pass the
  ;; assert. If phw-debug-mode is nil then we call simply the semantic
  ;; function and see what happens.
  (phw-semantic-assert-valid-tag tag)
  (phw--semantic-tag-start tag))


(defun phw-semantic-tag-end (tag)
  ;; if phw-debug-mode is not nil then the TAG is valid if we pass the
  ;; assert. If phw-debug-mode is nil then we call simply the semantic
  ;; function and see what happens.
  (phw-semantic-assert-valid-tag tag)
  (phw--semantic-tag-end tag))

;; Klaus: We must not reparse the buffer if `phw--semantic-current-tag'
...
;; errors!
;; NOT used anymore...
(defun phw-semantic-current-nonterminal ()
  (condition-case nil
      (phw--semantic-current-tag)
    (error (message "phw--semantic-current-tag has problems --> reparsed is performed!")
...
(defun phw-goto-window-methods ()
  "Make the PHW-methods window the current window.
If `phw-use-speedbar-instead-native-tree-buffer' is 'method then goto to the
speedbar-window."
  (interactive)
  (or (phw-goto-phw-window phw-methods-buffer-name)
      (and (equal phw-use-speedbar-instead-native-tree-buffer 'method)
           (phw-goto-window-speedbar))))

(defun phw-maximize-window-methods ()
  "Maximize the PHW-methods-window.
...
PHW-methods-window is not visible in current layout."
  (interactive)
  (if (equal phw-use-speedbar-instead-native-tree-buffer 'method)
      (phw-maximize-window-speedbar)
    (phw-maximize-phw-buffer phw-methods-buffer-name t)))

(defphw-window-dedicator-to-phw-buffer phw-set-methods-buffer phw-methods-buffer-name t
  "Display in current window the methods-buffer and make window dedicated."
  (let ((set-methods-buffer
         (not (equal phw-use-speedbar-instead-native-tree-buffer 'method))))
    ...
    ;; - if phw-use-speedbar-instead-native-tree-buffer is not 'method or
    ...
    (when set-methods-buffer
      ...
      (switch-to-buffer phw-methods-buffer-name))))


(defun phw-create-node (parent-node display name data type)
...
(defun phw-get-tag-type-display (tag-type)
  (let* ((show-tags-list (phw-get-show-tags-list))
         (display (phw-find-assoc tag-type show-tags-list)))
    (if display
	display
      (setq display (phw-find-assoc t show-tags-list))
      (if display
	  display
	'(t hidden nil)))))


(defun phw-get-tag-parent-names (parents)
  (when parents
    (let* ((parent (car parents))
	   (name (cond
		  ((phw--semantic-tag-p parent)
		   (phw--semantic-format-tag-name parent nil (phw-font-lock-tags)))
		  ((stringp parent)
		   (phw--semantic--format-colorize-text parent 'type)))))
      (if name
	  (if (phw-check-parent-for-exclude name)
	      (phw-get-tag-parent-names (cdr parents))
	    (cons name (phw-get-tag-parent-names (cdr parents))))
	(if (listp parent)
	    (append (phw-get-tag-parent-names parent)
		    (phw-get-tag-parent-names (cdr parents))))))))

(defun phw-get-tag-parents (tag)
...
there is no distinction between superclasses and interfaces."
  (phw-get-tag-parent-names
   (append (phw--semantic-tag-type-superclasses tag)
           (phw--semantic-tag-type-interfaces tag))))
;;    (phw--semantic-tag-type-parent tag)))


(defun phw-get-tag-name (tag &optional parent-tag)
  "Get the name of TAG with the appropriate fcn from
`phw-tag-display-function'.
This function MUST be called with the source-buffer as current buffer!"
  (condition-case nil
      (funcall (phw-get-tag-display-function)
               tag parent-tag (phw-font-lock-tags))
    (error (phw--semantic-format-tag-prototype tag parent-tag
                                               (phw-font-lock-tags)))))


(defun phw-find-add-tag-bucket (node type display sort-method buckets
...
PARENT-TAG is only propagated to `phw-add-tag-bucket'.
This function MUST be called with the source-buffer as current buffer!"
  (when (cdr buckets)
    (let ((bucket (cadr buckets)))
      (if (eq type (phw--semantic-tag-class (cadr bucket)))
	  (progn
	    (phw-add-tag-bucket node bucket display sort-method parent-tag
                                  no-bucketize)
	    (setcdr buckets (cddr buckets)))
	(phw-find-add-tag-bucket node type display sort-method
                                 (cdr buckets) parent-tag no-bucketize)))))

(defsubst phw-forbid-tag-display (tag)
  (phw--semantic--tag-put-property tag 'hide-tag t))
  
(defsubst phw-allow-tag-display (tag)
  (phw--semantic--tag-put-property tag 'hide-tag nil))

(defsubst phw-tag-forbidden-display-p (tag)
  (phw--semantic--tag-get-property tag 'hide-tag))

(defsubst phw-show-at-least-one-tag-p (taglist)
  "Not nil if at least one of the tags in TAGLIST should be displayed in the
Methods-buffer."
  (catch 'found
    (dolist (tag taglist)
      (if (not (phw-tag-forbidden-display-p tag))
          (throw 'found t)))
    nil))
...
(defun phw-apply-user-filter-to-tags (taglist)
  "Applies to the tags of TAGLIST the related filter of
`phw-methods-user-filter-alist' - if there is any."
  (save-match-data
    (let ((filters (cdr (assoc (current-buffer) phw-methods-user-filter-alist)))
          (filter-type nil)
          (filter nil)
          (inverse nil))
      (when filters
        (dolist (tag taglist)
          (dolist (filter-spec filters)
            ...
            (unless (equal (phw--semantic-tag-class tag) 'type)
              (cond ((equal filter-type 'regexp)
                     (if (funcall inverse
                                  (not (string-match filter
                                                     (phw--semantic-tag-name tag))))
                         (phw-forbid-tag-display tag)))
                    ((and (member filter '(private protected public))
                          (equal filter-type 'protection))
                     (if (funcall inverse
                                  (not (or (null (phw--semantic-tag-protection tag))
                                           (equal (phw--semantic-tag-protection tag)
                                                  filter))))
                         (phw-forbid-tag-display tag)))
                    ((and (symbolp filter)
                          (equal filter-type 'tag-class))
                     (if (funcall inverse
                                  (not (equal (phw--semantic-tag-class tag) filter)))
                         (phw-forbid-tag-display tag)))
...
(defun phw-tag-generate-node-name (text-name first-chars icon-name)
...
`phw-generate-node-name'."
  (if (phw-use-images-for-semantic-tags)
      (phw-generate-node-name text-name first-chars icon-name
                              phw-methods-buffer-name)
    text-name))


(defun phw-add-tag-bucket (node bucket display sort-method
                                &optional parent-tag no-bucketize)
  "Adds a tag bucket to a node unless DISPLAY equals 'hidden.
This function MUST be called with the source-buffer as current buffer!"
  (when bucket
    (let* ((name-bucket (phw-format-bucket-name (car bucket)))
           (image-name (format "%s-bucket" (phw--semantic-tag-class (cadr bucket))))
           (name (phw-tag-generate-node-name name-bucket -1 image-name))
           ;;(type (phw--semantic-tag-class (cadr bucket)))
           (bucket-node node)
           (new-tag-node nil)
           (new-tag-overlay nil))
      (unless (eq 'hidden display)
        (phw-apply-user-filter-to-tags (cdr bucket))
	(unless (or (eq 'flattened display)
                    ;; we must not create a bucket-node when each tag in the
                    ;; bucket is forbidden to be displayed
                    (not (phw-show-at-least-one-tag-p (cdr bucket))))
	  (setq bucket-node
                (tree-node-new name phw-methods-nodetype-bucket
                               (list 'phw-bucket-node
                                     (car bucket)
                                     (phw--semantic-tag-class (car (cdr bucket))))
                               nil node
                               (if phw-truncate-long-names 'end)))
	  (setf (tree-node->expanded bucket-node) (eq 'expanded display)))
        (dolist (tag (phw-sort-tags sort-method (cdr bucket)))
          ;; we create only a new node for a tag of the bucket when the tag is
          ;; not forbidden to be displayed.
          (unless (phw-tag-forbidden-display-p tag)
            (setq new-tag-node (tree-node-new "" phw-methods-nodetype-tag
                                              tag t bucket-node
                                              (if phw-truncate-long-names 'end)))
            (phw-update-tag-node tag
                                 new-tag-node
                                 parent-tag no-bucketize)
            (setq new-tag-overlay (phw--semantic-tag-overlay tag))
            ;; for partial reparsing/updating each semantic-tag with an
            ;; overlay needs the id of that tree-node which holds as data that
            ;; tag.
            (when (phw-overlay-p new-tag-overlay)
              (phw-overlay-put new-tag-overlay
                               'PHW-tree-node-id
          ...
          (phw-allow-tag-display tag))))))



(defconst phw-tag-image-name-alias-alist
...
(defsubst phw-get-icon-for-tag (abstract-p static-p type protection)
  (cdr (assq protection
              (cdr (assq type
                          (cdr (assq static-p
                                      (cdr (assq abstract-p
                                                  phw-tag-image-name-alias-alist)))))))))


;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: All this tag-icon-display-stuff
;; should be done by semantic - but for now we let do it by PHW because so we
...
(defun phw-displayed-tag-name (tag &optional parent-tag)
  "Return the tag-name of TAG as it will be displayed in the methods-buffer.
This function MUST be called with the source-buffer as current buffer!"
  (let* ((plain-tag-name (phw-get-tag-name tag parent-tag))
         (has-protection (if (= 0 (length plain-tag-name))
                             nil
                           (member (phw-first plain-tag-name)
                                   '(?- ?# ?+))))
         (icon-name (phw-get-icon-for-tag
                     (if (phw--semantic-tag-abstract-p tag parent-tag)
                         'abstract
                       'not-abstract)
                     (if (phw--semantic-tag-static-p tag parent-tag)
                         'static
                       'not-static)
                     (or (and (equal (phw--semantic-tag-class tag)
                                     'type)
                              (intern (phw--semantic-tag-type tag)))
                         (and (phw--semantic-tag-function-constructor-p tag)
                              'constructor)
                         (phw--semantic-tag-class tag))
                     (or (and (phw--semantic--tag-get-property tag 'adopted)
                              'unknown)
                         (and (not (member (phw--semantic-tag-class tag)
                                           '(type function variable)))
                              'unknown)
                         (phw--semantic-tag-protection tag parent-tag)))))
    (phw-tag-generate-node-name plain-tag-name
                                (if has-protection 1 -1)
                                icon-name)))


(defun phw-children-tags (parent-tag)
  ...
  (mapcar (function (lambda (c)
                      (typecase c
                        (phw--semantic-tag
                         c)
                        (string
                         (phw--semantic-tag-new-variable c nil nil nil))
                        (otherwise
                         (phw-error "Tag with name %s contains invalid childrens"
                                    (phw--semantic-tag-name parent-tag))))))
          (phw--semantic-tag-children-compatibility
           parent-tag phw-show-only-positioned-tags)))
                        

(defun phw-update-tag-node (tag node &optional parent-tag no-bucketize)
  "Updates a node containing a tag.
This function MUST be called with the source-buffer as current buffer!"
  (let ((children (phw-children-tags tag))
        (tag-name (phw-displayed-tag-name tag parent-tag)))
    (setf (tree-node->name node) tag-name)
    (unless (eq 'function (phw--semantic-tag-class tag))
      (phw-add-tags node children tag no-bucketize)
      ...
      ;; flexible
      (if (not (eq 'type (phw--semantic-tag-class tag)))
          (setf (tree-node->expanded node) nil)
        (let ((type-specifier (phw-get-type-specifier tag)))
          (setf (tree-node->expanded node)
                (and (tree-node->expandable node)
                     (phw-type-tag-expansion type-specifier))))))))

;; (phw-tag-generate-node-name "klaus" 1 "function-public")

(defun phw-post-process-taglist (taglist)
  "If for current major-mode post-process functions are found in
`phw-post-process-semantic-taglist' then these functions are called with
TAGLIST otherwise TAGLIST is returned.
This function MUST be called with the source-buffer as current buffer!"
  (let ((fcn-list (cdr (assoc major-mode phw-post-process-semantic-taglist))))
    (dolist (fcn fcn-list)
      (if (fboundp fcn)
          (setq taglist (funcall fcn taglist)))))
  (phw-set-current-tag-table taglist)
  ;; now we apply that tag-filters which must operate onto the whole
  ;; tag-table of the current buffer
  (phw-apply-tag-table-filters taglist))

(defun phw-apply-tag-table-filters (taglist)
...
(defun phw-methods-filter-perform-current-type (filter taglist)
    ...
    (if (null curr-type-filter)
      ...
      (catch 'not-found
        (dolist (type-name curr-type-filter)
          (setq found-type-tag
                (car (phw--semantic-find-tags-by-name
                      type-name
                      (phw--semantic-find-tags-by-class 'type new-tag-list))))
          (if (null found-type-tag)
              (progn
                ...
                ;; `phw-replace-first-occurence' (replace the curr filter with
                ;; nil and then do (delq nil filters)
                (phw-methods-filter-apply nil nil nil "" "" (current-buffer))
                (phw-info-message
                 "PHW has removed all filters cause of changes in the type-hierarchy for the current-type!")
            ...
            (setq new-tag-list (phw-children-tags found-type-tag))))
...
(defun phw-group-function-tags-with-parents (taglist)
...
be defined outside the class-definition, e.g. C++, Eieio."
  (phw--semantic-adopt-external-members taglist))

(defun phw-filter-c-prototype-tags (taglist)
  "Filter out all prototypes.
Beginning with version 2.24 of PHW this function does nothing when
`phw-methods-separate-prototypes' is set to not nil \(default).
...
;;   ;; recognize a C-Header-file?
  (if phw-methods-separate-prototypes
      taglist
    (let ((header-extensions '("\\.h\\'" "\\.H\\'" "\\.HH\\'" "\\.hxx\\'" "\\.hh\\'")))
      (or (and (catch 'found
                 (dolist (ext header-extensions)
                   (if (save-match-data
                         (string-match ext (phw-buffer-file-name (current-buffer))))
                       (throw 'found t)))
                 nil)
               taglist)
          (phw-filter taglist
                      (function (lambda (x)
                                  (not (phw--semantic-tag-prototype-p x)))))))))

;; Filtering the Methods-buffer by the user ----------------

(defvar phw-methods-user-filter-alist nil
...
`phw-apply-user-filter-to-tags' whereas tag-filters which have to be applied
onto the whole tag-table are performed by `phw-apply-tag-table-filters'.")


(defun phw-methods-filter-by-prot (inverse source-buffer &optional prot)
  "Filter the Methods-buffer by protection."
  (let ((choice (or prot
                    (phw-query-string "Protection filter:"
                                      '("private" "protected" "public")))))
    (phw-methods-filter-apply 'protection
...
(defun phw-methods-filter-by-tag-class (inverse source-buffer
                                                &optional tag-class)
  "Filter the Methods-buffer by a tag-class."
  (let* ((curr-semantic-symbol->name-assoc-list
          (with-current-buffer source-buffer
            (phw--semantic-symbol->name-assoc-list)))
         (choice (or tag-class
                     (phw-query-string "Tag-class filter:"
                                       (mapcar 'cdr
                                               curr-semantic-symbol->name-assoc-list))))
         (class (or tag-class
                    (symbol-name
                     (car (delq nil (mapcar (function (lambda (e)
                                                        (if (phw-string= (cdr e) choice)
                                                            (car e))))
                                            curr-semantic-symbol->name-assoc-list)))))))
    (phw-methods-filter-apply 'tag-class
...
;; dynamically - see `phw-methods-menu-tagfilter-entries'.

(defun phw-methods-filter-by-regexp (inverse source-buffer &optional regexp)
  "Filter the Methods-buffer by a regular expression."
  (let ((regexp-str (or regexp (read-string "Filter-regexp: "))))
    (phw-methods-filter-apply 'regexp
...
(tree-buffer-defpopup-command phw-methods-filter-by-regexp-popup
  "Filter the Methods-buffer by regexp from popup."
  (phw-methods-filter-by-regexp nil (phw-methods-get-data-store 'source-buffer)))

(tree-buffer-defpopup-command phw-methods-filter-by-regexp-popup-inverse
  "Filter the Methods-buffer by inverse regexp from popup."
  (phw-methods-filter-by-regexp t (phw-methods-get-data-store 'source-buffer)))

(defun phw-methods-filter-by-function (inverse source-buffer &optional fcn-name)
  "Filter the Methods-buffer by a filter-function."
  (let ((filter-fcn-name (or fcn-name
                             (completing-read "Tag-filter-function: "
                                              obarray 'fboundp t))))
    (phw-methods-filter-apply 'function
...
(tree-buffer-defpopup-command phw-methods-filter-by-function-popup
  "Filter the Methods-buffer by function-filter from popup."
  (phw-methods-filter-by-function nil (phw-methods-get-data-store 'source-buffer)))

(tree-buffer-defpopup-command phw-methods-filter-by-function-popup-inverse
  "Filter the Methods-buffer by inverse function-filter from popup."
  (phw-methods-filter-by-function t (phw-methods-get-data-store 'source-buffer)))

(tree-buffer-defpopup-command phw-methods-filter-by-nothing-popup
  "Remove any filter from the Methods-buffer from popup."
  (phw-methods-filter-apply nil nil nil "" ""
                            (phw-methods-get-data-store 'source-buffer)))

(tree-buffer-defpopup-command phw-methods-filter-delete-last-popup
  "Remove the last added filter from the Methods-buffer from popup."
  (phw-methods-filter-apply nil nil nil "" "" (phw-methods-get-data-store 'source-buffer) t))


(defun phw-get-type-node-of-node (node)
  ...
  (let ((parent (tree-node->parent node)))
    (catch 'found
      (while (not (eq (tree-buffer-get-root) parent))
        (if (equal (and (= (tree-node->type parent) phw-methods-nodetype-tag)
                        (phw--semantic-tag-class (tree-node->data parent)))
                   'type)
            (throw 'found parent)
...
(defun phw-get-type-name-hierarchy-of-current-node ()
...
the returned list contains just the name of the tag of the current node."
  (let ((type-hierarchy nil)
    ...
    (when (and curr-node
               (= (tree-node->type curr-node) phw-methods-nodetype-tag))
      (while (progn
               (setq type-hierarchy (cons (phw--semantic-tag-name
                                           (tree-node->data curr-node))
                                          type-hierarchy))
               (setq curr-node (phw-get-type-node-of-node curr-node)))))
    (nreverse type-hierarchy)))


(defun phw-get-type-tag-of-tag (&optional tag table always-parent-type)
...
If TAG is nil the tag returned by `phw-get-real-curr-tag' is used. If TABLE is
nil then the tag-table of the current buffer is used; otherwise the tag-table
TABLE is used."
  (let* ((table (or table (phw-get-current-tag-table)))
         (curr-tag (or tag (phw-get-real-curr-tag)))
         (function-parent (phw--semantic-tag-function-parent curr-tag)))
    (cond ((phw--semantic-tag-faux-p curr-tag)
           (and (not always-parent-type) curr-tag))
          ((and (not always-parent-type)
                (equal (phw--semantic-tag-class curr-tag) 'type))
           curr-tag)
          (t (if function-parent
                 ...
                 (catch 'found
                   (dolist (tag (phw--semantic-flatten-tags-table table))
                     (if (and (equal (phw--semantic-tag-class tag) 'type)
                              (phw-string= (phw--semantic-tag-name tag)
                                           function-parent)
                              (delq nil
                                    (mapcar (lambda (child)
                                              (if (phw--semantic-equivalent-tag-p
                                                   child curr-tag)
                                                  curr-tag))
                                            (phw-children-tags tag))))
                         (throw 'found tag)))
                   nil)
               ...
               (catch 'found
                 (dolist (tag (cdr (reverse
                                    (phw--semantic-find-tag-by-overlay
                                     (phw-semantic-tag-start curr-tag)
                                     (phw-semantic-tag-buffer curr-tag)))))
                   (if (equal (phw--semantic-tag-class tag) 'type)
                       (throw 'found tag)))
                 nil))))))


(defun phw-get-type-name-hierarchy-of-current-tag (&optional tag)
...
TAG. If TAG is nil then the current tag returned by `phw-get-real-curr-tag' is
used; if point does not stay on a tag then nil is returned."
  (let ((type-hierarchy nil)
        (curr-tag (or tag (phw-get-real-curr-tag))))
    (when curr-tag
      (while (progn
               (setq type-hierarchy (cons (phw--semantic-tag-name curr-tag)
                                          type-hierarchy))
               (setq curr-tag (phw-get-type-tag-of-tag curr-tag nil t)))))
    (nreverse type-hierarchy)))

(defun phw-methods-filter-by-current-type (inverse source-buffer &optional
                                                   tag)
  "Display only the current-type and its contents in the methods-buffer. The
argument INVERSE is ignored here."
  (let* ((curr-type-tag (or (and (phw--semantic-tag-p tag)
                                 (with-current-buffer source-buffer
                                   (phw-get-type-tag-of-tag tag)))
                            (cond ((phw-point-in-edit-window-number)
                                   (if (phw--semantic-active-p)
                                       (with-current-buffer source-buffer
                                         (phw-get-type-tag-of-tag (phw-get-real-curr-tag)))))
                                  ((equal (current-buffer)
                                          (phw-buffer-obj phw-methods-buffer-name))
                                   (let ((node (tree-buffer-get-node-at-point)))
                                     (and node
                                          (tree-node->data (phw-get-type-node-of-node node)))))
                                  (t (phw-error "PHW can not identify the current-type-tag!")))))
         (curr-tag-type-name-hierachy (and curr-type-tag
                                           (with-current-buffer source-buffer
                                             (phw-get-type-name-hierarchy-of-current-tag
                                              curr-type-tag)))))
    (if (and curr-type-tag curr-tag-type-name-hierachy)
        (phw-methods-filter-apply 'current-type
                                  (cons curr-tag-type-name-hierachy
                                        'phw-methods-filter-perform-current-type)
                                  nil
                                  "Type"
                                  (phw--semantic-tag-name curr-type-tag)
                                  source-buffer)
      (phw-error "PHW can not identify the current-type!"))))
                          
(tree-buffer-defpopup-command phw-methods-filter-by-current-type-popup
  "Display only the current-type from popup."
  (phw-methods-filter-by-current-type nil
                                      (phw-methods-get-data-store 'source-buffer)
                                      (tree-node->data node)))


(defun phw-get-source-buffer-for-tag-filter ()
  ...
  (cond ((phw-point-in-edit-window-number)
        ...
        ((equal (current-buffer)
                (phw-buffer-obj phw-methods-buffer-name))
         (phw-methods-get-data-store 'source-buffer))
        (t (or (and phw-last-source-buffer
                    (buffer-live-p phw-last-source-buffer)
                    phw-last-source-buffer)
               (phw-error "There is no source-file to filter!")))))
  
(defun phw-methods-filter-inverse ()
  "Apply an inverse filter to the Methods-buffer. This is the same as calling
`phw-methods-filter' with a prefix arg."
  (interactive)
  (phw-methods-filter-internal t))

(defun phw-methods-filter-protection (&optional inverse)
...
`phw-methods-filter'."
  (interactive "P")
  (phw-methods-filter-internal inverse 'protection))

(defun phw-methods-filter-tagclass (&optional inverse)
  "Filter the methods-buffer by tag-class. If INVERSE is not nil \(called
with a prefix arg) then an inverse filter is applied. For further details see
`phw-methods-filter'."
  (interactive "P")
  (phw-methods-filter-internal inverse 'tag-class))

(defun phw-methods-filter-current-type ()
  "Display in the Methods-buffer only the current type and its members. For
further details see `phw-methods-filter'."
  (interactive)
  (phw-methods-filter-internal nil 'curr-type))

(defun phw-methods-filter-regexp (&optional inverse)
  "Filter the methods-buffer by a regexp. If INVERSE is not nil \(called
with a prefix arg) then an inverse filter is applied. For further details see
`phw-methods-filter'."
  (interactive "P")
  (phw-methods-filter-internal inverse 'regexp))

(defun phw-methods-filter-function (&optional inverse)
  "Filter the methods-buffer by a function. If INVERSE is not nil \(called
with a prefix arg) then an inverse filter is applied. For further details see
`phw-methods-filter'."
  (interactive "P")
  (phw-methods-filter-internal inverse 'function))

(defun phw-methods-filter-nofilter ()
  "Remove any filter from the Methods-buffer. For further details see
`phw-methods-filter'."
  (interactive)
  (phw-methods-filter-internal nil 'no-filter))

(defun phw-methods-filter-delete-last ()
  "Remove the most recent filter from the Methods-buffer. For further details see
`phw-methods-filter'."
  (interactive)
  (phw-methods-filter-internal nil 'delete-last))

(defun phw-methods-filter (&optional inverse)
...
- Filter by current type: In languages which have types like Java or C++ this
  ...
  and methods). If PHW can not identify the current type in the source-buffer
...
- No special filter: This means to display all tags specified with the option
  `phw-show-tokens'. If currently some of the above filters are applied they
  will be all removed.
...
Be aware that the tag-list specified by the option `phw-show-tags' is the
...
`phw-methods-filter-replace-existing'. But regardless of the setting in
`phw-methods-filter-replace-existing' applying one of the not-inverse filters
...
See the option `phw-default-tag-filter' if you search for automatically
applied default-tag-filters."
  (interactive "P")
  (phw-methods-filter-internal inverse))

(defun phw-methods-filter-internal (inverse &optional filter-type)
  "FILTER-TYPE has to be one of the symbols 'regexp, 'protection,
'tag-class, 'curr-type, 'function, 'no-filter or 'delete-last."
  (if (with-current-buffer phw-methods-buffer-name
        (tree-buffer-empty-p))
      (message "There is nothing to filter in an empty Methods-buffer!")
    (let* ((source-buffer (phw-get-source-buffer-for-tag-filter))
      ...
      (case choice
        (protection
         (phw-methods-filter-by-prot inverse source-buffer))
        (tag-class
         (phw-methods-filter-by-tag-class inverse source-buffer))
        (regexp
         (phw-methods-filter-by-regexp inverse source-buffer))
        (curr-type
         (phw-methods-filter-by-current-type inverse source-buffer))
        (function
         (phw-methods-filter-by-function inverse source-buffer))
        (delete-last
         (phw-methods-filter-apply nil nil nil "" "" source-buffer t))
        (no-filter
         (phw-methods-filter-apply nil nil nil "" "" source-buffer))
        (otherwise
         (phw-methods-filter-apply nil nil nil "" "" source-buffer))))))

(defun phw-methods-filter-apply (filtertype filter inverse filter-type-display
                                            filter-display
...
otherwise nil."
  (with-current-buffer source-buffer
    (if (and (not remove-last)
             ...
             (not (phw--semantic-active-p)))
        (phw-error "A %s-filter '%s' can only applied to semantic-supported sources!"
                   filtertype filter)))
  (let* ((filter-elem (assoc source-buffer phw-methods-user-filter-alist))
         ...
         (replace-all (and (not remove-last)
                           (not (equal phw-methods-filter-replace-existing 'never))
                           (or (equal phw-methods-filter-replace-existing 'always)
         ...
         (filters (or (and replace-filter-type
                           (progn
                             (setcdr filter-elem
                                     (phw-remove-assoc filtertype (cdr filter-elem)))
    ...
    (if filter-elem
      ...
      (if filters
          (setq phw-methods-user-filter-alist
                (cons (cons source-buffer filters) phw-methods-user-filter-alist)))))
  (when (buffer-live-p source-buffer)
    (with-current-buffer source-buffer
      (if (phw--semantic-active-p)
          ;; For semantic-sources we do not use `phw-rebuild-methods-buffer)'
          ;; because this would always reparse the source-buffer even if not
          ;; necessary.
          (save-restriction
            (widen)
            (phw-rebuild-methods-buffer-with-tagcache
             (phw-fetch-semantic-tags)))
        (phw-rebuild-methods-buffer-fully)))
    (if (with-current-buffer phw-methods-buffer-name
          (tree-buffer-empty-p))
        (progn
          (phw-methods-filter-apply nil nil nil "" "" source-buffer t)
          (message "PHW has not applied this filter because it would filter out all nodes!")
          nil)
      t)))
        
  
(defun phw-methods-filter-modeline-prefix (buffer-name sel-dir sel-source)
...
the option `phw-mode-line-prefixes'."
  (let* ((filters (and sel-source
                       (cdr (assoc (phw-buffer-obj (cdr sel-source))
                                   phw-methods-user-filter-alist))))
         (top-filter-spec (phw-last filters))
...
(defun phw-default-tag-filter-for-current-source ()
  "Check if for the file of the current buffer a default-tag-filter should be
applied. If yes, then the filters-list of `phw-default-tag-filter' is returned
otherwise nil."
  (catch 'found
    (dolist (spec phw-default-tag-filter)
      (let ((m-mode (caar spec))
            (regexp (cdar spec)))
        (if (and (equal major-mode m-mode)
                 (save-match-data
                   (string-match regexp (phw-buffer-file-name (current-buffer)))))
            (throw 'found (cdr spec)))))
    nil))

(defun phw-apply-default-tag-filter ()
  "Applies all default-tag-filters specified in `phw-default-tag-filter' for
the current file."
  (remove-hook 'post-command-hook 'phw-apply-default-tag-filter)
  (ignore-errors
    (let ((tag-filter-list (phw-default-tag-filter-for-current-source)))
      (dolist (filter-spec tag-filter-list)
        (let ((filter-apply-fcn
               (case (nth 0 filter-spec)
                 (protection 'phw-methods-filter-by-prot)
                 (tag-class  'phw-methods-filter-by-tag-class)
                 (regexp 'phw-methods-filter-by-regexp)
                 (function 'phw-methods-filter-by-function)))
              (filter
               (case (nth 0 filter-spec)
                 (protection
                  (typecase (nth 1 filter-spec)
                    ...
                    (otherwise
                     (phw-error "Not a valid tag-filter: %s" (nth 1 filter-spec)))))
                 (tag-class
                  (typecase (nth 1 filter-spec)
                    (symbol (symbol-name (nth 1 filter-spec)))
                    (string (nth 1 filter-spec))
...
(defun phw-find-file-hook ()
  "Adds `phw-apply-default-tag-filter' to `post-command-hook'. This function
removes itself from the `post-command-hook'."
  (add-hook 'post-command-hook 'phw-apply-default-tag-filter))

;; adding tags to the Methods-buffer 

(defun phw-add-tags (node tags &optional parent-tag no-bucketize)
  "Add TAGS to the node NODE.
If NO-BUCKETIZE is not nil then TAGS will not bucketized by
`phw--semantic-bucketize' but must already been bucketized! If not nil
...
This function MUST be called with the source-buffer as current buffer!"
  (phw-add-tag-buckets
   ...
   (if no-bucketize
       tags
     (phw--semantic-bucketize tags
                              (and parent-tag
                                   (phw--semantic-symbol->name-assoc-list-for-type-parts)
                                   (equal (phw--semantic-tag-class parent-tag)
                                          'type))))
   no-bucketize))


(defun phw-access-order (access)
...
(defun phw-sort-tags (sort-method tags)
  (if sort-method
      (let ((tags-by-name
	     (sort tags (function (lambda (a b)
				      (phw-string< (phw--semantic-tag-name a)
                                                   (phw--semantic-tag-name b)))))))
	(if (eq 'access sort-method)
	    (sort tags-by-name
		  (function
		   (lambda (a b)
		     (< (phw-access-order (phw--semantic-tag-protection a))
			(phw-access-order (phw--semantic-tag-protection b))))))
	  tags-by-name))
    tags))


(defun phw-add-tag-buckets (node parent-tag buckets &optional no-bucketize)
  "Creates and adds tag nodes to the given node.
The PARENT-TAG is propagated to the functions `phw-add-tag-bucket' and
`phw-find-add-tag-bucket'.
This function MUST be called with the source-buffer as current buffer!"
  (setq buckets (cons nil buckets))
  (dolist (tag-display (phw-get-show-tags-list))
    (let* ((type (car tag-display))
      ...
      (cond
       ((eq 'parent type)
 	(when (and parent-tag
 		   (eq 'type (phw--semantic-tag-class parent-tag)))
 	  (let ((parents (phw-get-tag-parents parent-tag)))
	    (when parents
	      (let* ((name-bucket (phw-format-bucket-name "Parents"))
                     (name (phw-tag-generate-node-name name-bucket -1
                                                       "parent-bucket"))
                     (parent-node nil))
                (setq parent-node (phw-create-node node display
                                                   name
                                                   (list 'phw-bucket-node
                                                         "Parents"
                                                         'parent)
                                                   phw-methods-nodetype-bucket))
                (when node
		  (dolist (parent (if sort-method
				      (sort parents 'phw-string<) parents))
                    (let* ((plain-parent-name
                            (if (phw-font-lock-tags)
                                (phw--semantic--format-colorize-text parent 'type)
                              parent))
                           ...
                           (parent-name (phw-tag-generate-node-name plain-parent-name
                                                                    -1
                                                                    "parent-unknown")))
                      (tree-node-new parent-name
                                     phw-methods-nodetype-externtag
                                     parent t parent-node
                                     (if phw-truncate-long-names 'end))))))))))
       (t (phw-find-add-tag-bucket node type display sort-method buckets
                                   parent-tag no-bucketize)))))
  (let ((type-display (phw-get-tag-type-display t)))
    (dolist (bucket buckets)
      (phw-add-tag-bucket node bucket (cadr type-display)
                            (caddr type-display) parent-tag no-bucketize))))

(defvar phw-partial-reparse-always-full-fetch nil
  "Always perform a full tag fetching when a partial reparse occur.

This is only a fallback if partial reparsing support of PHW is too buggy. In
this case set it to not nil.")

(defvar phw-partial-reparse-debug nil
  "If not nil the partial reparse support of PHW outputs to message buffer.")

(defun phw-partial-reparse-debug (&rest args)
  "Run ARGS through `format' and write it to the *Messages*-buffer.
Does nothing if `phw-partial-reparse-debug' is nil."
  (when (and phw-partial-reparse-debug args)
    (message (concat (format "PHW %s partial-reparse debug: " phw-version)
                     (apply 'format args)))))

(defun phw-update-after-partial-reparse (updated-tags)
  "Updates the method buffer and all internal PHW-caches after a partial
semantic-reparse. This function is added to the hook
`semantic-after-partial-cache-change-hook'.

If `phw-partial-reparse-always-full-fetch' is not nil then always a full fetch
for tags is performed; this is just a fallback if there are problems with the
partial reparse and update-mechanism."
  (if (or phw-partial-reparse-always-full-fetch
          ...
          (catch 'tag-without-overlay
            (dolist (tag updated-tags)
              (when (not (phw-overlay-p (phw--semantic-tag-overlay tag)))
                (phw-partial-reparse-debug "Called for a tag without overlay:%s - full-fetch needed" tag)
                (throw 'tag-without-overlay t)))
            nil))
      ;; perform a full fetch
      (phw-rebuild-methods-buffer-with-tagcache (phw-fetch-semantic-tags))

    ;; now our partial reparse/update mechanism begins
    (phw-partial-reparse-debug "Partial reparse starts for the tags:")
    (dolist (tag updated-tags)
      (phw-partial-reparse-debug "- Tag-name:%s,tag-class:%s,full-tag:%s"
                                 (phw--semantic-tag-name tag)
                                 (phw--semantic-tag-class tag)
                                 tag))
    (when (and phw-minor-mode ;; PHW must be active - just for saveness
               ;; PHW-frame must be active
               ...
               (phw-buffer-is-phw-buffer-of-current-layout-p phw-methods-buffer-name)
               ;; current buffer must have a filename.
               (phw-buffer-file-name (current-buffer))
               ;; the parsed buffer must be displayed in a window within the
               ;; PHW-frame: This prevents the methods-buffer being confused by
               ;; the background-parsing of other files by semantic-idle-timer
               (or phw-method-buffer-rebuild-allowed-for-invisible-buffers
                   (get-buffer-window (current-buffer) phw-frame)))
      (let ((full-fetch-needed nil)
            (taglist updated-tags)
            (tag nil)
            (at-least-one-node-updated nil)
            (post-processed-list nil)
            (fcn-list (cdr (assoc major-mode phw-post-process-semantic-taglist))))
        (while (and (not full-fetch-needed) taglist)
          (setq tag (car taglist))
          (phw-partial-reparse-debug "Processing tag: %s" (phw--semantic-tag-name tag))
          ...
          (let* ((one-tag-list (list tag))
                 (tag-for-update nil)
                 (tag-ident-name (phw--semantic-tag-name tag))
                 (tag-ident-class (phw--semantic-tag-class tag))
                 (tag-ident-parent (phw--semantic-tag-named-parent tag))
                 (tag-ident-parent-name (if (phw--semantic-tag-p tag-ident-parent)
                                            (phw--semantic-tag-name tag-ident-parent)
                                          tag-ident-parent)))
            (phw-partial-reparse-debug "tag-ident-name:%s,tag-ident-class:%s,tag-ident-parent:%s"
                                       tag-ident-name
                                       tag-ident-class
            ...
            (phw-partial-reparse-debug "one-tag-list after post-processing: %s"
                                       one-tag-list)
            ;; now we apply that tag-filters which must operate onto the whole
            ;; tag-table - but here we use each tag as a separate tag-table
            (setq one-tag-list (phw-apply-tag-table-filters one-tag-list))
            (phw-partial-reparse-debug "one-tag-list after tag-table filtering %s"
                                       one-tag-list)
            ;; now we must see how get from the one-tag-list our orginal tag so
            ;; we can apply the tag-filters:
            (if (null one-tag-list)
                (phw-partial-reparse-debug "tag has been filtered out by tag-table filters")
              (if (and (equal (phw--semantic-tag-name (car one-tag-list))
                              tag-ident-name)
                       (equal (phw--semantic-tag-class (car one-tag-list))
                              tag-ident-class))
                ...
                ;; case)
                (let ((try (car (phw--semantic-tag-components-with-overlays
                                 (car one-tag-list)))))
                  (phw-partial-reparse-debug "We try the first member - it is: %s" try)
                  (when (phw--semantic-tag-p try)
                    (if (and (equal (phw--semantic-tag-name try)
                                    tag-ident-name)
                             (equal (phw--semantic-tag-class try)
                                    tag-ident-class))
              ...
              (if (null tag-for-update)
                  ...
                  (progn
                    (phw-partial-reparse-debug "full-fetch-needed because we didn't found our tag")
                ...
                (phw-partial-reparse-debug "tag-for-update before tag-filters: %s" tag-for-update)
                (phw-apply-user-filter-to-tags (list tag-for-update))
                (phw-partial-reparse-debug "tag-for-update after tag-filters: %s" tag-for-update)
                (if (phw-tag-forbidden-display-p tag-for-update)
                    ...
                    (progn
                      (phw-partial-reparse-debug "tag-for-update is filtered out by tag-filters")
                      (phw-allow-tag-display tag-for-update)
                      )
                  ...
                  (let* ((id (phw-overlay-get (phw--semantic-tag-overlay tag-for-update)
                                              'PHW-tree-node-id))
                         (node (when id
                                 (with-current-buffer phw-methods-buffer-name
                                   (tree-node-search-subtree-by-id (tree-buffer-get-root)
                                                                   id))))
                         (data-parent (and node
                                           (phw--semantic-tag-named-parent (tree-node->data node))))
                         (data-parent-name (if (phw--semantic-tag-p data-parent)
                                               (phw--semantic-tag-name data-parent)
                                             data-parent)))
                    (if (and node (equal data-parent-name tag-ident-parent-name))
                        ...
                        (let ((new-tag-name (phw-displayed-tag-name tag-for-update)))
                          ...
                          ;; post-processed tag
                          (phw-partial-reparse-debug "We update node for tag:%s with new name:%s"
                                                     (phw--semantic-tag-name tag-for-update)
                                                     new-tag-name)
                          (with-current-buffer phw-methods-buffer-name
                      ...
                      (phw-partial-reparse-debug "full-fetch-needed: id:%s,node-p:%s,parent-old:%s,parent-new:%s"
                                                 id (and node t)
        ...
        (when (and (not full-fetch-needed) at-least-one-node-updated)
          (phw-partial-reparse-debug "We perform tree-buffer-update")
          (phw-exec-in-window phw-methods-buffer-name
            (tree-buffer-update)))

        (if full-fetch-needed
            (progn
              (phw-rebuild-methods-buffer-with-tagcache (phw-fetch-semantic-tags))
              (phw-partial-reparse-debug "full fetch has been performed")
              )
          ;; as last step we must update the current-tag-table
          ;; for current source-buffer
          ;; remark: There is no need to update the internal
          ;; phw-tag-tree-cache because we haven't created or deleted any
          ;; tree-nodes but only updated existing ones. So the
          ;; phw-tag-tree-cache contains autom. the new datas of the modified
          ;; tree-nodes because the cache contains the same cells!
          (phw-partial-reparse-debug "We set current tag table")
          (phw-set-current-tag-table (phw-fetch-semantic-tags))
          )))))

;; currently not used
(defun phw-semantic-active-for-file (filename)
...
(defun phw-update-methods-after-saving ()
...
current-buffer is saved."
  (when (and (equal (selected-frame) phw-frame)
             phw-auto-update-methods-after-save
             ...
             (equal (current-buffer)
                    (window-buffer phw-last-edit-window-with-point)))
    (phw-select-source)
    (if (phw--semantic-active-p)
        (phw-update-methods-buffer--internal nil nil t)
      (phw-rebuild-methods-buffer-for-non-semantic))))

(defun phw-fetch-semantic-tags (&optional full)
...
`phw-force-reparse-when-semantic-idle-scheduler-off' is not nil then use
`phw--semantic-fetch-tags' otherwise use `phw--semantic-fetch-available-tags'.
...
necessary!"
  (if (or full
          (and (or (not (boundp 'semantic-idle-scheduler-mode))
               ...
               phw-force-reparse-when-semantic-idle-scheduler-off)
          )
      (phw--semantic-fetch-tags)
    (phw--semantic-fetch-available-tags)))


(defvar phw-method-buffer-needs-rebuild t
  "This variable is only set and evaluated by the functions
`phw-update-methods-buffer--internal' and
`phw-rebuild-methods-buffer-with-tagcache'!")

(defvar phw-method-buffer-rebuild-allowed-for-invisible-buffers nil
  "This variable is only set and evaluated by the functions
`phw-update-methods-buffer--internal' and
`phw-rebuild-methods-buffer-with-tagcache'!")


(defun phw-update-methods-buffer--internal (&optional scroll-to-top
...
done by `phw-rebuild-methods-buffer-with-tagcache' because this function is in
...
`phw-rebuild-methods-buffer-with-tagcache'. The function
`phw-rebuild-methods-buffer-for-non-semantic' is the only one settings this
argument to not nil!
...
drive `phw-rebuild-methods-buffer-with-tagcache' so it does not prevent
rebuilding a not visible source-buffer."
  (when (and (equal (selected-frame) phw-frame)
             (get-buffer-window phw-methods-buffer-name))
    ;; Set here `phw-method-buffer-needs-rebuild' to t so we can see below if
    ;; `phw-rebuild-methods-buffer-with-tagcache' was called auto. after
    ;; `phw-fetch-semantic-tags'.
    (setq phw-method-buffer-needs-rebuild t)
    ;; set this global variable according to the value of
    ;; invisible-sourcebuffer-allowed so that the hook-driven call to
    ;; `phw-rebuild-methods-buffer-with-tagcache' and the call below as well
    ;; do not prevent rebuilding an invisible source-buffer.
    ;; `phw-method-buffer-rebuild-allowed-for-invisible-buffers' is evaluated
    ;; by `phw-rebuild-methods-buffer-with-tagcache' and also reset to nil at
    ;; the end.
    (setq phw-method-buffer-rebuild-allowed-for-invisible-buffers
          invisible-sourcebuffer-allowed)

    (let ((current-tagcache (and (phw--semantic-active-p)
                                   ;; if we manually bovinate the buffer we
                                   ;; must widen the source to get all tags.
                                   (save-excursion
                                     (save-restriction
                                       (widen)
                                       (phw-fetch-semantic-tags full-semantic))))))
      ;; If the `phw-fetch-semantic-tags' has done no reparsing but
      ...
      ;; therefore `phw-rebuild-methods-buffer-with-tagcache' was not called.
      ;; Therefore we call it here manually.
      ;; `phw-rebuild-methods-buffer-with-tagcache' is the only function which
      ;; sets `phw-method-buffer-needs-rebuild' to nil to signalize that a
      ;; "manually" rebuild of the method buffer is not necessary.
      ;;
      ;; `phw-update-methods-buffer--internal' is called by
      ;; `phw-basic-buffer-sync' and `phw-set-selected-source' (depending
      ...
      ;; `phw-rebuild-methods-buffer-with-tagcache'...
      (when phw-method-buffer-needs-rebuild
        ...
        (phw-rebuild-methods-buffer-with-tagcache
         ...
         (and (phw--semantic-active-p) (not full-semantic))
         ;; we force a rebuild also for an empty cache because an empty
         ;; cache is returned by `phw-fetch-semantic-tags' when a file
         ...
         ;; `phw-rebuild-methods-buffer-with-tagcache'. This is maybe a
...
(setq phw-tag-tree-cache nil)

(defun phw-clear-tag-tree-cache (&optional source-name)
  "Clears either the whole tag-tree-cache \(SOURCE-NAME is nil) or
removes only the tag-tree for SOURCE-NAME from the cache."
  (if (not source-name)
      (setq phw-tag-tree-cache nil)
    (setq phw-tag-tree-cache
          (delq (assoc source-name phw-tag-tree-cache) phw-tag-tree-cache))))

(defvar phw-current-post-processed-tag-table nil
  "This is the current tag-table of the current source-buffer returned by
`phw-post-process-taglist'. Do not set this variable, only the function
`phw-rebuild-methods-buffer-with-tagcache' is allowed to do this.")
(make-variable-buffer-local 'phw-current-post-processed-tag-table)

(defun phw-get-current-tag-table ()
  "Return the current tag-table of the current source-buffer returned by
`phw-post-process-taglist'. Use always this function if you just need the
current post-processed tag-table of the current buffer and you do not need or
want rebuilding the Methods-buffer."
  phw-current-post-processed-tag-table)

(defun phw-set-current-tag-table (table)
  "Set the current tag-table of the current source-buffer to TABLE. Return
TABLE."
  (setq phw-current-post-processed-tag-table table))


(defun phw-methods-get-data-store (key)
  "Get the value for KEY from the tree-buffer-data-store of the Methods-buffer."
  (with-current-buffer phw-methods-buffer-name
    (cdr (assoc key (tree-buffer-get-data-store)))))
  

(defun phw-rebuild-methods-buffer-with-tagcache (updated-cache
                                                 &optional no-update-semantic
                                                 force-nil-cache
                                                 non-semantic-rebuild)
  "Rebuilds the PHW-method buffer after toplevel-parsing by semantic. This
function is added to the hook `semantic-after-toplevel-cache-change-hook'.

If NO-UPDATE-SEMANTIC is not nil then the tags of the PHW-methods-buffer are
not updated with UPDATED-CACHE but the method-buffer is rebuild with these
tags PHW has already cached in it `phw-tag-tree-cache'. Only relevant for
...
to be rescanned/reparsed and therefore the Method-buffer will be rebuild too."
  ...
  (when (and phw-minor-mode ;; PHW must be active - just for saveness
             ...
             ;; `phw-update-methods-buffer--internal' then we do an update
    ...
    (if (not (phw--semantic-active-p)) (setq no-update-semantic nil))

    ;; the following cache-mechanism MUST use the (buffer-name) instead of
    ;; phw-path-selected-source because in case of opening a buffer not via
    ;; directory-window but via the standard-mechanism of Emacs this function
    ;; is called via hook BEFORE phw-path-selected-source is set currently by
    ;; the synchronize-mechanism of PHW. Also if we create a new cache-element
    ...
    (let* ((cache-key (buffer-name))
           (cache (assoc cache-key phw-tag-tree-cache))
           (curr-buff (current-buffer))
           (tag-sync-necessary nil)
           (curr-major-mode major-mode)
           (ezimage-use-images (if (phw-use-images-for-semantic-tags)
                                   nil
                                 (phw--ezimage-use-images)))
           (semantic-format-use-images-flag (if (phw-use-images-for-semantic-tags)
                                                nil
                                              (phw--semantic-format-use-images-flag)))
           (my-format-face-alist (if (phw-use-images-for-semantic-tags)
                                     (phw-remove-assoc 'abstract
                                                       (phw-remove-assoc 'static
                                                                         (phw--semantic-format-face-alist)))
                                   (phw--semantic-format-face-alist)))
           (semantic-format-face-alist my-format-face-alist)
           (semantic-bucketize-tag-class
            (if phw-methods-separate-prototypes
                (function (lambda (tag)
                            (if (phw--semantic-tag-prototype-p tag)
                                'prototype
                              (phw--semantic-tag-class tag))))
              semantic-bucketize-tag-class))
           (semantic-symbol->name-assoc-list-for-type-parts
            (and (phw--semantic-active-p)
                 (phw--semantic-symbol->name-assoc-list-for-type-parts)
                 (or (and (null (cdr (assoc 'function
                                            (phw--semantic-symbol->name-assoc-list-for-type-parts))))
                          (phw--semantic-symbol->name-assoc-list-for-type-parts))
                     (append (phw--semantic-symbol->name-assoc-list-for-type-parts)
                             (list (cons 'prototype
                                         (format "%s-prototypes"
                                                 (phw-string-make-singular
                                                  (cdr (assoc 'function
                                                              (phw--semantic-symbol->name-assoc-list-for-type-parts)))))))))))
           (semantic-symbol->name-assoc-list
            (and (phw--semantic-active-p)
                 ...
                 (or (and (null (cdr (assoc 'function
                                            (phw--semantic-symbol->name-assoc-list))))
                          (phw--semantic-symbol->name-assoc-list))
                     (append (phw--semantic-symbol->name-assoc-list)
                             (list (cons 'prototype
                                         (format "%s-prototypes"
                                                 (phw-string-make-singular
                                                  (cdr (assoc 'function
      ...
      (if phw-debug-mode
          (dolist (a-tag updated-cache)
            (phw-semantic-assert-valid-tag a-tag)))
      ...
      ;; etags/imenu)
      (when (and phw-process-non-semantic-files
                 (null updated-cache)                 
                 (not (phw--semantic-active-p))
                 (phw-buffer-file-name (current-buffer))
                 (or non-semantic-rebuild (null cache)))
        (setq updated-cache (ignore-errors
                              (phw-get-tags-for-non-semantic-files)))
      ...
      (unless (or (and no-update-semantic cache) ;; for semantic-sources
                  (and (not (phw--semantic-active-p)) ;; for non-semantic-sources
                       (not non-semantic-handling)
                       ;; for clearing out non-semantic-buffers too after
                       ;; killing one; see `phw-kill-buffer-hook'.
                       (not force-nil-cache)))
        ...
        (if non-semantic-handling
            (if (equal non-semantic-handling 'parsed)
                (phw-create-non-semantic-tree new-tree updated-cache))
          (phw-add-tags new-tree
                        (phw-post-process-taglist updated-cache)))
        (if cache
            (setcdr cache new-tree)
          (setq cache (cons cache-key new-tree))
          (setq phw-tag-tree-cache (cons cache phw-tag-tree-cache))))
      ...
      (with-current-buffer phw-methods-buffer-name
        ...
        (setq phw-methods-root-node (cdr cache))
        (tree-buffer-update))
    ...
    ;; we have changed the implementation of phw-navigate.el from storing
    ...
    (setq phw-method-buffer-needs-rebuild nil))

  ;; prevent from allowing rebuilding for invisible buffers unless it is
  ;; explicitly demanded
  (setq phw-method-buffer-rebuild-allowed-for-invisible-buffers nil))

(defun phw-save-without-auto-update-methods ()
  (let ((phw-auto-update-methods-after-save nil))
    (save-buffer)))


(defun phw-rebuild-methods-buffer-for-non-semantic ()
  "Rebuild the PHW-method-buffer for current source-file of the edit-window.
This function does nothing if point stays not in an edit-window of the
PHW-frame or if current source-file is supported by semantic!

Before rebuilding the Methods-buffer the hook
`phw-rebuild-non-semantic-methods-before-hook' is called. The Method-buffer is
...
The option `phw-auto-save-before-etags-methods-rebuild' is checked before
rescanning the source-buffer and rebuilding the methods-buffer.

This function is called by the command `phw-rebuild-methods-buffer'."
  (when (and phw-minor-mode
             (equal (selected-frame) phw-frame)
             (not (phw--semantic-active-p))
             (not (member major-mode phw-non-semantic-exclude-modes))
             (phw-point-in-edit-window-number))
    (when (run-hook-with-args-until-failure
           'phw-rebuild-non-semantic-methods-before-hook
           (phw-buffer-file-name))
      ;; For etags supported non-semantic-sources we maybe have to save the
      ;; buffer first.
      (when (and (buffer-modified-p)
                 (not (and (boundp 'imenu--index-alist)
                           imenu--index-alist))
                 (or (equal phw-auto-save-before-etags-methods-rebuild t)
                     (member major-mode
                             phw-auto-save-before-etags-methods-rebuild)))
        ;; to prevent files from being parsed too often we need to temp.
        ;; switch off the auto-method-updating-after-save feature
        (phw-save-without-auto-update-methods))
      (phw-update-methods-buffer--internal nil t))))


(defun phw-rebuild-methods-buffer-for-semantic (&optional clear-cache)
  "Rebuild the PHW-method-buffer for current source-file of the edit-window.
This function does nothing if point stays not in an edit-window of the
PHW-frame or if current source-file is not supported by semantic!
...
reparsing."
  (when (and phw-minor-mode
             ...
             (phw--semantic-active-p)
             (phw-point-in-edit-window-number))
    (and clear-cache (phw--semantic-clear-toplevel-cache))
    (phw-update-methods-buffer--internal nil nil t)))


(defun phw-rebuild-methods-buffer (&optional clear-cache)
  "Updates the methods buffer with the current source-buffer.
This means PHW asks the parsing-engine for tags for the current
...
+ If an elisp-file is parsed which contains in the middle a defun X where the
  ...
  and you will get an incomplete PHW-method buffer. In such a case you must
  complete the defun X and then call this function to completely reparse the
  elisp-file and rebuild the PHW method buffer!

+ For not semantic supported buffers which can be parsed by imenu or etags
  \(see `phw-process-non-semantic-files') because for these buffers there is
...
`phw-auto-save-before-etags-methods-rebuild' is checked before rescanning the
source-buffer and rebuilding the methods-buffer.

If point is in one of the phw-windows or in the compile-window then this
command rebuids the methods-buffer with the contents of the source-buffer the
last selected edit-window."
  (interactive "P")
  (save-selected-window
    (when (not (phw-point-in-edit-window-number))
      (let ((phw-mouse-click-destination 'last-point))
        (phw-select-edit-window)))
    (phw-rebuild-methods-buffer-fully clear-cache)))

(defun phw-rebuild-methods-buffer-fully (&optional clear-cache)
...
the tags are fetched by `phw--semantic-fetch-tags' for semantic-sources!
  ...
  (when (phw-point-in-edit-window-number)
    (if (phw--semantic-active-p)
        (let ((semantic-idle-scheduler-max-buffer-size 0))
          (phw-rebuild-methods-buffer-for-semantic clear-cache))
      (phw-rebuild-methods-buffer-for-non-semantic))))

(defvar phw-auto-expand-tag-tree-old 'expand-spec)

(defun phw-toggle-auto-expand-tag-tree (&optional arg)
  "Toggle auto expanding of the PHW-methods-buffer.
...
`phw-auto-expand-tag-tree' is saved so it can be used for the next switch on
by this command."
  (interactive "P")
  (let* ((new-value (if (null arg)
                        (if phw-auto-expand-tag-tree
                            (progn
                              (setq phw-auto-expand-tag-tree-old
                                    phw-auto-expand-tag-tree)
                              nil)
                          phw-auto-expand-tag-tree-old)
                          ...
                          (progn
                            (if phw-auto-expand-tag-tree
                                (setq phw-auto-expand-tag-tree-old
                                      phw-auto-expand-tag-tree))
                            nil)
                        (or phw-auto-expand-tag-tree
                            phw-auto-expand-tag-tree-old)))))
    (setq phw-auto-expand-tag-tree new-value)
...
(defun phw-get-real-curr-tag ()
  "Get the \"real\" current tag. This will be in most cases the tag returned
by `phw--semantic-current-tag' but there are exceptions:
...
- If the current-tag is a label-tag then we are interested in the type-tag
  ...
  (let* ((tagstack (reverse (phw--semantic-find-tag-by-overlay)))
         (curr-tag (car tagstack))
         (next-tag (car (cdr tagstack)))
         )
    (if (or (and (equal (phw--semantic-tag-class curr-tag) 'variable)
                 (equal (phw--semantic-tag-class next-tag) 'function)
                 (member curr-tag
                         (phw--semantic-tag-function-arguments next-tag)))
            (equal (phw--semantic-tag-class curr-tag) 'label))
        (setq curr-tag next-tag))
    curr-tag))


(defun phw-try-highlight-tag (highlight-tag curr-tag table)
...
current buffer."
  (let* ((type-tag (and curr-tag
                        (phw-get-type-tag-of-tag curr-tag table t)))
         (bucket-data
          (and (not type-tag)
               (list 'phw-bucket-node
                     (cdr (assoc (or (and (phw--semantic-tag-prototype-p highlight-tag)
                                          'prototype)
                                     (phw--semantic-tag-class highlight-tag))
                                 (phw-methods-get-data-store
                                  'semantic-symbol->name-assoc-list)))
                     (phw--semantic-tag-class highlight-tag))))
         (type-node nil))
    (or (and curr-tag
             ;; this is save because phw-try-highlight-tag runs only within
             ;; phw-tag-sync which is defined to do nothing if
             ;; phw-methods-buffer-name is not visible in the current layout
             ;; ==> the return value 'window-not-visible is not possible here!
             (phw-exec-in-window phw-methods-buffer-name
               (or (tree-buffer-highlight-node-by-data/name
                    highlight-tag nil nil
                    (equal phw-highlight-tag-with-point 'highlight))
                   ...
                   (when (and highlight-tag
                              ...
                              ;;(member (car (cdr (assoc (phw--semantic-tag-class highlight-tag)
                              ;;                         (phw-get-show-tags-list))))
                              ;;        '(expanded collapsed))
                              (or (equal phw-auto-expand-tag-tree 'all)
                                  (member (phw--semantic-tag-class highlight-tag)
                                          (phw-normalize-expand-spec
                                           phw-methods-nodes-expand-spec))))
                     (let ((bucket-node
                            (tree-buffer-search-displayed-node-list
                             (function (lambda (node)
                                         (if (and (tree-buffer-node-data-equal-p
                                             ...
                                             node))))))
                       (when bucket-node
                         (phw-expand-methods-node-internal
                          bucket-node
                          100
                          (equal phw-auto-expand-tag-tree 'all)
                          nil t)
        ...
        (if curr-tag
            (phw-try-highlight-tag highlight-tag type-tag table)))))

(defvar phw-tag-sync-do-nothing nil
  "Only set by `phw-display-tag' and only evaluated by `phw-tag-sync'")
...
(defphw-autocontrol/sync-function phw-tag-sync phw-methods-buffer-name nil nil
  "Synchronizing tag-display of the methods-buffer with current point."
  (if nil ;; phw-tag-sync-do-nothing
      ...
      ;; `phw-tag-visit-post-actions') and if we would sync then in the
      ...
      ;; phw-tag-visit-post-actions are forbidden to put the point outside
      ...
      ;; code in because im not at 100% sure if there are other needs in PHW
      ;; which need this mechanism - but for now we can disable it.... ;-)
      (setq phw-tag-sync-do-nothing nil)
    (when phw-highlight-tag-with-point
      (let ((curr-tag (phw-get-real-curr-tag)))
        (when (or force (not (equal phw-selected-tag curr-tag)))
          (setq phw-selected-tag curr-tag)
          (if (null curr-tag)
              (phw-exec-in-window phw-methods-buffer-name
                ;; If there is no tag to highlight then we remove the
                ;; highlighting
                (tree-buffer-highlight-node-by-data/name nil)
                (if (equal phw-auto-expand-tag-tree-collapse-other 'always)
            ...
            (if phw-auto-expand-tag-tree-collapse-other
                (phw-exec-in-window phw-methods-buffer-name
                  (when (and curr-tag
                             (or (equal phw-auto-expand-tag-tree 'all)
                                 (member (phw--semantic-tag-class curr-tag)
                                         (phw-normalize-expand-spec
                                          phw-methods-nodes-expand-spec))))
                    (phw-expand-methods-node-internal
                     (tree-buffer-get-root)
            ...
            (when (not (phw-try-highlight-tag curr-tag curr-tag
                                              (phw-get-current-tag-table)))
              ...
              (phw-exec-in-window phw-methods-buffer-name
                (when (and curr-tag
                  ...
                  (phw-expand-methods-node-internal
                   ...
                   nil t)
                  (tree-buffer-highlight-node-by-data/name
                   curr-tag nil nil (equal phw-highlight-tag-with-point 'highlight)))
                ))))))))


(defun phw-string-make-singular (string)
  (if (equal (aref string (1- (length string))) ?s)
      (substring string 0 (1- (length string)))
    string))


(defun phw-methods-node-get-semantic-type (node)
  (cond ((= phw-methods-nodetype-bucket (tree-node->type node))
         (nth 2 (tree-node->data node)))
        ((= phw-methods-nodetype-tag (tree-node->type node))
         (ignore-errors (phw--semantic-tag-class (tree-node->data node))))
        (t nil)))


(defun phw-expand-methods-nodes (&optional force-all)
  "Set the expand level of the nodes in the PHW-methods-buffer.
...
the options `phw-methods-nodes-expand-spec' and
`phw-methods-nodes-collapse-spec'! With optional argument FORCE-ALL all tags
...
`phw-expand-methods-switch-off-auto-expand' is not nil. But it can be switched
on again quickly with `phw-toggle-auto-expand-tag-tree' or \[C-c . a].
...
`phw-process-non-semantic-files') FORCE-ALL is always true!"
  (interactive "P")
  (let* ((first-node (save-excursion
                       (goto-char (point-min))
                       (tree-buffer-get-node-at-point)))
         (level (phw-read-number
    ...
    (when phw-expand-methods-switch-off-auto-expand
      (phw-toggle-auto-expand-tag-tree -1))
    (phw-expand-methods-node-internal (with-current-buffer phw-methods-buffer-name
                                         (tree-buffer-get-root))
                                       level force-all t t)))

(defun phw-expand-methods-node-internal (node level
                                               &optional force-all
                                               resync-tag update-tree-buffer)
  "Set the expand level of NODE and its subnodes in the PHW-methods-buffer.
...
of FORCE-ALL see `phw-expand-methods-nodes'.
...
`phw-process-non-semantic-files') FORCE-ALL is always true!"
  ...
  ;; t! We "misuse" (phw-methods-get-data-store
  ...
  ;; not nil!
  (setq force-all
        (if (not (phw-methods-get-data-store 'semantic-symbol->name-assoc-list))
            t
          force-all))
  (phw-exec-in-window phw-methods-buffer-name
    (let ( ;; normalizing the elements of `phw-methods-nodes-expand-spec'
          ;; and `phw-methods-nodes-collapse-spec'.
          (norm-expand-types (phw-normalize-expand-spec
                              phw-methods-nodes-expand-spec))
          (norm-collapse-types (phw-normalize-expand-spec
                                phw-methods-nodes-collapse-spec))
         ...
         (and (not force-all)
              (function (lambda (node current-level)
                          (or (equal norm-expand-types 'all)
                              (member (phw-methods-node-get-semantic-type node)
                                      norm-expand-types)))))
         (and (not force-all)
              (function (lambda (node current-level)
  ...
  ;; edit-window.
  (if resync-tag (phw-tag-sync 'force)))



(defun phw-normalize-expand-spec (spec)
  (if (equal 'all spec)
      'all
    (mapcar (function (lambda (elem)
                        (intern
                         (downcase (phw-string-make-singular
                                    (symbol-name elem))))))
            spec)))


(defun phw-search-type-tag-by-semantic-analyzer (tag)
...
`phw-find-external-tag-functions'."
  (phw-search-tag-by-semantic-analyzer (phw--semantic-tag-name tag) 'type))

(defun phw-search-type-tag-by-semanticdb (tag)
...
This function is fully fitting the needs of the option
`phw-find-external-tag-functions'."
  (phw-search-tag-by-semanticdb (phw--semantic-tag-name tag) 'type))

(defun phw-next-tag-parent-node (node)
  ...
  (let ((parent-node (tree-node->parent node)))
    (cond ((null parent-node) nil)
          ((= (tree-node->type parent-node) phw-methods-nodetype-tag)
           parent-node)
          (t (phw-next-tag-parent-node parent-node)))))

;; (defun klaus-phw-test-ext-function (tag)
;;   (progn
;;     (find-file "~/klaus.cc")
;;     t))

(defphw-tree-buffer-callback phw-method-clicked phw-methods-buffer-name select
  ...
  (if shift-mode
      (phw-mouse-over-method-node node nil nil 'force))
  ;; First of all we must highlight the tag
  (tree-buffer-highlight-node-by-data/name (tree-node->data node))
  (if (= (tree-node->type node) phw-methods-nodetype-bucket)
      ;; Type phw-methods-nodetype-bucket = a title of a group
    ...
    (let ((node-tag (if (phw--semantic-tag-p (tree-node->data node))
                        (tree-node->data node)
                      (phw--semantic-tag (tree-node->data node) 'type)))
      ...
      ;; included file
      (unless destination
        (when (equal 'include (phw--semantic-tag-class node-tag))
          (with-current-buffer (phw-path-selected-source 'buffer)
            (let ((file (phw--semantic-dependency-tag-file node-tag)))
              (when (and file (phw-file-exists-p file))
                (setq destination (list (phw-source-make file)
                                        nil)))))))

      ;; now we process faux-tags and external tags like parent-types
      (unless destination
        (when (or (= node-type phw-methods-nodetype-externtag)
                  (and (phw--semantic-tag-p node-tag)
                       (phw--semantic-tag-faux-p node-tag)))
          (save-excursion
            ;; tag-to-go:
            ...
            (let ((tag-to-go (if (phw--semantic-tag-faux-p node-tag)
                                 (car (phw--semantic-tag-children-compatibility
                                       node-tag t))
                               ;; in case of an external tag
                               node-tag))
                  (parent-tag-of-tag-to-go (and (= node-type phw-methods-nodetype-externtag)
                                                (tree-node->data (phw-next-tag-parent-node node))))
                  found)
              (when tag-to-go
                (phw--semantic-go-to-tag tag-to-go parent-tag-of-tag-to-go)
                ...
                ;; defined in `phw-find-external-tag-functions' until one of
                ;; them returns not nil
                (setq found (catch 'foundit
                              (dolist (f (append (cdr (assoc major-mode phw-find-external-tag-functions))
                                                 (cdr (assoc 'default phw-find-external-tag-functions))))
                                (let ((f-result (funcall f node-tag)))
                                  (if f-result (throw 'foundit f-result))))
                              nil))
                (setq destination
                      (cond ((and (phw--semantic-tag-p found)
                                  (phw--semantic-tag-with-position-p found))
                             (list (phw-source-make (phw-buffer-file-name
                                                     (phw-semantic-tag-buffer found))
                                                    (phw-semantic-tag-buffer found))
                                   found))
      ...
      (unless destination
        (when (and (phw--semantic-tag-p node-tag)
                   (phw--semantic-tag-with-position-p node-tag)
...
;;                    (equal (phw-semantic-tag-buffer node-tag)
;;                           (phw-path-selected-source 'buffer))
                   )
          (setq destination (list (phw-source-make (phw-buffer-file-name
                                                    (phw-path-selected-source 'buffer))
                                                   (phw-path-selected-source 'buffer))
                                  node-tag))))

      (when (and destination (listp destination))
        (phw-display-tag (nth 0 destination) ;; the source
                         (nth 1 destination) ;; a tag or nil
                         (phw-get-edit-window
                          (phw-combine-phw-button/edit-win-nr phw-button edit-window-nr))
                         no-post-action
                         (if (and shift-mode
                                  (not (member 'phw-tag-visit-narrow-tag
                                               additional-post-action-list)))
                             (cons 'phw-tag-visit-narrow-tag
                                   additional-post-action-list)
                           additional-post-action-list))))))


(defun phw-tag-visit-smart-tag-start (tag)
...
Returns point."
  ...
  ;; search.
  (search-forward (phw--semantic-tag-name tag)
                  (max (phw-line-end-pos)
                       (phw-semantic-tag-end tag))
                  t)
...
(defun phw-start-of-tag-doc (tag)
...
return the start of the documentation. Otherwise return nil"
  ...
  ;; in elisp
  (let ((comment (ignore-errors (phw--semantic-documentation-for-tag tag
                                                                     'flex))))
    (if (and comment
             (not (stringp comment)))
        ;; probably we have a semantic flex-object
        (phw--semantic-lex-token-start comment))))


(defun phw-tag-visit-display-doc-start (tag)
...
If this function is set in `phw-tag-visit-post-actions' then it's strongly
recommended not to add `phw-tag-visit-recenter' or
`phw-tag-visit-recenter-top' after this this function!
...
`phw-tag-visit-post-actions'!

Returns current point."
  (let ((tag-doc-start  (phw-start-of-tag-doc tag)))
...
(defalias 'phw-tag-visit-goto-doc-start 'phw-tag-visit-display-doc-start)

(defvar phw-unhighlight-hook-called nil
...
(defun phw-unhighlight-tag-header ()
  (let ((key (phw-event-to-key last-input-event)))
    (when (not (or (and (equal key 'mouse-release)
                        (not phw-unhighlight-hook-called))
                   (equal key 'mouse-movement)))
      (phw-overlay-delete phw-method-overlay)
      (remove-hook 'pre-command-hook 'phw-unhighlight-tag-header)))
  (setq phw-unhighlight-hook-called t))


(defun phw-tag-visit-highlight-tag-header (tag)
  "Highlights line where `phw-tag-visit-smart-tag-start' puts point for
TAG. Returns current point"
  (save-excursion
    (phw-tag-visit-smart-tag-start tag)
    (phw-overlay-move phw-method-overlay
                      (phw-line-beginning-pos)
                      (phw-line-end-pos)
                      (current-buffer)))
  (setq phw-unhighlight-hook-called nil)
  (add-hook 'pre-command-hook 'phw-unhighlight-tag-header)
  (point))


(defun phw-display-tag (source tag &optional window
                               no-tag-visit-post-actions
                               additional-post-action-list)
  "Display SOURCE and go to TAG.
SOURCE is a source in the sense of `phw-source-make'. TAG is either nil \(then
...
`phw-tag-visit-post-actions' are not performed. If
ADDITIONAL-POST-ACTION-LIST is a list of function-symbols then these functions
are performed after these ones of `phw-tag-visit-post-actions'. So if
...
nil too then no post-actions are performed."
  ...
  (if (null tag)
    ...
    (when (phw--semantic-tag-with-position-p tag)
      (phw-semantic-assert-valid-tag tag)
      ;; let us set the mark so the user can easily jump back.
      (if phw-tag-jump-sets-mark
          (push-mark nil t))
      (widen)
      (goto-char (phw-semantic-tag-start tag))
      ;; the following 2 lines prevent the autom. tag-sync-mechanism from
      ;; starting.
      (setq phw-tag-sync-do-nothing t)
      ;; Klaus Berndl <klaus.berndl@sdm.de>: See the comment in
      ;; `phw-tag-sync' for an explanation why this is now commented out.
      ;; (setq phw-selected-tag tag)
      ;; process post action
      (unless no-tag-visit-post-actions
        ;; first the default post actions
        (dolist (f (cdr (assoc 'default phw-tag-visit-post-actions)))
          (phw-call-tag-visit-function tag f))
        ;; now the mode specific actions
        (dolist (f (cdr (assoc major-mode phw-tag-visit-post-actions)))
          (phw-call-tag-visit-function tag f)))
      ...
      ;; implementation of phw-nav-tag-history-item. Not longer storing
      ...
      (let* ((tag-buf (current-buffer))
             (tag-name (phw--semantic-tag-name tag))
             (tag-start (move-marker (make-marker)
                                     (phw-semantic-tag-start tag) tag-buf))
             (tag-end (move-marker (make-marker)
                                   (phw-semantic-tag-end tag) tag-buf)))
        (phw-nav-add-item (phw-nav-tag-history-item-new
                           tag-name
                           tag-buf
                           tag-start
                           tag-end
                           (member 'phw-tag-visit-narrow-tag
                                   additional-post-action-list)))))))


(defun phw-mouse-over-method-node (node &optional window no-message click-force)
...
`phw-methods-show-node-info'. NODE is the node for which help text should be
...
should be printed here."
  (let ((str (when (or click-force
                       (phw-show-minibuffer-info node window
                                                 (car phw-methods-show-node-info)))
               (concat
                (tree-node->name node)
                (if (and (= phw-methods-nodetype-tag
                            (tree-node->type node))
                         (tree-node->data node)
                         (equal (cdr phw-methods-show-node-info) 'name+type))
                    (concat ", "
                            (symbol-name (phw--semantic-tag-class
                                          (tree-node->data node))))
                  "")))))
    (prog1 str
      (unless no-message
...
(defun phw-call-tag-visit-function (tag fcn)
...
the location before calling FCN."
  (when (fboundp fcn)
    (let* ((start (phw--semantic-tag-start tag))
           (end (phw--semantic-tag-end tag))
           (result-point (save-excursion
                           (funcall fcn tag))))
      (if (and (>= result-point start)
               (<= result-point end))
          (goto-char result-point)
        (phw-warning "The tag-visit-function `%s' moves point outside of tag - ignored!"
                     fcn)))))

(defun phw-tag-visit-narrow-tag (tag)
...
Returns current point."
  (when (not (phw-speedbar-sb-tag-p tag))
    (narrow-to-region (or (phw-start-of-tag-doc tag)
                          (phw-semantic-tag-start tag))
                      (phw-semantic-tag-end tag)))
  (point))


(defun phw-tag-visit-recenter (tag)
  "Recenter the source-buffer, so current line is in the middle of the window.
If this function is added to `phw-tag-visit-post-actions' then it's
recommended to add this function add the end of the action list for 'default
or a `major-mode' and not to add the function `phw-tag-visit-recenter-top'
too!

Returns current point."
  (set-window-start
   (selected-window)
   (phw-line-beginning-pos (- (/ (phw-window-full-height) 2))))
  (point))

(defun phw-tag-visit-recenter-top (tag)
  "Recenter the source-buffer, so current line is in the middle of the window.
If this function is added to `phw-tag-visit-post-actions' then it's
recommended to add this function add the end of the action list for 'default
or a `major-mode' and not to add the function `phw-tag-visit-recenter' too!
...
(tree-buffer-defpopup-command phw-methods-menu-jump-and-narrow
  "Jump to the token related to the node under point an narrow to this token."
  (phw-method-clicked node 1 nil nil nil t '(phw-tag-visit-narrow-tag
                                             phw-tag-visit-highlight-tag-header)))


(tree-buffer-defpopup-command phw-methods-menu-widen
...
(defun phw-methods-menu-activate-hs ()
  "Activates `hs-minor-mode' in the buffer of `phw-path-selected-source'. If
this fails then nil is returned otherwise t."
  (with-current-buffer (get-file-buffer (phw-path-selected-source 'file))
...
(tree-buffer-defpopup-command phw-methods-menu-show-block
  "Runs `hs-show-block' for the current node under point."
  (if (not (phw-methods-menu-activate-hs))
      (phw-error "hs-minor-mode can not be activated!")
    ;; point must be at beginning of tag-name
    (phw-method-clicked node 1 nil nil nil t '(phw-tag-visit-smart-tag-start))
    (save-excursion
    ...
    ;; position `phw-method-clicked' has set the point.
    (phw-tag-visit-highlight-tag-header (tree-node->data node))))


(tree-buffer-defpopup-command phw-methods-menu-hide-block
...
(tree-buffer-defpopup-command phw-methods-menu-collapse-all
  "Collapse all expandable and expanded nodes"
  (phw-expand-methods-node-internal (tree-buffer-get-root) -1 nil t t))

(tree-buffer-defpopup-command phw-methods-menu-collapse-current
  "Collapse the current node"
  (let ((phw-methods-nodes-collapse-spec 'all))
    (phw-expand-methods-node-internal node -1 nil t t)))

(tree-buffer-defpopup-command phw-methods-menu-expand-all-0
  "Expand all nodes exactly to level 0."
  (phw-expand-methods-node-internal (tree-buffer-get-root) 0 nil t t))

(tree-buffer-defpopup-command phw-methods-menu-expand-current-0
  "Expand current node exactly to level 0."
  (let ((phw-methods-nodes-expand-spec 'all))
    (phw-expand-methods-node-internal node 0 nil t t)))

(tree-buffer-defpopup-command phw-methods-menu-expand-all-1
  "Expand all nodes to exactly level 1."
  (phw-expand-methods-node-internal (tree-buffer-get-root) 1 nil t t))

(tree-buffer-defpopup-command phw-methods-menu-expand-current-1
  "Expand current node exactly to level 0."
  (let ((phw-methods-nodes-expand-spec 'all))
    (phw-expand-methods-node-internal node 1 nil t t)))

(tree-buffer-defpopup-command phw-methods-menu-expand-all-2
  "Expand all nodes to exactly level 2."
  (phw-expand-methods-node-internal (tree-buffer-get-root) 2 nil t t))

(tree-buffer-defpopup-command phw-methods-menu-expand-current-2
  "Expand current node exactly to level 0."
  (let ((phw-methods-nodes-expand-spec 'all))
    (phw-expand-methods-node-internal node 2 nil t t)))

(tree-buffer-defpopup-command phw-methods-menu-expand-all-full
  "Expand all expandable nodes recursively, i.e. completely."
  (phw-expand-methods-node-internal (tree-buffer-get-root) 100 nil t t))

(tree-buffer-defpopup-command phw-methods-menu-expand-current-full
  "Expand the current node recursively, i.e. completely."
  (let ((phw-methods-nodes-expand-spec 'all))
    (phw-expand-methods-node-internal node 100 nil t t)))


(defvar phw-common-methods-menu nil
  "Built-in menu for the methods-buffer.")


(setq phw-common-methods-menu
      '( ;;("---")
        ("Expand/Collapse"
         (phw-methods-menu-collapse-current "Collapse this node completely")
         (phw-methods-menu-expand-current-0 "Expand this node to level 0")
         (phw-methods-menu-expand-current-1 "Expand this node to level 1")
         (phw-methods-menu-expand-current-2 "Expand this node to level 2")
         (phw-methods-menu-expand-current-full "Expand this node completely")
         ("---")
         (phw-methods-menu-collapse-all "Collapse all completely")
         (phw-methods-menu-expand-all-0 "Expand all to level 0")
         (phw-methods-menu-expand-all-1 "Expand all to level 1")
         (phw-methods-menu-expand-all-2 "Expand all to level 2")
         (phw-methods-menu-expand-all-full "Expand all completely")
         )
        ("---")
        (phw-maximize-phw-window-menu-wrapper "Maximize window")))


(defvar phw-methods-tag-menu nil)
(setq phw-methods-tag-menu
      (append '(("Hide/Show"
                 (phw-methods-menu-hide-block "Jump to tag and hide block")
                 (phw-methods-menu-show-block "Jump to tag and show block"))
                ("Narrow/Widen"
                 (phw-methods-menu-jump-and-narrow "Jump to tag and narrow")
                 (phw-methods-menu-widen "Undo narrowing of edit-window")))
              phw-common-methods-menu))


(defvar phw-methods-menu-title-creator
  (function (lambda (node)
              (let ((data (tree-node->data node)))
                (if (and data (/= phw-methods-nodetype-bucket
                                  (tree-node->type node)))
                    (typecase data
                      (phw--semantic-tag (phw--semantic-tag-name data))
                      (string data)
...
(dotimes (i 8)
  (eval `(tree-buffer-defpopup-command
             ,(intern (format "phw-jump-to-tag-in-editwin%d" (1+ i)))
           ,(format "Jump to current tag in the %d. edit-window." (1+ i))
           (phw-method-clicked node 3 ,(1+ i) nil nil))))

(defun phw-methods-menu-editwin-entries ()
  ...
  (let ((edit-win-list (phw-canonical-edit-windows-list))
    ...
    (when (> (length edit-win-list) 1)
      (dotimes (i (min 8 (length edit-win-list)))
        (setq result
              (append result
                      (list (list (intern (format "phw-jump-to-tag-in-editwin%d" (1+ i)))
...
(defun phw-methods-menu-tagfilter-entries ()
  "Generate popup-menu-entries for the tag-filtering"
  (let* ((curr-semantic-symbol->name-assoc-list
          ;; we must not use here (phw-methods-get-data-store
          ;; 'semantic-symbol->name-assoc-list) because we do not want the
          ;; function-prototypes...
          (with-current-buffer (phw-methods-get-data-store 'source-buffer)
            (phw--semantic-symbol->name-assoc-list)))
    ...
    (when curr-semantic-symbol->name-assoc-list
      (dolist (tag-class curr-semantic-symbol->name-assoc-list)
        (let ((fcn-sym (intern (format "phw-methods-filter-by-%s-tagclass"
                                       (car tag-class)))))
          ...
          (when (not (fboundp fcn-sym))
            (eval `(tree-buffer-defpopup-command ,fcn-sym
                     ...
                     (phw-methods-filter-by-tag-class nil
                                                      (phw-methods-get-data-store 'source-buffer)
                                                      ,(symbol-name (car tag-class))))))))
      (dolist (tag-class curr-semantic-symbol->name-assoc-list)
        (let ((fcn-sym (intern (format "phw-methods-filter-by-%s-tagclass-inverse"
                                       (car tag-class)))))
          ...
          (when (not (fboundp fcn-sym))
            (eval `(tree-buffer-defpopup-command ,fcn-sym
                     ,(format "Filter all tags with tag-class unequal '%s."
                              (car tag-class))
                     (phw-methods-filter-by-tag-class t
                                                      (phw-methods-get-data-store 'source-buffer)
                                                      ,(symbol-name (car tag-class))))))))
      (dolist (prot prot-list)
        (let ((fcn-sym (intern (format "phw-methods-filter-by-%s-prot" prot))))
          (setq prot-menu-elems (cons (cons fcn-sym prot)
                                      prot-menu-elems))
          (when (not (fboundp fcn-sym))
            (eval `(tree-buffer-defpopup-command ,fcn-sym
                     ,(format "Filter all tags with %s protection." prot)
                     (phw-methods-filter-by-prot nil
                                                 (phw-methods-get-data-store 'source-buffer)
                                                 ,prot))))))
      (dolist (prot prot-list)
        (let ((fcn-sym (intern (format "phw-methods-filter-by-%s-prot-inverse" prot))))
    ...
    (append nil ;; (list (list "---")) ;; we want a separator
            (list (append (list "Filter tags")
                          (list '(phw-methods-filter-by-nothing-popup
                                  "No tag filter")
                                '(phw-methods-filter-delete-last-popup
                                  "Remove last added")
                                '("---")
                                '(phw-methods-filter-by-regexp-popup
                                  "By regexp"))
                          ...
                          (when curr-semantic-symbol->name-assoc-list
                            (list '(phw-methods-filter-by-current-type-popup
                                    "By current type")))
                          (list '(phw-methods-filter-by-function-popup
                                  "By a filter-function")
                                '("---")
                                '(phw-methods-filter-by-regexp-popup-inverse
                                  "By inverse regexp"))
                          ...
                          (list '(phw-methods-filter-by-function-popup-inverse
                                  "By a inverse filter-function")))))))
    
(defun phw-methods-menu-creator (tree-buffer-name node)
  ...
  (let ((dyn-user-extension
         (and (functionp phw-methods-menu-user-extension-function)
              (funcall phw-methods-menu-user-extension-function
                       tree-buffer-name node)))
        (dyn-builtin-extension-edit-win (phw-methods-menu-editwin-entries))
        (dyn-builtin-extension-tagfilter (phw-methods-menu-tagfilter-entries)))
    (list (cons phw-methods-nodetype-tag
                (funcall (or phw-methods-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-methods-menu-user-extension
                                 dyn-builtin-extension-tagfilter
                                 phw-methods-tag-menu
                                 dyn-builtin-extension-edit-win)))
          (cons phw-methods-nodetype-bucket
                (funcall (or phw-methods-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-methods-menu-user-extension
                                 dyn-builtin-extension-tagfilter
                                 phw-common-methods-menu)))
          (cons phw-methods-nodetype-externtag
                (funcall (or phw-methods-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-methods-menu-user-extension
...
(defconst phw-methods-incr-searchpattern-node-prefix
  '("\\([-+#(]\\|[^-+#(][^ \n]+ \\)?" . 1)
...
;; phw--semantic-adopt-external-members. the following function is a save
;; "equal"-condition for PHW because currently the method buffer always
;; displays only tags from exactly the buffer of the current edit-window. If
;; `phw--semantic-equivalent-tag-p' fails we return the result of an
;; eq-comparison.
(defun phw-compare-methods-buffer-node-data (l r)
  (cond ((or (stringp l) (stringp r))
         (equal l r))
        ((or (equal 'phw-bucket-node (car l))
             (equal 'phw-bucket-node (car r)))
         (equal l r))
        (t ;; tags
         (condition-case nil
             (phw--semantic-equivalent-tag-p l r)
           (error (eq l r))))))

(defun phw-methods-node-mouse-highlighted-p (node)
...
moved over it."
  (or (not (equal (tree-node->type node) phw-methods-nodetype-tag))
      (phw--semantic-tag-with-position-p (tree-node->data node))
      (phw--semantic-tag-faux-p (tree-node->data node))))

(defphw-tree-buffer-creator phw-create-methods-tree-buffer phw-methods-buffer-name
  ...
  (tree-buffer-create
   ...
   :node-mouse-over-fn 'phw-mouse-over-method-node
   :mouse-highlight-fn 'phw-methods-node-mouse-highlighted-p
   :node-data-equal-fn 'phw-compare-methods-buffer-node-data
   :maybe-empty-node-types (list phw-methods-nodetype-bucket)
   :leaf-node-types nil
   :menu-creator 'phw-methods-menu-creator
   :menu-titles (list (cons phw-methods-nodetype-tag phw-methods-menu-title-creator)
                      (cons phw-methods-nodetype-bucket phw-methods-menu-title-creator)
                      (cons phw-methods-nodetype-externtag phw-methods-menu-title-creator))
   ...
   :trunc-lines (phw-member-of-symbol/value-list phw-methods-buffer-name
                                                 phw-tree-truncate-lines)
   :read-only t
   :tree-indent phw-tree-indent
   :incr-search-p phw-tree-incremental-search
   :incr-search-additional-pattern phw-methods-incr-searchpattern-node-prefix
   ...
   :additional-images-dir (phw-member-of-symbol/value-list phw-methods-buffer-name
   ...
   :highlight-node-face phw-method-face
   :general-face phw-methods-general-face
   :after-create-hook (append
                       (list (function (lambda ()
                                         (phw-common-after-tree-buffer-create-actions)
                                         (setq phw-methods-root-node (tree-buffer-get-root)))))
                       phw-common-tree-buffer-after-create-hook
                       phw-methods-buffer-after-create-hook)
   :after-update-hook nil))


(defun phw-dump-semantic-toplevel ()
  "Dump the current semantic-tags in special buffer and display them."
  (interactive)
  (let ((tags (phw-fetch-semantic-tags t))
        (source-buf (current-buffer)))
    (save-selected-window
      (set-buffer (get-buffer-create "*phw-tag-dump*"))
      (erase-buffer)
      (phw-dump-semantic-tags-internal tags nil source-buf 1)
      (switch-to-buffer-other-window (get-buffer-create "*phw-tag-dump*"))
      (goto-char (point-min)))))

(defun phw-dump-semantic-tags-internal (table parent source-buffer indent)
  (dolist (tag table)
    ;; we ca not use format here because XEmacs-format removes all
    ;; text-properties! 
    (insert (concat (make-string indent ? )
                    (with-current-buffer source-buffer
                      (phw--semantic-format-tag-uml-prototype tag parent t))
                    ", tag-class: "
                    (format "%s" (phw--semantic-tag-class tag))
                    "\n"))
    (unless (equal (phw--semantic-tag-class tag) 'function)
      (phw-dump-semantic-tags-internal (phw-children-tags tag)
                                       (if (equal (phw--semantic-tag-class tag)
                                                  'type)
                                           tag)
                                       source-buffer
                                       (+ 2 indent)))))

(defphw-advice-set phw-methods-browser-advices
  "Adviced functions needed by the methods-browser of PHW.")

(defphw-advice make-indirect-buffer after phw-methods-browser-advices
...
Returns always the newly created indirect buffer."
  (when (and (not phw-running-xemacs) ;; clone not available with XEmacs
             ;; clone-flag is not nil
             (ad-get-arg 2))
    (with-current-buffer ad-return-value
      (message "PHW: semantic cache for indirect buffer %s cleared!" (current-buffer))
      (phw--semantic-clear-toplevel-cache)))
  ad-return-value)

(defphw-advice custom-save-all around phw-methods-browser-advices
...
by semantic and also killed afterwards."
  (if phw-minor-mode
      (let ( ;; XEmacs 21.4 does not set this so we do it here, to ensure that
        ...
        ;; desired.
        (ignore-errors
          (kill-buffer (find-file-noselect (phw-custom-file)))))
    ad-do-it))

(phw-disable-advices 'phw-methods-browser-advices t)

(silentcomp-provide 'phw-method-browser)

;;; phw-method-browser.el end here
