;;; phw-file-browser.el --- the file-browser of Emacs
...
;; This file contains the code of the file-browser of PHW
...
(silentcomp-defun phw-speedbar-update-contents)
...
(defgroup phw-directories nil
  "Settings for the directories-buffer in the Emacs code browser."
  :group 'phw
  :prefix "phw-")

(defgroup phw-sources nil
  "Settings for the sources-buffers in the Emacs code browser."
  :group 'phw
  :prefix "phw-")

(defgroup phw-history nil
  "Settings for the history-buffer in the Emacs code browser."
  :group 'phw
  :prefix "phw-")

(defgroup phw-version-control nil
  "Settings for the version-control support in the PHW."
  :group 'phw
  :prefix "phw-")

(defcustom phw-source-path nil
...
- a simple string which has to be the full path of a directory \(this string
  is displayed in the directory-browser of PHW) or
- a 2-element list whereas the first element is the full path of a directory
  ...
  :set (function (lambda (symbol value)
                   ...
                   (if (and (boundp 'phw-minor-mode)
                            phw-minor-mode
			    (functionp 'phw-update-directories-buffer))
...
(defcustom phw-add-path-for-not-matching-files '(t . nil)
  "*Add path of a file to `phw-source-path' if not already contained.
This is done during the auto. windows synchronization which happens if a file
is opened not via the file/directory-browser of PHW. In such a situation PHW
adds the path of the new file auto. to `phw-source-path' at least temporally
...
1. Should only the root-part \(which means for Unix-like systems always '/'
   ...
   source-path to `phw-source-path' or the whole directory-part? For
   remote-files \(e.g. tramp, ange-ftp- or efs-files) the root-part is the
   complete host-part + the root-dir at that host \(example:
   /berndl@phw.sourceforge.net:/ would be the root-part of
   /berndl@phw.sourceforge.net:/tmp/test.txt).
...
not via the PHW-browser which are not located in any of the paths of
`phw-source-path' because then only one path for each drive \(windows) or the
root-path \(unix) is added to the directory buffer of PHW."
  :group 'phw-directories
  :type '(cons (boolean :tag "Add only root path")
               (boolean :tag "Ask for saving for future sessions")))


(defvar phw-source-path-functions nil
  "List of functions to call for finding sources.
Each time the function `phw-update-directories-buffer' is called, the
...
(defcustom phw-display-default-dir-after-start t
  "*Automatically display current default-directory after activating PHW.
If a file-buffer is displayed in the edit-window then PHW synchronizes its
tree-buffers to this file-buffer - at least if the option `phw-basic-buffer-sync' it
not nil. So for this situation `phw-display-default-dir-after-start' takes no
effect but this option is for the case if no file-buffer is displayed in the
edit-window after startup:

If true then PHW selects autom. the current default-directory after activation
even if no file-buffer is displayed in the edit-window. This is useful if PHW
...
selected in the PHW-directories buffer and the PHW-sources buffer displays the
contents of this directory."
  :group 'phw-directories
  :type 'boolean)


(defcustom phw-show-sources-in-directories-buffer '("left7" "left13"
                                                    "left14" "left15")
...
layouts sources should be displayed in the directories window."
  ...
  :set (function (lambda (symbol value)
		   ...
		   (if (and phw-minor-mode
			    (functionp 'phw-set-selected-directory)
                            phw-path-selected-directory)
		       (phw-set-selected-directory phw-path-selected-directory t))))
...
(defcustom phw-directories-show-node-info '(if-too-long . path)
...
- if-too-long: Node info is only displayed by moving with the mouse over a
  ...
  In the PHW directories buffer this means also if a node is shortend or if
  the node has an alias \(see `phw-source-path').
...
(defcustom phw-directories-update-speedbar 'auto
...
directory in the PHW-directories-buffer so the speedbar displays the contents
of that directory.

Of course this option makes only sense if the integrated speedbar is displayed
in addition to the PHW-directories-buffer.
...
If `phw-show-sources-in-directories-buffer' is not nil or you have a layout
where an PHW-sources-buffer is visible then you probably want to use the
PHW-directories-buffer \(and/or the PHW-sources-buffer) for directory- and
file-browsing. If you have in addition an integrated speedbar running then you
probably want to use speedbar instead of the PHW-methods-buffer for
...
If `phw-show-sources-in-directories-buffer' is nil and there is also no
PHW-sources-buffer visible in the current layout then you probably want to use
an integrated speedbar for browsing directory-contents \(i.e. the files) and
file-contents \(instead of the PHW-methods-buffer for example). In this case
...
(defun phw-show-sources-in-directories-buffer-p ()
  "Return not nil if in current layout sources are shown in the
directories-buffer."
  (case phw-show-sources-in-directories-buffer
    (never nil)
    (always t)
    (otherwise
     (and (listp phw-show-sources-in-directories-buffer)
          (member phw-layout-name
                  phw-show-sources-in-directories-buffer)))))

(defcustom phw-cache-directory-contents '(("^/\\([^:/]*@\\)?\\([^@:/]*\\):.*" . 0)
                                          (".*" . 50))
  "*Cache contents of certain directories.
This can be useful if `phw-source-path' contains directories with many files
...
sources- and/or directories- buffer of PHW \(if displayed in current layout!)
...
`phw-cache-directory-contents-not'!

The cache entry for a certain directory will be refreshed and actualized only
by using the POWER-click \(see `phw-primary-secondary-mouse-buttons') in the
directories-buffer of PHW.

Default-value: PHW caches the contents of all remote directories regardless of
...
\"~\" because PHW tries always to match full path-names!"
  :group 'phw-directories
  :group 'phw-most-important
...
(defcustom phw-cache-directory-contents-not nil
...
directory-contents will never being cached. See `phw-cache-directory-contents'
...
- setting `phw-cache-directory-contents' to \(\".*\" . 500): Caches all
  directories with more then 500 entries
- setting `phw-cache-directory-contents-not' to a value which matches these
...
NOT use \"~\" because PHW tries always to match full path-names!"
  :group 'phw-directories
  :type `(repeat (regexp :tag "Directory-regexp")))

(defcustom phw-ping-program "ping"
...
also `phw-ping-options'."
  :group 'phw-directories
  :type  'string)

(defcustom phw-ping-options
...
These options have to ensure that the program set in `phw-ping-program' only
...
the specified ping-program \(see `phw-ping-program'). Therefore at least on of
these options must be the string HOST \(uppercase) which will be replaced
internally by PHW with that host-name the accessibility of this host
...
HOST, which means the ping-program of `phw-ping-program' will be
called with one argument which will be the host-name which should be
tested.

See also `phw-ping-program'."
  :group 'phw-directories
  :type  '(repeat string))

(defcustom phw-host-accessible-check-valid-time nil
...
should be valid. During this time-intervall PHW pings such a remote host only
once, all other checks use the cached value of that real check. But it the
cached value is older than the value of this option PHW will ping again.

Per default PHW discards after 1 minute the cached ping-state of each remote
host. But if you are sure that a certain remote host is always accessible
\(i.e. means in consequence that you are always online when working with PHW
...
(defcustom phw-prescan-directories-for-emptyness 'unless-remote
  "*Prescan directories for emptyness.
PHW does this so directories are displayed as empty in the directories-buffer
even without user-interaction \(i.e. in previous PHW-versions the emptyness of
a directory has been first checked when the user has clicked onto a
directory). PHW optimizes this check as best as possible but if a directory
...
Therefore PHW performs this check stealthy \(see `phw-stealthy-tasks-delay')
...
The option `phw-prescan-directories-exclude-regexps' offers are more fine
...
(defcustom phw-prescan-directories-exclude-regexps nil
...
`phw-prescan-directories-for-emptyness' is not nil."
  :group 'phw-directories
  :type '(repeat (regexp :tag "Directory-regexp")))

(defsubst phw-directory-should-prescanned-p (dir)
  "Return not nil if DIR should be prescanned for emptyness.
The check is performed according to the settings in the options
`phw-prescan-directories-for-emptyness' and
`phw-prescan-directories-exclude-regexps'."
  (and (or (equal t phw-prescan-directories-for-emptyness)
           (and (equal 'unless-remote phw-prescan-directories-for-emptyness)
                (not (phw-remote-path dir))))
       (not (phw-match-regexp-list dir phw-prescan-directories-exclude-regexps))))

(defcustom phw-grep-function (cond ((fboundp 'lgrep)
                                    'lgrep)
                                   ((fboundp 'igrep)
                                    'igrep)
                                   (t 'grep))
...
(defcustom phw-grep-recursive-function (cond ((fboundp 'rgrep)
                                              'rgrep)
                                             ((fboundp 'igrep-find)
                                              'igrep-find)
                                             (t 'grep-find))
  "*Function used for performing a recursive grep.
For more Details see option `phw-grep-function' and replace \"grep\" with
\"recursive grep\".
...
(defcustom phw-after-directory-change-hook nil
...
PHW but it means this hook runs always when the current directory changes
...
different but only when auto-synchronizing of the PHW-windows is on (see
`phw-basic-buffer-sync'). It runs not when switching between buffers and the
...
(defcustom phw-sources-perform-read-only-check 'unless-remote
...
the option `phw-source-read-only-face'.

Because this check can be take some time if files are used via a mounted
net-drive PHW performs this check stealthy \(see `phw-stealthy-tasks-delay')
so normally there should no performance-decrease or additional waiting-time
for the user. But to get sure this option offers three choices: t,
'unless-remote and nil. See `phw-prescan-directories-for-emptyness' for an
explanation for these three choices.

The option `phw-read-only-check-exclude-regexps' offers are more fine
...
(defcustom phw-read-only-check-exclude-regexps nil
...
`phw-sources-perform-read-only-check' is not nil."
  :group 'phw-sources
  :group 'phw-directories
  :type '(repeat (regexp :tag "Directory-regexp")))

(defsubst phw-sources-read-only-check-p (dir)
  "Return not nil if the sources of DIR should be checked for read-only-state.
The check is performed according to the settings in the options
`phw-sources-perform-read-only-check' and
`phw-read-only-check-exclude-regexps'."
  (and (or (equal t phw-sources-perform-read-only-check)
           (and (equal 'unless-remote phw-sources-perform-read-only-check)
                (not (phw-remote-path dir))))
       (not (phw-match-regexp-list dir phw-read-only-check-exclude-regexps))))

(defcustom phw-directories-buffer-name " *PHW Directories*"
  "*Name of the PHW directory buffer.
Because it is not a normal buffer for editing you should enclose the name with
stars, e.g. \"*PHW Directories*\".

If it is necessary for you you can get emacs-lisp access to the buffer-object of
the PHW-directory-buffer by this name, e.g. by a call of `set-buffer'.
...
(defcustom phw-excluded-directories-regexps '("^\\(CVS\\|\\.[^xX]*\\)$")
...
(defsubst phw-check-dir-exclude (dir)
  (phw-match-regexp-list dir phw-excluded-directories-regexps))

(defcustom phw-auto-expand-directory-tree 'best
...
(defcustom phw-sources-buffer-name " *PHW Sources*"
  "*Name of the PHW sources buffer.
Because it is not a normal buffer for editing you should enclose the name with
stars, e.g. \"*PHW Sources*\".

If it is necessary for you you can get emacs-lisp access to the buffer-object of
the PHW-sources-buffer by this name, e.g. by a call of `set-buffer'.
...
(defcustom phw-sources-show-node-info '(if-too-long . name)
...
(defcustom phw-sources-exclude-cvsignore nil
...
listed in a .cvsignore-file from being displayed in the phw-sources-buffer
...
(defcustom phw-source-file-regexps
...
file is displayed in the sources-buffer of PHW iff: The file does not match
...
sources-buffer if the directory matches `phw-sources-exclude-cvsignore'
...
(defcustom phw-show-source-file-extension t
  "*Show the file extension of source files."
  :group 'phw-sources
  :type 'boolean)

(defcustom phw-sources-sort-method 'name
...
See also `phw-sources-sort-ignore-case'."
  :group 'phw-sources
...
(defcustom phw-sources-sort-ignore-case t
  "*Ignore case for sorting the source-files of the Sources-buffer.
See also `phw-sources-sort-method'."
  :group 'phw-sources
  :type 'boolean)

(defcustom phw-history-buffer-name " *PHW History*"
  "*Name of the PHW history buffer.
Because it is not a normal buffer for editing you should enclose the name with
stars, e.g. \"*PHW History*\".

If it is necessary for you you can get emacs-lisp access to the buffer-object of
the PHW-history-buffer by this name, e.g. by a call of `set-buffer'.
...
(defcustom phw-history-exclude-file-regexps '("TAGS$" "semantic\\.cache$")
...
(defsubst phw-check-filename-for-history-exclude (filename)
  (phw-match-regexp-list filename phw-history-exclude-file-regexps))

(defcustom phw-history-show-node-info '(always . name-path)
...
(defcustom phw-history-make-buckets 'directory-with-source-path
...
- 'directory-with-source-path: Same as 'directory but the best
  ...
  `phw-source-path') substituts the matching part of the
  directory with its alias \(if there is any set). For this a
  special face is used \(see
  `phw-history-bucket-node-dir-soure-path-face').
  ...
  :set (function (lambda (symbol value)
		   (set symbol value)
                   (when (and (boundp 'phw-minor-mode)
                              phw-minor-mode)
                     (phw-exec-in-window phw-history-buffer-name
...
(defcustom phw-history-stick-indirect-buffers-to-basebuffer t
...
setting of `phw-history-sort-method'.
...
(defcustom phw-history-sort-method 'name
...
See also `phw-history-sort-ignore-case'.

If the history is bucketized \(see `phw-history-make-buckets') then this
...
(defcustom phw-history-sort-ignore-case t
  "*Ignore case for sorting the history-entries.
See also `phw-history-sort-method'."
  :group 'phw-history
  :initialize 'custom-initialize-default
  :set (function (lambda (symbol value)
		   (set symbol value)
...
(defcustom phw-kill-buffer-clears-history nil
...
(defcustom phw-directories-menu-user-extension
  '(("Version Control"
     (phw-dir-popup-cvs-status "CVS Status" )
     (phw-dir-popup-cvs-examine "CVS Examine")
     (phw-dir-popup-cvs-update "CVS Update")))
...
c) Submenu: A list where the first element is the title of the submenu
   ...
   value of the `phw-max-submenu-depth' *BEFORE* first loading PHW!
...
\(tree-buffer-defpopup-command phw-my-special-dir-popup-function
...
built-in menu-entries of `phw-directories-menu' but the whole menu can be
re-arranged with `phw-directories-menu-sorter'.

These menu-extensions are static. A dynamic menu-extension can be achieved via
`phw-directories-menu-user-extension-function'."
  :group 'phw-directories
  :type (phw-create-menu-user-ext-type 1 phw-max-submenu-depth))

(defcustom phw-directories-menu-user-extension-function 'ignore
...
`phw-directories-menu-user-extension'. This function is called when the user
...
extensions of `phw-directories-menu-user-extension' but the whole menu can be
re-arranged with `phw-directories-menu-sorter'."
  :group 'phw-directories
  :type 'function)

(defcustom phw-sources-menu-user-extension
  '(("Version control"
     (phw-file-popup-ediff-revision "Ediff against revision")
     ("---")
     (phw-file-popup-vc-next-action "Check In/Out")
     (phw-file-popup-vc-log "Revision history")
     (phw-file-popup-vc-annotate "Annotate")
     (phw-file-popup-vc-diff "Diff against last version")
     ("---")
     (phw-file-popup-vc-refresh-file "Recompute state for file")
     (phw-file-popup-vc-refresh-dir "Recompute state for whole dir")))
  "*Static user extensions for the popup-menu of the sources buffer.
For further explanations see `phw-directories-menu-user-extension'.

The node-argument of a menu-function contains as data the source
for which the popup-menu has been opened. Use always
`phw-source-get-*' to extract whatever you need from the
node-data. E.g. use `phw-source-get-filename' to get the full
filename of the source of the node.

Per default the static user-extensions are added at the beginning of the
built-in menu-entries of `phw-sources-menu' but the whole menu can be
re-arranged with `phw-sources-menu-sorter'."
  :group 'phw-sources
  :type (phw-create-menu-user-ext-type 1 phw-max-submenu-depth))

(defcustom phw-sources-menu-user-extension-function 'ignore
  "*Dynamic user extensions for the popup-menu of the sources buffer.
A function which has to return a list in the same format like the option
`phw-sources-menu-user-extension'. This function is called when the user
...
Per default the dynamic user-extensions are added in front of the static
extensions of `phw-sources-menu-user-extension' but the whole menu can be
re-arranged with `phw-sources-menu-sorter'."
  :group 'phw-sources
  :type 'function)

(defcustom phw-history-menu-user-extension
  '(("Version control"
     (phw-file-popup-ediff-revision "Ediff against revision")
     ("---")
     (phw-file-popup-vc-next-action "Check In/Out")
     ...
     ("---")
     (phw-file-popup-vc-refresh-file "Recompute state for file")
     (phw-file-popup-vc-refresh-all-files "Recompute state for whole history")))
...
`phw-kill-buffer-clears-history'). Use always `phw-source-get-*'
to extract whatever you need from the node-data. E.g. use
`phw-source-get-filename' to get the full filename of the source
of the node and use `phw-source-get-buffername' or `phw-source-get-buffer' to
get the buffername rsp. the buffer-object.

Per default the static user-extensions are added at the beginning of the
built-in menu-entries of `phw-history-menu' but the whole menu can be
re-arranged with `phw-history-menu-sorter'."
  :group 'phw-history
  :type (phw-create-menu-user-ext-type 1 phw-max-submenu-depth))

(defcustom phw-history-menu-user-extension-function 'ignore
  "*Dynamic user extensions for the popup-menu of the history buffer.
A function which has to return a list in the same format like the option
`phw-history-menu-user-extension'. This function is called when the user
...
Per default the dynamic user-extensions are added in front of the static
extensions of `phw-history-menu-user-extension' but the whole menu can be
re-arranged with `phw-history-menu-sorter'."
  :group 'phw-history
  :type 'function)

(defcustom phw-directories-menu-sorter nil
...
`phw-directories-menu-user-extension' and the built-in-menu
`phw-directories-menu'. If nil then no special sorting will be done and the
...
argument see `phw-directories-menu-user-extension'. The function must return a
...
(defcustom phw-sources-menu-sorter nil
  "*Function which re-sorts the menu-entries of the directories buffer.
...
`phw-sources-menu-user-extension' and the built-in-menu
`phw-sources-menu'. If nil then no special sorting will be done and the
user-extensions are placed in front of the built-in-entries.

For the guidelines for such a sorter-function see
`phw-directories-menu-sorter'."
  :group 'phw-sources
  :type '(choice :tag "Menu-sorter" :menu-tag "Menu-sorter"
                 (const :tag "No special sorting" :value nil)
                 (function :tag "Sort-function" :value identity)))
...
(defcustom phw-history-menu-sorter nil
  "*Function which re-sorts the menu-entries of the directories buffer.
If a function then this function is called to sort the menu-entries of the
combined menu-entries of the user-menu-extensions of
`phw-history-menu-user-extension' and the built-in-menu
`phw-history-menu'. If nil then no special sorting will be done and the
user-extensions are placed in front of the built-in-entries.
...
(defcustom phw-directories-buffer-after-create-hook nil
...
creating the directories-buffer of PHW and it's local key-map. So for example a
function could be added which performs calls of `local-set-key' to define new
key-bindings only for the directories-buffer of PHW.
...
(defcustom phw-sources-buffer-after-create-hook nil
  "*Local hook running after the creation of the sources-buffer.
Every function of this hook is called once without arguments direct after
creating the sources-buffer of PHW and it's local key-map. So for example a
function could be added which performs calls of `local-set-key' to define new
key-bindings only for the sources-buffer of PHW."
  :group 'phw-sources
  :type 'hook)


(defcustom phw-history-buffer-after-create-hook nil
  "*Local hook running after the creation of the history-buffer.
Every function of this hook is called once without arguments direct after
creating the history-buffer of PHW and it's local key-map. So for example a
function could be added which performs calls of `local-set-key' to define new
key-bindings only for the history-buffer of PHW."
  :group 'phw-history
  :type 'hook)

(defvar phw-vc-needed-vc-package-available-p
  (if (locate-library "vc-svn") t nil)
  "Not nil if that vc-package is installed needed by PHW to enable vc-support.
If this variable is nil all other vc related options of PHW have no effect!")

(defcustom phw-vc-enable-support (and phw-vc-needed-vc-package-available-p
                                      'unless-remote)
  "*Enable support for version-control \(VC) systems.
If on then in the directories-buffer \(if the value of the option
`phw-show-sources-in-directories-buffer' is on for current layout), the
...
Version-control-server PHW performs this check stealthy \(see
`phw-stealthy-tasks-delay') so normally there should no performance-decrease
...
`phw-prescan-directories-for-emptyness' for an explanation for these three
choices.

The option `phw-vc-directory-exclude-regexps' offers are more fine granularity
to exclude the sources of certain directories from the VC-state-check.

See `phw-vc-supported-backends' how to customize the VC-support itself."
  :group 'phw-version-control
  :group 'phw-sources
  :group 'phw-most-important
  :type '(radio (const :tag "Switch on" :value t)
                (const :tag "Switch off for remote directories" :value unless-remote)
...
(defcustom phw-vc-directory-exclude-regexps nil
...
`phw-vc-enable-support' is not nil."
  :group 'phw-version-control
  :group 'phw-sources
  :type '(repeat (regexp :tag "Directory-regexp")))

(defsubst phw-vc-directory-should-be-checked-p (dir)
  "Return not nil if the sources of DIR should be checked for VC-state.
The check is performed according to the settings in the options
`phw-vc-enable-support' and `phw-vc-directory-should-be-checked-p'."
  (and dir
       phw-vc-needed-vc-package-available-p
       (or (equal t phw-vc-enable-support)
           (and (equal 'unless-remote phw-vc-enable-support)
                (not (phw-remote-path dir))))
       (not (phw-match-regexp-list dir phw-vc-directory-exclude-regexps))))

(defcustom phw-vc-state-mapping '((up-to-date       . up-to-date)
                                  (edited           . edited)
                                  ...
                                  (ignored          . ignored)
                                  (unknown          . unknown))
  "*Mapping from VC-state-values of the backends to VC-state-values of PHW.
PHW understands the following state-values:
...
All state-values a check-vc-state-function of `phw-vc-supported-backends' can
return must have a mapping to one of the PHW-state-values listed above. If for
...
state-values of `phw-vc-state', `vc-state' and `vc-recompute-state' \(both GNU
Emacs) and `vc-cvs-status' \(Xemacs) to the PHW-VC-state-values."
  :group 'phw-version-control
  :group 'phw-sources
  :initialize 'custom-initialize-default  
  ...
  :type '(repeat (cons (choice :tag "Backend VC-state"
                               ...
                               (symbol :tag "Other..."))
                       (choice :tag "PHW VC-state"
                               :menu-tag "PHW VC-state"
                               (const :tag "up-to-date" :value up-to-date)
                               (const :tag "edited" :value edited)
                               (const :tag "needs-patch" :value needs-patch)
                               (const :tag "needs-merge" :value needs-merge)
...
;; Remark why we need for PHW the information if a *directory* is managed by
...
;; in the directories buffer (when phw-show-sources-in-directories-buffer is
...
(defcustom phw-vc-supported-backends
              '((phw-vc-dir-managed-by-CVS . phw-vc-state)
                (phw-vc-dir-managed-by-RCS . phw-vc-state)
                (phw-vc-dir-managed-by-SCCS . phw-vc-state)
                (phw-vc-dir-managed-by-SVN . phw-vc-state)
                (phw-vc-dir-managed-by-GIT . phw-vc-state)
                (phw-vc-dir-managed-by-BZR . phw-vc-state)
                (phw-vc-dir-managed-by-MTN . phw-vc-state)
                (phw-vc-dir-managed-by-HG . phw-vc-state))
...
to be mapped with `phw-vc-state-mapping' to the allowed PHW-VC-state values.

PHW runs for a certain DIRECTORY all identify-backend-functions in that order
...
check-vc-state-function to cache any state because PHW automatically caches
internally all necessary informations for directories and files for best
possible performance.

To prepend PHW from checking the VC-state for any file set
`phw-vc-enable-support' to nil.

Default value: Support for CVS, RCS, SCCS, Subversion, Git,
Bazaar, Monotone and Mercurial. To identify the VC-backend the
functions `phw-vc-dir-managed-by-CVS',
`phw-vc-dir-managed-by-RCS' rsp. `phw-vc-dir-managed-by-SCCS'
rsp. `phw-vc-dir-managed-by-SVN' rsp.
`phw-vc--dir-managed-by-GIT' rsp. `phw-vc-dir-managed-by-BZR'
rsp. `phw-vc-dir-managed-by-MTN' rsp. `phw-vc-dir-managed-by-HG'
are used.

For all eight backends the function `phw-vc-state' of the
VC-package is used by default \(which uses a heuristic and
therefore faster but less accurate approach), but there is also
`phw-vc-recompute-state' available which is an alias for
...
Example: If `phw-vc-recompute-state' \(to get real state-values not
only heuristic ones) should be used to check the state for CVS-managed files
and `phw-vc-state' for all other backends then an element
\(phw-vc-dir-managed-by-CVS . phw-vc-recompute-state) should be added at the
beginning of this option."
  ...
  :type '(repeat (cons :tag "Backend-identifier and state-checker"
                       (choice :tag "Identify-backend-function"
                               ...
                               (const :tag "phw-vc-dir-managed-by-CVS"
                                      :value phw-vc-dir-managed-by-CVS)
                               (const :tag "phw-vc-dir-managed-by-RCS"
                                      :value phw-vc-dir-managed-by-RCS)
                               (const :tag "phw-vc-dir-managed-by-SCCS"
                                      :value phw-vc-dir-managed-by-SCCS)
                               (const :tag "phw-vc-dir-managed-by-SVN"
                                      :value phw-vc-dir-managed-by-SVN)
                               (const :tag "phw-vc-dir-managed-by-GIT"
                                      :value phw-vc-dir-managed-by-GIT)
                               (const :tag "phw-vc-dir-managed-by-BZR"
                                      :value phw-vc-dir-managed-by-BZR)
                               (const :tag "phw-vc-dir-managed-by-MTN"
                                      :value phw-vc-dir-managed-by-MTN)
                               (const :tag "phw-vc-dir-managed-by-HG"
                                      :value phw-vc-dir-managed-by-HG)
                               (function :tag "Any function"))
                       (choice :tag "Check-state-function"
                               :menu-tag "Check-state-function"
                               (const :tag "phw-vc-state (heuristic)"
                                      :value phw-vc-state)
                               (const :tag "phw-vc-recompute-state (accurate)"
                                      :value phw-vc-recompute-state)
                               (function :tag "Any function")))))
...
(defconst phw-directories-nodetype-directory 100)
(defconst phw-directories-nodetype-sourcefile 200)
(defconst phw-directories-nodetype-sourcepath 300)
(defconst phw-sources-nodetype-sourcefile 400)
(defconst phw-history-nodetype-bucket 500)
(defconst phw-history-nodetype-filebuffer 600)
(defconst phw-history-nodetype-indirect-filebuffer 700)


;; accessors for the FILES-AND-SUBDIRS-cache

(defun phw-files-and-subdirs-cache-add (dir cached-value)
  "Add the files and subdirs of DIR to the cache."
  (phw-multicache-put-value 'phw-filename-cache dir 'FILES-AND-SUBDIRS
                             cached-value))

(defun phw-files-and-subdirs-cache-get (dir)
  "Get the files and subdirs of DIR from the cache. Nil if not cached."
  (phw-multicache-get-value 'phw-filename-cache dir 'FILES-AND-SUBDIRS))

(defun phw-files-and-subdirs-cache-remove (dir)
  "Remove DIR from the cache."
  (phw-multicache-clear-value 'phw-filename-cache dir 'FILES-AND-SUBDIRS))

(defun phw-files-and-subdirs-cache-clear ()
  "Clear the whole FILES-AND-SUBDIRS-cache."
  (phw-multicache-clear-subcache 'phw-filename-cache 'FILES-AND-SUBDIRS))

(defun phw-files-and-subdirs-cache-dump (&optional no-nil-value)
...
(defun phw-directory-empty-cache-add (dir cached-value)
  "Add information if DIR is empty or not to the cache."
  (phw-multicache-put-value 'phw-filename-cache dir 'EMPTY-DIR-P
                             cached-value))

(defun phw-directory-empty-cache-get (dir)
  "get information if DIR is empty or not from the cache."
  (phw-multicache-get-value 'phw-filename-cache dir 'EMPTY-DIR-P))

(defun phw-directory-empty-cache-remove (dir)
  "Remove DIR from the EMPTY-DIR-P-cache."
  (phw-multicache-clear-value 'phw-filename-cache dir 'EMPTY-DIR-P))

(defun phw-directory-empty-cache-remove-all (dir)
  "Remove DIR and all its suddirs from the EMPTY-DIR-P-cache."
  (phw-directory-empty-cache-remove dir)
  ;; now we remove the subdirs
  (save-match-data
    (phw-multicache-mapsubcache
     'phw-filename-cache 'EMPTY-DIR-P
...
(defun phw-directory-empty-cache-clear ()
  "Clear the whole EMPTY-DIR-P-cache."
  (phw-multicache-clear-subcache 'phw-filename-cache 'EMPTY-DIR-P))

(defun phw-directory-empty-cache-dump (&optional no-nil-value)
...
(defun phw-sources-cache-remove (dir)
  "Remove the cache-entry for DIR from the cache."
  (phw-multicache-clear-value 'phw-filename-cache dir 'SOURCES))

(defun phw-sources-cache-add-full (dir cache-elem-full)
...
list as returned by `phw-sources-cache-get-full'."
  (phw-multicache-apply-to-value
   'phw-filename-cache dir 'SOURCES
...
(defun phw-sources-cache-add-filtered (dir cache-elem-filtered)
...
has to be a list as returned by `phw-sources-cache-get-filtered'."
  (phw-multicache-apply-to-value
   'phw-filename-cache dir 'SOURCES
   (function (lambda (old-cached-value)
               (if (consp old-cached-value)
...
(defun phw-sources-cache-get-full (dir)
...
cache-entry for DIR is available then nil is returned."
  (car (phw-multicache-get-value 'phw-filename-cache dir 'SOURCES)))

(defun phw-sources-cache-get-filtered (dir)
...
then nil is returned."
  (cdr (phw-multicache-get-value 'phw-filename-cache dir 'SOURCES)))

(defun phw-sources-cache-clear ()
  "Clear the whole SOURCES-cache."
  (phw-multicache-clear-subcache 'phw-filename-cache 'SOURCES))

(defun phw-sources-cache-dump (&optional no-nil-value)
  "Dump the whole SOURCES-cache. If NO-NIL-VALUE is not nil then these
cache-entries are not dumped. This command is not intended for end-users of
PHW."
  (interactive "P")
  (phw-multicache-print-subcache 'phw-filename-cache 'SOURCES no-nil-value))

;; accessors for the VC-cache

(defun phw-vc-cache-add-file (file state checked-buffer-names)
  (phw-multicache-put-value 'phw-filename-cache file 'VC
                            (list state
                                  (phw-subseq (current-time) 0 2)
                                  checked-buffer-names))
  state)

(defun phw-vc-cache-add-dir (dir backend)
  (phw-multicache-put-value 'phw-filename-cache dir 'VC backend)
  backend)

(defun phw-vc-cache-get (file)
  (phw-multicache-get-value 'phw-filename-cache file 'VC))

(defun phw-vc-cache-remove (file)
  "Remove FILE from the VC-cache."
  (phw-multicache-clear-value 'phw-filename-cache file 'VC))

(defun phw-vc-cache-remove-files-of-dir (dir)
  "Remove all files contained in DIR from the VC-cache."
  (let* ((dir-sep-string (phw-directory-sep-string dir))
         (regexp (concat "^"
                         (regexp-quote dir)
...
(defun phw-vc-cache-clear ()
  "Clear the whole VC-cache."
  (phw-multicache-clear-subcache 'phw-filename-cache 'VC))

(defun phw-vc-cache-dump (&optional no-nil-value)
  "Dump the whole VC-cache. If NO-NIL-VALUE is not nil then these
cache-entries are not dumped. This command is not intended for end-users of
PHW."
  (interactive "P")
  (phw-multicache-print-subcache 'phw-filename-cache 'VC no-nil-value))

;; accessors for the REMOTE-PATH cache

(defun phw-remote-path-cache-add (path remote-path)
  "Add the value of REMOTE-PATH for PATH to the REMOTE-PATH-cache."
  (phw-multicache-put-value 'phw-filename-cache path 'REMOTE-PATH
                            remote-path))

(defun phw-remote-path-cache-get (path)
  "Return the cached value for PATH from the REMOTE-PATH-cache."
  (phw-multicache-get-value 'phw-filename-cache path 'REMOTE-PATH))

(defun phw-remote-path-cache-dump (&optional no-nil-value)
  "Dump the whole REMOTE-PATH-cache. If NO-NIL-VALUE is not nil then these
cache-entries are not dumped. This command is not intended for end-users of
PHW."
  (interactive "P")
  (phw-multicache-print-subcache 'phw-filename-cache 'REMOTE-PATH no-nil-value))

;; accessors for the HOST-ACCESSIBLE cache

(defun phw-host-accessible-cache-add (host accessible-p)
  "Add the value of ACCESSIBLE-P to the HOST-ACCESSIBLE-cache with key HOST."
  (phw-multicache-put-value 'phw-filename-cache host 'HOST-ACCESSIBLE
                            (cons (current-time) accessible-p)))

(defun phw-host-accessible-cache-get (host valid-time)
...
entry is older then VALID-TIME \(in seconds) then it is discarded."
  (let ((value (phw-multicache-get-value 'phw-filename-cache host
                                         'HOST-ACCESSIBLE)))
    (if (or (null value)
            (> (phw-time-diff (current-time) (car value) t) valid-time))
        ;; either not yet cached or outdated
        nil
      ;; return the valid cache-value
      (cdr value))))

(defun phw-host-accessible-cache-dump (&optional no-nil-value)
  "Dump the whole HOST-ACCESSIBLE-cache. If NO-NIL-VALUE is not nil then these
cache-entries are not dumped. This command is not intended for end-users of
PHW."
  (interactive "P")
  (phw-multicache-print-subcache 'phw-filename-cache 'HOST-ACCESSIBLE no-nil-value))


;; ---- end of filename-cache implementation -----------------------

(defun phw-file-browser-initialize-caches ()
  "Initialize the caches of the file-browser of PHW."
  (phw-reset-history-filter)
  (phw-filename-cache-init))

(defun phw-file-browser-initialize (&optional no-caches)
  "Initialize the file-browser of PHW. If optional arg NO-CACHES is not nil
then the caches used by the file-browser will not be initialized."
  (setq phw-path-selected-directory nil
        phw-path-selected-source nil)
  (unless no-caches
    (phw-file-browser-initialize-caches)))
  
(defun phw-goto-window-directories ()
  "Make the PHW-directories window the current window.
If `phw-use-speedbar-instead-native-tree-buffer' is 'dir then goto to the
speedbar-window."
  (interactive)
  (or (phw-goto-phw-window phw-directories-buffer-name)
      (and (equal phw-use-speedbar-instead-native-tree-buffer 'dir)
           (phw-goto-window-speedbar))))

(defun phw-goto-window-sources ()
  "Make the PHW-sources window the current window.
If `phw-use-speedbar-instead-native-tree-buffer' is 'source then goto to the
speedbar-window."
  (interactive)
  (or (phw-goto-phw-window phw-sources-buffer-name)
      (and (equal phw-use-speedbar-instead-native-tree-buffer 'source)
           (phw-goto-window-speedbar))))

(defun phw-goto-window-history ()
  "Make the PHW-history window the current window."
  (interactive)
  (phw-goto-phw-window phw-history-buffer-name))

(defun phw-maximize-window-directories ()
  "Maximize the PHW-directories-window.
...
PHW-directories-window is not visible in current layout."
  (interactive)
  (if (equal phw-use-speedbar-instead-native-tree-buffer 'dir)
      (phw-maximize-window-speedbar)
    (phw-maximize-phw-buffer phw-directories-buffer-name t)))

(defun phw-maximize-window-sources ()
  "Maximize the PHW-sources-window.
I.e. delete all other PHW-windows, so only one PHW-window and the
edit-window\(s) are visible \(and maybe a compile-window). Works also if the
PHW-sources-window is not visible in current layout."
  (interactive)
  (if (equal phw-use-speedbar-instead-native-tree-buffer 'source)
      (phw-maximize-window-speedbar)
    (phw-maximize-phw-buffer phw-sources-buffer-name t)))

(defun phw-maximize-window-history ()
  "Maximize the PHW-history-window.
I.e. delete all other PHW-windows, so only one PHW-window and the
edit-window\(s) are visible \(and maybe a compile-window). Works also if the
PHW-history-window is not visible in current layout."
  (interactive)
  (phw-maximize-phw-buffer phw-history-buffer-name t))

(defphw-window-dedicator-to-phw-buffer phw-set-directories-buffer
    phw-directories-buffer-name t
  "Display the Directories-buffer in current window and make window dedicated."
  (let ((set-directories-buffer
         (not (equal phw-use-speedbar-instead-native-tree-buffer 'dir))))
    ;; first we act depending on the value of
    ;; phw-use-speedbar-instead-native-tree-buffer
    ...
    ;; - if phw-use-speedbar-instead-native-tree-buffer is not 'dir or
    ;; - if setting the speedbar buffer has failed.
    (when set-directories-buffer
      (if (null phw-use-speedbar-instead-native-tree-buffer)
          (ignore-errors (phw-speedbar-deactivate)))
      (switch-to-buffer phw-directories-buffer-name))))

(defphw-window-dedicator-to-phw-buffer phw-set-sources-buffer
    phw-sources-buffer-name t
  "Display the Sources-buffer in current window and make window dedicated."
  (let ((set-sources-buffer
         (not (equal phw-use-speedbar-instead-native-tree-buffer 'source))))
    ;; first we act depending on the value of
    ;; phw-use-speedbar-instead-native-tree-buffer
    ...
    ;; - if phw-use-speedbar-instead-native-tree-buffer is not 'source or
    ;; - if setting the speedbar buffer has failed.
    (when set-sources-buffer
      (if (null phw-use-speedbar-instead-native-tree-buffer)
          (ignore-errors (phw-speedbar-deactivate)))
      (switch-to-buffer phw-sources-buffer-name))))

(defphw-window-dedicator-to-phw-buffer phw-set-history-buffer phw-history-buffer-name t
  "Display the History-buffer in current window and make window dedicated."
  (switch-to-buffer phw-history-buffer-name))

(defphw-autocontrol/sync-function phw-basic-buffer-sync nil phw-basic-buffer-sync nil
  "Synchronizing the basic tree-buffers of PHW.

The basic phw-buffers are the tree-buffers for drirectories, sources, history
and methods.
...
At the end the hooks in `phw-basic-buffer-sync-hook' run."
  (when (and phw-minor-mode
    ...
    (let* ((filename (phw-buffer-file-name (current-buffer))))
      (cond ( ;; synchronizing for real filesource-buffers and indirect
             ...
             (and filename
                  (phw-buffer-or-file-readable-p filename)
                  (or force
                      (not (equal (phw-source-make filename (buffer-name))
             ...
             ;; * KB: With current PHW implementation this sit-for seems not
             ...
             ;; `phw-source-path' or in the paths returned from
             ;; `phw-source-path-functions' we must at least add the new
             ;; source path temporally to our paths. But the user has also
             ;; the choice to save it for future sessions too.
             (if (null (phw-matching-source-paths filename))
                 (let* ((norm-filename (phw-fix-filename filename))
                        (remote-path (phw-remote-path norm-filename))
                        (source-path (if (car phw-add-path-for-not-matching-files)
                                       ...
                                       (phw-file-name-directory norm-filename))))
                   (phw-add-source-path source-path (phw-fix-filename source-path)
                                        (not (cdr phw-add-path-for-not-matching-files)))))
             ...
             ;; `phw-select-source'!
             ;; (phw-update-directories-buffer)
             (phw-path-selected-source-set filename (buffer-name))
             (phw-select-source force)
             (phw-update-methods-buffer--internal 'scroll-to-begin)
             (setq phw-major-mode-selected-source major-mode)
             
             ;; Klaus Berndl <klaus.berndl@sdm.de>: is now be done at the
             ;; end of `phw-rebuild-methods-buffer-with-tagcache' which is
             ;; called by `phw-update-methods-buffer--internal'!
             
             ;; selected source has changed, therefore we must initialize
             ;; phw-selected-tag again.
             (phw-tag-sync 'force)
             )
            ...
            ( ;; synchronizing for dired-mode
             ...
             (phw-set-selected-directory
              (or (and (stringp dired-directory)
                       (phw-file-exists-p dired-directory)
                       dired-directory)
                  (and (listp dired-directory)
                       (car dired-directory)))))
            (t nil)))
    (run-hooks 'phw-basic-buffer-sync-hook)
    ))

  
(defun phw-expand-directory-tree (path node)
    ...
    (dolist (child (tree-node->children node))
      (let ((data (tree-node->data child)))
        (when (and (>= (length path) (length data))
                   (phw-string= (substring path 0 (length data)) data)
          ...
          (let ((was-expanded (or (not (tree-node->expandable child))
            ...
            (phw-update-directory-node child)
            (throw 'exit
                   (or (when (> (length path) (length data))
                         (phw-expand-directory-tree path child))
                       (not was-expanded)))))))))


(defun phw-check-directory-for-caching (dir number-of-contents)
  "Return not nil if DIR matches not any regexp of the option
`phw-cache-directory-contents-not' but matches at least one regexp in
`phw-cache-directory-contents' and NUMBER-OF-CONTENTS is greater then the
related threshold."
  (and (not (catch 'exit
              (dolist (elem phw-cache-directory-contents-not)
                (let ((case-fold-search t))
                  ...
                  nil))))
       (catch 'exit
         (dolist (elem phw-cache-directory-contents)
           (let ((case-fold-search t))
             (save-match-data
...
(defun phw-check-directory-for-source-regexps (dir)
  "Return the related source-exclude-include-regexps of
`phw-source-file-regexps' if DIR matches any directory-regexp in
`phw-source-file-regexps'."
  (phw-match-regexp-list dir phw-source-file-regexps 'car 'cdr))


(defun phw-files-from-cvsignore (dir)
  "Return an expanded list of filenames which are excluded by the .cvsignore
file in current directory."
  (let ((cvsignore-content (phw-file-content-as-string
                            (phw-expand-file-name ".cvsignore" dir)))
        (files nil))
    (when cvsignore-content
      (dolist (f (split-string cvsignore-content))
        (setq files (append (phw-directory-files dir nil
                                                 (wildcard-to-regexp f) t)
                            files)))
      files)))


(defun phw-check-directory-for-cvsignore-exclude (dir)
  "Return not nil if DIR matches a regexp in `phw-sources-exclude-cvsignore'."
  (phw-match-regexp-list dir phw-sources-exclude-cvsignore))

(defun phw-get-sources-sort-function (sort-method &optional ignore-case)
...
and IGNORE-CASE return a function which can be used as argument for `sort'."
  (case sort-method
    (name
     (function (lambda (a b)
                 (phw-string< a b phw-sources-sort-ignore-case))))
    (extension
     (function
      (lambda(a b)
        (let ((ext-a (phw-file-name-extension a t))
              (ext-b (phw-file-name-extension b t)))
          (if (phw-string= ext-a ext-b phw-sources-sort-ignore-case)
              (phw-string< a b phw-sources-sort-ignore-case)
            (phw-string< ext-a ext-b phw-sources-sort-ignore-case))))))
    (otherwise
     (function (lambda (a b)
                 nil)))))


(defun phw-get-files-and-subdirs (dir)
...
according to `phw-sources-sort-method'."
  (when (file-accessible-directory-p dir)
      (or (phw-files-and-subdirs-cache-get dir)
	  ;; dir is not cached
	  (let ((files (phw-directory-files dir nil nil t))
		(source-regexps (or (phw-check-directory-for-source-regexps
				     (phw-fix-filename dir))
				    '(("") (""))))
		(cvsignore-files (if (phw-check-directory-for-cvsignore-exclude dir)
				     (phw-files-from-cvsignore dir)))
		sorted-files source-files subdirs cached-value)
	    ;; if necessary sort FILES
	    (setq sorted-files
		  (if phw-sources-sort-method
		      (sort files (phw-get-sources-sort-function
				   phw-sources-sort-method))
		    files))
	    ;; divide real files and subdirs. For really large directories (~ >=
	    ;; 2000 entries) this is the performance-bottleneck in the
	    ;; file-browser of PHW.
	    (dolist (file sorted-files)
	      (if (phw-file-directory-p (phw-fix-filename dir file))
		  (when (not (phw-check-dir-exclude file))
;;                 (when (not (phw-file-accessible-directory-p file))
;;                   (phw-merge-face-into-text file
;;                                             phw-directory-not-accessible-face))
		    (setq subdirs (append subdirs (list file))))
		(when (and (not (member file cvsignore-files))
			   (or (phw-match-regexp-list file (cadr source-regexps))
			       (not (phw-match-regexp-list file (car source-regexps)))))
	    ...
	    (if (phw-check-directory-for-caching dir (length sorted-files))
		(phw-files-and-subdirs-cache-add dir cached-value))
	    ;; return the result
	    cached-value))))


(defun phw-update-sources-buffer (dir-before-update)
  "Updates the sources-buffer with all sources contained in
`phw-path-selected-directory' - the contents are either newly computed or come
from the `phw-sources-cache'. DIR-BEFORE-UPDATE is the directory which was
selected before this update."
  ...
  ;; comment in `phw-rebuild-methods-buffer-with-tagcache'). If we would
  ;; make the nodes in the Sources-buffer "expandable" this caching would not
  ;; work!
  
  (phw-exec-in-window phw-sources-buffer-name
    ;; if we have a filtered cache we must display it - otherwise we use the
    ;; full cache if there is any
    (let ((cache-elem (or (phw-sources-cache-get-filtered phw-path-selected-directory)
                          (phw-sources-cache-get-full phw-path-selected-directory))))
      (if cache-elem
          (progn
        ...
        (let ((new-tree (tree-node-new-root))
          ...
          (phw-tree-node-add-files
           new-tree
           phw-path-selected-directory
           (car (phw-get-files-and-subdirs phw-path-selected-directory))
           phw-sources-nodetype-sourcefile
           phw-show-source-file-extension old-children t)
          ...
          (when (phw-check-directory-for-caching
                 phw-path-selected-directory
                 (tree-buffer-number-of-displayed-nodes))
            (setq new-cache-elem (list (tree-buffer-get-root)
                                       (tree-buffer-displayed-nodes-copy)
                                       (phw-buffer-substring (point-min)
                                                             (point-max))))
            (phw-sources-cache-add-full phw-path-selected-directory
                                        new-cache-elem))))
           
      (when (not (phw-string= dir-before-update phw-path-selected-directory))
        (phw-scroll-window (point-min) (point-min))))))

(defun phw-sources-filter-by-ext (ext-str)
  "Filter the sources by extension EXT-STR."
  (if (= (length ext-str) 0)
      (phw-apply-filter-to-sources-buffer
...
(tree-buffer-defpopup-command phw-popup-sources-filter-by-ext
  "Filter the sources by extension by popup."
  (phw-sources-filter-by-ext
   (read-string "Insert the filter-extension without leading dot: "
                (and node
                     (phw-file-name-extension (tree-node->data node))))))

(defun phw-sources-filter-by-regexp (&optional regexp filter-display)
...
regexp-filter. Otherwise the regexp itself."
  (let ((regexp-str (or regexp (read-string "Insert the filter-regexp: "))))
    (if (> (length regexp-str) 0)
        (phw-apply-filter-to-sources-buffer regexp-str filter-display))))
  
(tree-buffer-defpopup-command phw-popup-sources-filter-by-regexp
  "Filter the sources by regexp by popup."
  (phw-sources-filter-by-regexp))
  
(tree-buffer-defpopup-command phw-popup-sources-filter-none
  "Remove any filter from the sources by popup."
  (phw-apply-filter-to-sources-buffer nil))
  

(defun phw-sources-filter ()
...
- No filter: This means to display an entry for every file in the current
  ...
  `phw-source-file-regexps' and `phw-sources-exclude-cvsignore').
Such a filter is only applied to the current selected directory, i.e. each
directory has its own filtered sources-buffer."
  (interactive)
  (let ((choice (phw-query-string "Filter sources by:"
                                  '("extension" "regexp" "nothing"))))
    (cond ((phw-string= choice "extension")
           (phw-sources-filter-by-ext
            (read-string "Insert the filter-extension without leading dot: ")))
          ((phw-string= choice "regexp")
           (phw-sources-filter-by-regexp))
          (t (phw-apply-filter-to-sources-buffer nil)))))

(defun phw-sources-filter-modeline-prefix (buffer-name sel-dir sel-source)
...
the option `phw-mode-line-prefixes'."
  (let ((filtered-cache-elem (phw-sources-cache-get-filtered sel-dir)))
    (if (null filtered-cache-elem)
        nil ;; no prefix if no filter
      (format "[Filter: %s]" (cdr (nth 3 filtered-cache-elem))))))

(defun phw-apply-filter-to-sources-buffer (filter-regexp &optional filter-display)
  "Apply the regular expression FILTER-REGEXP to the files of
`phw-path-selected-directory' and display only the filtered files in the
...
nil. Returns 'window-not-visible if the PHW-sources-buffer is not visible."
  (prog1
      (phw-exec-in-window phw-sources-buffer-name
        (if (or (null filter-regexp) (= (length filter-regexp) 0))
            ...
            (progn
              ...
              (phw-sources-cache-add-filtered phw-path-selected-directory nil)
              ;; update the sources buffer - because the filtered cache is nil
              ;; the full sources are displayed.
              (phw-update-sources-buffer phw-path-selected-directory)
              (tree-buffer-highlight-node-by-data/name (phw-path-selected-source 'file))
              nil)
          ;; apply the filter-regexp
          (let ((new-tree (tree-node-new-root))
                (old-children (tree-node->children (tree-buffer-get-root)))
                (all-files (car (phw-get-files-and-subdirs phw-path-selected-directory)))
                (filtered-files nil))
            (save-match-data
            ...
            (if (null filtered-files)
                (progn
                  (phw-apply-filter-to-sources-buffer nil)
                  (message "PHW has not applied this filter because it would filter out all files!")
                  nil)
              ;; building up the new files-tree
              (phw-tree-node-add-files
               new-tree
              ...
              ;; `phw-update-sources-buffer' displays the filtered sources.
              (phw-sources-cache-add-filtered phw-path-selected-directory
    ...
    ;; displayed in the mode-line. See `phw-sources-filter-modeline-prefix'.
...
;; phw-source-path-functions only return paths without alias
;; (setq phw-source-path-functions
;;       (list (lambda ()
;;               '("c:/Programme/emacs-23.1/site-lisp/package-development/phw"
...
(defun phw-normed-source-paths (&optional no-duplicates)
  "Return a normalized list of all source-paths.

This is a list created from all elements of `phw-source-path' and all
source-paths created by `phw-source-path-functions'.

Each element is a cons whereas car is the normed and expanded pathname \(done by
`phw-fix-filename') and cdr is either the alias defined for this path \(see
`phw-source-path') or - if there is no alias defined - the path itself \(in
...
matches exactly the car of another element."
  (let ((res (mapcar (function (lambda (elem)
                                 (let* ((path (phw-fix-filename (if (listp elem) (nth 0 elem) elem)))
                                        (alias (if (listp elem) (nth 1 elem) path)))
                                   (cons path alias))))
                     (append phw-source-path
                             (phw-get-source-paths-from-functions)
                             ))))
    (if no-duplicates
        (phw-delete-duplicates
         res
         (function (lambda (l r)
...
(defun phw-matching-source-paths (path-to-match &optional sorted)
  "Return all source-paths of `phw-source-path' which match PATH-TO-MATCH. If
...
they occur in `phw-source-path'.
Each matching path is a cons in the sense of `phw-normed-source-paths'."
  (let* ((p-t-m (phw-fix-filename path-to-match))
         (normed-current-source-paths (phw-normed-source-paths))
         (matching-paths
          (delq nil
                (mapcar (lambda (elem)
                          (save-match-data
...
(defun phw-get-best-matching-source-path (path)
  "Return the best-matching source-path for PATH.
It's either nil if no source-path matches or a cons in the sense of
`phw-normed-source-paths'."
  (car (phw-matching-source-paths path t)))

(defun phw-set-selected-directory (path &optional force)
  "Set the contents of the PHW-directories and -sources buffer correct for the
value of PATH. If PATH is equal to the value of `phw-path-selected-directory'
then nothing is done unless first optional argument FORCE is not nil."
  (let ((last-dir phw-path-selected-directory))
    (setq phw-path-selected-directory (phw-fix-filename path))
    ;; if phw-path-selected-directory has not changed then there is no need
    ...
    (when (or force (not (phw-string= last-dir phw-path-selected-directory)))
      (when (or (not (phw-show-sources-in-directories-buffer-p))
                phw-auto-expand-directory-tree)
        (phw-exec-in-window phw-directories-buffer-name
          (let (start was-expanded)
            (when phw-auto-expand-directory-tree
              ;; Expand tree to show selected directory
              (setq start
                    (if (equal phw-auto-expand-directory-tree 'best)
                        ;; If none of the source-paths in the buffer
                        ;; `phw-directories-buffer-name' matches then nil
                        ;; otherwise the node of the best matching
                        ;; source-path
                        (let ((best-source-path
                               (car (phw-get-best-matching-source-path
                                     phw-path-selected-directory))))
                          (if best-source-path
                               ...
                               (function
                                (lambda (node)
                                  (if (and (tree-buffer-node-data-equal-p
                                            (tree-node->data node)
                                            (phw-fix-filename best-source-path))
              ...
              (when (and (equal phw-auto-expand-directory-tree 'best)
                         start)
                ...
                ;; start-node
                (phw-update-directory-node start))
              ;; start recursive expanding of either the best-matching node or
              ;; the root-node itself.
              (if (or (phw-expand-directory-tree phw-path-selected-directory
                                                 (or start
                                                     (tree-buffer-get-root)))
                      (not was-expanded))
                  (tree-buffer-update)
                ...
                (and force (phw-stealth-tasks-after-directories-update))))
            ;;              (phw-expand-directory-tree phw-path-selected-directory
            ;;                                           (or start
            ;;                                               (tree-buffer-get-root)))
            ;;                (tree-buffer-update))
            (when (not (phw-show-sources-in-directories-buffer-p))
              (tree-buffer-highlight-node-by-data/name phw-path-selected-directory
                                                       nil start)))))
      ;; now we update the sources buffer for `phw-path-selected-directory'
      (phw-update-sources-buffer last-dir)
      ;; now we run the hooks
      (run-hook-with-args 'phw-after-directory-change-hook
                          last-dir phw-path-selected-directory)
      ))
  ...
  (save-excursion
    (dolist (buf (phw-phw-buffer-registry-buffer-list))
      (set-buffer buf)
      (setq default-directory
            (concat phw-path-selected-directory
                    (and (not (= (aref phw-path-selected-directory
                                       (1- (length phw-path-selected-directory)))
                                 (phw-directory-sep-char phw-path-selected-directory)))
                         (phw-directory-sep-string phw-path-selected-directory))))))
  ;; set the modelines of all visible tree-buffers new
  (phw-mode-line-format))


(defun phw-get-source-name (filename)
  "Returns the source name of a file."
  (let ((f (phw-file-name-nondirectory filename)))
    (if phw-show-source-file-extension
        f
      (phw-file-name-sans-extension f))))

(defun phw-select-source (&optional force)
...
even if current directory is equal to `phw-path-selected-directory'."
  (phw-set-selected-directory (phw-file-name-directory
                               (phw-path-selected-source 'file)) force)
    
  ;; Update directory buffer
  (when (phw-show-sources-in-directories-buffer-p)
    (phw-exec-in-window phw-directories-buffer-name
      (tree-buffer-highlight-node-by-data/name (phw-path-selected-source 'file))))
...
(defun phw-reset-history-filter ()
  "Reset the `phw-history-filter' so all file-buffers are displayed."
  (setq phw-history-filter (cons (function
                                  (lambda (buf file)
                                    t))
                                 nil)))

(defun phw-history-filter-reset-p ()
  (null (cdr phw-history-filter)))

(phw-reset-history-filter)

(defun phw-indirect-buffers-of-buffer (&optional buffer-or-name)
...
;; removed.
(defun phw-history-kill-buffer-clear (curr-buf)
  "Does all necessary clearence when CURR-BUF is killed."
  (let* ((buffer-file (phw-fix-filename (phw-buffer-file-name curr-buf)))
         (node (if buffer-file
                   (phw-exec-in-window phw-history-buffer-name
                     (tree-buffer-find-displayed-node-by-data/name
                      (phw-source-make buffer-file curr-buf)))))
    ...
    (when (tree-node-p node)
      (when (or (buffer-base-buffer curr-buf) ; indirect-buffers always!
                (equal phw-kill-buffer-clears-history 'auto)
                (and (equal phw-kill-buffer-clears-history 'ask)
        ...
        (with-current-buffer phw-history-buffer-name
      ...
      (phw-add-buffers-to-history-new nil buffer-name-to-ignore-list-for-rebuild))))

(defun phw-add-all-buffers-to-history ()
  "Add all current file-buffers to the history-buffer of PHW.
Dependend on the values of `phw-history-make-buckets' and
`phw-history-sort-method' afterwards the history is bucketized or
not and sorted either by name or by extension \(if bucketized
then the sorting is only within each bucket). If
`phw-history-sort-method' is nil the most recently used buffers
...
bucket)."
  (interactive)
  (phw-reset-history-filter)
  (phw-add-buffers-to-history-new 'no-dead-buffers))

(defalias 'phw-clear-history 'phw-add-all-buffers-to-history)

(defun phw-history-content-all-dead-buffers-alist ()
               ...
               (function
                (lambda (node)
                  (let ((data (tree-node->data node)))
                    (unless (or (= (tree-node->type node) phw-history-nodetype-bucket)
                                (get-buffer (phw-source-get-buffername data)))
                      (cons (phw-source-get-buffername data)
                            (phw-source-get-filename data))))))))))

(defun phw-add-buffers-to-history-new (&optional no-dead-buffers ignore-buffername-list)
...
- is not excluded by `phw-check-filename-for-history-exclude'
...
It takes into account the values of the options `phw-history-make-buckets' and 
`phw-history-stick-indirect-buffers-to-basebuffer'.

It calls at the end `phw-mode-line-format'.
  ...
  (let* ((never-bucket-string "No_hist_bucketizing")
         (aggr-sort-fcn (function
                         (lambda (l r)
                           ...
                           (if (phw-string= (car l) (car r) phw-history-sort-ignore-case)
                               (case phw-history-sort-method
                                 (extension
                                  (let ((ext-l (file-name-extension (cdr (cdr l)) t))
                                        (ext-r (file-name-extension (cdr (cdr r)) t)))
                                    (if (phw-string= ext-l ext-r phw-history-sort-ignore-case)
                                        (phw-string< (car (cdr l)) (car (cdr r))
                                                     phw-history-sort-ignore-case)
                                      (phw-string< ext-l ext-r phw-history-sort-ignore-case))))
                                 (name
                                  (phw-string< (car (cdr l)) (car (cdr r))
                                               phw-history-sort-ignore-case))
                                 (otherwise nil))
                             (phw-string< (car l) (car r) phw-history-sort-ignore-case)))))
         (aggr-same-fcn (function
                         (lambda (l r)
                           ;; l and r are strings (= the car of an item of base-alist)
                           (phw-string= l r phw-history-sort-ignore-case))))
         ...
         ;; - not excluded by `phw-check-filename-for-history-exclude'
         ...
         (buf-file-alist
          (delq nil
                (mapcar (function
                         (lambda (buf)
                           (let ((file-name (phw-fix-filename
                                             (phw-buffer-file-name buf)))
                             ...
                             (if (and file-name
                                      ...
                                      ;; into a buffer
                                      (phw-buffer-or-file-readable-p file-name)
                                      (not (member (buffer-name buf)
                                                   ignore-buffername-list))
                                      (not (phw-check-filename-for-history-exclude file-name))
                                      (funcall (car phw-history-filter)
                                               (buffer-name buf)
                                               file-name))
                                 (if (and phw-history-stick-indirect-buffers-to-basebuffer
                                          base-buf)
         ...
         (additonal-dead-history-buffer-alist
          (if no-dead-buffers
            ...
            (phw-set-difference (phw-history-content-all-dead-buffers-alist)
                                buf-file-alist
                                'member)))
         (base-alist (mapcar (function
                              (lambda (elem)
                                ;; an elem is a cons (<buffername> . <filename>)
                                (cons (case phw-history-make-buckets
                                        (never never-bucket-string)
                                        ((directory directory-with-source-path)
                                         (phw-substring-no-properties
                                          (phw-fix-filename
                                           (phw-file-name-directory
                                            (cdr elem)))
                                          (if phw-running-xemacs 0)))
                                        (mode (symbol-name
                                               (if (phw-buffer-obj (car elem))
                                                   (with-current-buffer (phw-buffer-obj (car elem))
                                                     major-mode)
                                                 ;; for dead buffers of the
                                                 ;; history we use auto-mode-alist
                                                 (or (phw-match-regexp-list (car elem)
                                                                            auto-mode-alist
                                                                            'car 'cdr)
                                                     'no-major-mode-avail))))
                                        (extension (file-name-extension (cdr elem) t))
                                        (otherwise (or (phw-match-regexp-list
                                                        (car elem)
                                                        phw-history-make-buckets)
                                                       "No matchings")))
                                      elem)))
                             (append buf-file-alist
                                     additonal-dead-history-buffer-alist)))
         (aggregated-alist-with-buckets (phw-aggregate-alist base-alist
                                                             aggr-same-fcn
                                                             aggr-sort-fcn))
         (aggregated-indirect-buffers-alist (phw-aggregate-alist indirect-buffer-base
                                                                 'string=
                                                                 (function
                                                                  (lambda (l r)
                                                                    (string< (car l)
...
;;           aggregated-indirect-buffers-alist)
    (with-current-buffer phw-history-buffer-name
      ...
      (dolist (bucket-elem aggregated-alist-with-buckets)
        (let* ((best-matching-sp (if (eq phw-history-make-buckets 'directory-with-source-path)
                                    (phw-get-best-matching-source-path (car bucket-elem))))
               ...
               (bucket-name-formated-shrink-start-pos (length (nth 0 phw-bucket-node-display)))
               (bucket-node nil))
          (if (or (null best-matching-sp)
              ...
              (setq bucket-name-formated
                    (phw-format-bucket-name
                     ;; we use concat only to get a new string-object
                     (phw-merge-face-into-text (concat (car bucket-elem))
                                               phw-history-bucket-node-face)
                     nil 'only-name))
            (setq bucket-name-formated
                  (phw-format-bucket-name
                   (concat (phw-merge-face-into-text
                            (phw-merge-face-into-text
                             best-matching-alias phw-history-bucket-node-face)
                            phw-history-bucket-node-dir-soure-path-face)
          ...
          (dolist (elem (cdr bucket-elem))
            (let* ((buf-name (car elem))
                   ...
                   ;; `phw-history-stick-indirect-buffers-to-basebuffer' is not
                   ;; nil --> see above the mechanism how indirect-buffer-base
                   ;; is build
                   (indirect-buffer-p (buffer-base-buffer (phw-buffer-obj buf-name)))
                   ...
                   (buf-name-formatted (cond (indirect-buffer-p
                                              (phw-merge-face-into-text
                                               buf-name
                                               phw-history-indirect-buffer-face))
                                             ((member elem
                                                      additonal-dead-history-buffer-alist)
                                              (phw-merge-face-into-text
                                               buf-name
                                               phw-history-dead-buffer-face))
                                             (t buf-name)))
                   (file-name (cdr elem))
                   (dir (phw-file-name-directory file-name))
                   (vc-p (and (phw-vc-directory-should-be-checked-p dir)
                              (phw-vc-managed-dir-p dir)))
                   (node-name (if vc-p
                                  (phw-vc-generate-node-name buf-name-formatted
                                                             (nth 0 (phw-vc-cache-get file-name)))
                                (phw-generate-node-name buf-name-formatted -1 "leaf"
                                                        ;; here phw-sources-buffer-name is
                   ...
                   (node (tree-node-new
                          ...
                          (if indirect-buffer-p
                              phw-history-nodetype-indirect-filebuffer
                            phw-history-nodetype-filebuffer)
                          (phw-source-make file-name buf-name)
                          t
                          bucket-node)))
              ;; if `phw-history-stick-indirect-buffers-to-basebuffer' is nil
              ...
              ;; case 
              (dolist (indirect-elem (phw-find-assoc-value buf-name
                                                           aggregated-indirect-buffers-alist))
                (let* ((ind-buf-name (car indirect-elem))
                       ...
                       ;; s.a.
                       (ind-buf-name-formatted (phw-merge-face-into-text
                                                ind-buf-name
                                                phw-history-indirect-buffer-face))
                       (ind-node-name (if vc-p
                                          (phw-vc-generate-node-name ind-buf-name-formatted
                                                                     (nth 0 (phw-vc-cache-get file-name)))
                                        (phw-generate-node-name ind-buf-name-formatted -1 "leaf"
                                                                ;; here phw-sources-buffer-name is
                                                                ;; also correct for history
                                                                ;; because we want the same
                                                                ;; icon-stuff as in the sources buffer
                  ...
                  (tree-node-new
                   ...
                   ind-node-name
                   phw-history-nodetype-indirect-filebuffer
                   (phw-source-make file-name ind-buf-name)
                   t
                   node)))
              )))))
    (phw-exec-in-window phw-history-buffer-name
      (tree-buffer-update)
      (tree-buffer-highlight-node-by-data/name (phw-path-selected-source)))
    (prog1
        (if (and (with-current-buffer phw-history-buffer-name
                   (tree-buffer-empty-p))
                 (not (phw-history-filter-reset-p)))
            (progn
              (phw-add-all-buffers-to-history)
              (message "PHW has not applied this filter because it would filter out all entries!")
              nil)
          t)
...
;;(insert (pp (phw-klaus-add-buffers-to-history)))

;;(phw-add-buffers-to-history-new)

(defun phw-history-filter-modeline-prefix (buffer-name sel-dir sel-source)
...
by the option `phw-mode-line-prefixes'."
  (and (cdr phw-history-filter)
       (format "[Filter: %s]" (cdr phw-history-filter))))


(defun phw-set-selected-source (source other-edit-window
                                       no-edit-buffer-selection)
  "Updates all the PHW buffers and loads the SOURCE. The source is also
...
PHW-Buffer-updating is done but the content of the main-edit window is not
...
indirect-buffer."
  (if no-edit-buffer-selection
      ...
      (with-current-buffer (phw-source-get-buffer source)
        (phw-path-selected-source-set (phw-source-get-filename source)
                                      (buffer-name))
        (phw-update-methods-buffer--internal 'scroll-to-begin nil t t))
    ;; open the selected source in the correct edit-window and do all the
    ;; update and parsing stuff with this buffer
    (phw-display-source source other-edit-window)
    (phw-path-selected-source-set (phw-source-get-filename source)
                                  (buffer-name))
    (phw-update-methods-buffer--internal 'scroll-to-begin)
    (setq phw-major-mode-selected-source major-mode)
...
(defun phw-update-directory-node (node)
  ...
  (let ((old-children (tree-node->children node))
    ...
    (if (phw-file-accessible-directory-p path)
        (let ((files-and-dirs (phw-get-files-and-subdirs path)))
          (phw-tree-node-add-files node path (cdr files-and-dirs)
                                   phw-directories-nodetype-directory
                                   t old-children)
          (if (phw-show-sources-in-directories-buffer-p)
              (phw-tree-node-add-files node path (car files-and-dirs)
                                       phw-directories-nodetype-sourcefile
                                       phw-show-source-file-extension
                                       old-children t))
...
(defphw-tree-buffer-callback phw-directory-expand phw-directories-buffer-name expand nil
  "Expand the clickes directory and add all subnodes if any.
Subnodes can be directories or sources."
  (phw-update-directory-node node))

(defun phw-get-source-paths-from-functions ()
  "Return a list of paths found by querying `phw-source-path-functions'."
  (let ((func phw-source-path-functions)
	(paths nil)
	(rpaths nil))
    (while func
      (setq paths (append paths (funcall (car func)))
	    func (cdr func)))
    (while paths
      (setq rpaths (cons (phw-fix-filename (car paths)) rpaths)
	    paths (cdr paths)))
    rpaths))


(defun phw-update-directories-buffer ()
  "Updates the PHW directories buffer."
  (interactive)
  (unless (or (not phw-minor-mode)
              (not (equal (selected-frame) phw-frame)))
    (phw-exec-in-window phw-directories-buffer-name
      (let* ((node (tree-buffer-get-root))
             (old-children (tree-node->children node))
             (normed-paths (phw-normed-source-paths)))
        (setf (tree-node->children node) nil)
        (dolist (path normed-paths)
          (let* ((dir (car path))
                 (name (cdr path))
                 (remote-dir (phw-remote-path dir))
                 (not-accessible nil))
            (if (or (not remote-dir)
                    (phw-host-accessible-p (nth 1 remote-dir)))
                (if (phw-file-accessible-directory-p dir)
                    (tree-node-add-children
                     node
                     (phw-new-child old-children name
                                    phw-directories-nodetype-sourcepath
                                    dir
                                    nil
                                    (if phw-truncate-long-names
                                        'beginning)))
                  (setq not-accessible t))
              (setq not-accessible t))
            (when not-accessible
              (phw-warning "Source-path element %s is not accessible - ignored!" path))))
        (tree-buffer-update)))
    ))


;; remote-path stuff 
;; (phw-host-accessible-valid-time "phw.sourceforge.net")
;; (phw-host-accessible-cache-get "phw.sourceforge.net" 60)
(defsubst phw-host-accessible-valid-time (host)
  "Get the valid-cache-time of a remote HOST concering its ping-state. If host
doesn't match any regexp of `phw-host-accessible-check-valid-time' then return
60 seconds."
  (or (phw-match-regexp-list host phw-host-accessible-check-valid-time
                             'car 'cdr)
      60))

;; (phw-host-accessible-valid-time "phw.sourceforge.net")
;; (phw-host-accessible-cache-get "phw.sourceforge.net" 60)
(defun phw-host-accessible-p (host)
  "Return not nil if HOST is accessible."
  (let ((value (phw-host-accessible-cache-get
                host (phw-host-accessible-valid-time host))))
    (case value
      (NOT-ACCESSIBLE nil)
      ((nil) ;; not cached or outdated
       (let* ((options (phw-replace-all-occurences (phw-copy-list phw-ping-options)
                                                   "HOST" host))
              (result (equal 0 (apply 'call-process
                                      phw-ping-program
                                      nil nil nil
                                      options))))
         (phw-host-accessible-cache-add host (or result 'NOT-ACCESSIBLE))
         result))
      (otherwise value))))


;; (phw-host-accessible-p "phw.cvs.sourceforge.net")
...
component after that :-separator. Supports tramp, ange-ftp and efs."
  (let ((value (phw-remote-path-cache-get path)))
    (case value
      (NOT-REMOTE nil)
      ((nil)
         ...
         (setq result
               (and dissection
                    (list full-host-user-part
                          (car host/real-path)
                          (cdr host/real-path))))
         (phw-remote-path-cache-add path (or result 'NOT-REMOTE))
         result))
      (otherwise value))))

;;(phw-remote-path "/berndl@phw.sourceforge.net:~")
;;(directory-files "/berndl@phw.sourceforge.net:~/beate")
;; (phw-remote-path "~")

;; empty dirs

(defun phw-check-emptyness-of-dir (dir)
  "Checks if DIR is an empty directory. If empty return not nil otherwise nil."
  (let ((cache-value (phw-directory-empty-cache-get dir))
        (show-sources (phw-show-sources-in-directories-buffer-p)))
    (if (and cache-value
             (equal (cdr cache-value) show-sources))
        (car cache-value)
      (phw-directory-empty-cache-remove dir)
      (let ((entries (and (phw-file-accessible-directory-p dir)
                          (phw-directory-files dir nil nil t)))
              ...
              (catch 'found
                (dolist (e entries)
                  (when (not (member e '("." ".." "CVS")))
                    (setq full-file-name (phw-fix-filename dir e))
                    (if (phw-file-directory-p full-file-name)
                        (throw 'found 'nil)
                      (if (not just-files-means-empty)
                          (throw 'found 'nil)))))
                t))
        ;; now we add this value to the cache
        (phw-directory-empty-cache-add (phw-fix-filename dir)
                                       (cons empty-p show-sources))
        empty-p))))


(defphw-stealthy phw-stealthy-empty-dir-check
...
directory. This function is only for use by `phw-stealthy-updates'!"
  (when (equal 'window-not-visible
               (phw-exec-in-window phw-directories-buffer-name
                 ...
                 (let ((lines-of-buffer (count-lines (point-min) (point-max)))
                       (curr-node nil)
                       (dir-empty-p nil))
                   (phw-exit-on-input 'empty-dir-check-stealthy
                     (save-excursion
                       (while (<= state lines-of-buffer)
                         (phw-throw-on-input 'lines-of-buffer-loop)
                         (phw-goto-line state)
                         (setq curr-node (tree-buffer-get-node-at-point))
                         (when (and ;;(not (= phw-directories-nodetype-sourcefile
                                      ;;      (tree-node->type curr-node)))
                                    (phw-directory-should-prescanned-p
                                     (tree-node->data curr-node))
                                    (phw-file-exists-p
                                     (tree-node->data curr-node)))
                           (setq dir-empty-p
                                 (phw-check-emptyness-of-dir (tree-node->data curr-node)))
...
(defun phw-stealthy-read-only-check--internal (state)
...
`defphw-stealthy'! STATE is the initial state-value the stealthy-function has
when called. Return the new state-value."
  (if (or (not phw-sources-perform-read-only-check)
          (not (or (string= (buffer-name (current-buffer))
                            phw-sources-buffer-name)
                   (and (string= (buffer-name (current-buffer))
                                 phw-directories-buffer-name)
                        (phw-show-sources-in-directories-buffer-p)))))
      'done
    ...
    (let ((lines-of-buffer (count-lines (point-min) (point-max)))
          ...
          (new-name nil)
          (read-only-p nil)
          (node-types-to-check (list phw-sources-nodetype-sourcefile
                                     phw-directories-nodetype-sourcefile)))
      (phw-exit-on-input 'read-only-stealthy
        (save-excursion
          (while (<= state lines-of-buffer)
            (phw-throw-on-input 'lines-of-buffer-loop)
            (phw-goto-line state)
            ...
            (when (and (member (tree-node->type curr-node) node-types-to-check)
                       (phw-sources-read-only-check-p
                        (phw-file-name-directory (tree-node->data curr-node))))
              (setq new-name (tree-node->name curr-node))
              (setq read-only-p
                    (not (phw-file-writable-p (tree-node->data curr-node))))
              (if read-only-p
                  (phw-merge-face-into-text new-name
                                            phw-source-read-only-face))
              ;; we update the node only if we have an empty dir and the node is
              ;; still expandable
              (when read-only-p
...
(defphw-stealthy phw-stealthy-ro-check-in-directories-buf
...
file is writable or not."
  (if (phw-show-sources-in-directories-buffer-p)
      (when (equal 'window-not-visible
                   (phw-exec-in-window phw-directories-buffer-name
                     (setq state
                           (phw-stealthy-read-only-check--internal state))))
        (setq state 'done))
    (setq state 'done)))


;; Test, if our new interrupt mechanism works...
;; (defphw-stealthy phw-stealthy-klaus-test
;;   "test"
;;   (setq state
;;         (if (equal (phw-exit-on-input 'testing
;;                      (let ((inhibit-quit nil)
;;                            (message-log-max nil))
;;                        (while t
;;                          (message "Looping ...")
;;                          (phw-throw-on-input 'user-input)
;;                          )))
;;                    'user-input)
;;             'done
;;           'Klaus)))
...
(defphw-stealthy phw-stealthy-ro-check-in-sources-buf
...
(defphw-advice-set phw-vc-advices 
  "All advices needed for the builtin VC-support of PHW.")
...
(defconst phw-vc-state-icon-alist '((up-to-date . ("vc-up-to-date" "(u)"))
                                    (edited . ("vc-edited" "(e)"))
                                    (added . ("vc-added" "(a)"))
...
`phw-vc-supported-backends'. Each element is a cons-cell where the car is the
...
(defsubst phw-vc-get-image-name-for-vc-state (state)
  "Return the associated image-name for the vc-state STATE."
  (or (nth 0 (cdr (assq state phw-vc-state-icon-alist)))
      "vc-unknown"))
  
(defsubst phw-vc-get-ascii-icon-for-vc-state (state)
  "Return the associated texual icon for the vc-state STATE."
  (or (nth 1 (cdr (assq state phw-vc-state-icon-alist)))
      "(?)"))


(defconst phw-vc-incr-searchpattern-node-prefix
  '("\\(\\(([uempx?])\\)? \\)?" . 2)
...
(defun phw-vc-check-state (file tree-buffer-name vc-state-fcn)
...
new state."
  (let* ((cached-state (phw-vc-cache-get file))
         ...
         (no-need-for-state-check-p
          (and last-check-time
               (or (null last-state) ;; FILE has been checked but is not in VC
                   (not (phw-time-less-p last-check-time
                                         (phw-subseq (nth 5 (phw-file-attributes file))
                                                     0 2))))))
         (result nil))
    (if no-need-for-state-check-p
      ...
      (setq result (and vc-state-fcn
                        ...
                        ;; PHW would eventually call this backend - but this
      ...
      ;; now we map the backend-state to one of the PHW-VC-state-values
      (setq result (or (cdr (assoc result phw-vc-state-mapping)) 'unknown)))
  ...
  (let* ((full-sources-cache (phw-sources-cache-get-full dir))
    ...
    (if filtered-sources-cache
      ...
      (if full-sources-cache
          ...
          (phw-sources-cache-add-full dir
                                      (list (tree-buffer-get-root)
                                            (tree-buffer-displayed-nodes-copy)
                                            (phw-buffer-substring (point-min)
                                                                  (point-max))))))))
...
;; (defun phw-vc-cvs-root-remote-p (root)
...
;; (phw-vc-cvs-root-remote-p ":ext:berndl@cvs.sourceforge.net:/usr/local/root")
;; (phw-vc-cvs-root-remote-p "berndl@cvs.sourceforge.net:/usr/local/root")
;; (phw-vc-cvs-root-remote-p ":ext:cvs.sourceforge.net:/usr/local/root")
;; (phw-vc-cvs-root-remote-p "cvs.sourceforge.net:/usr/local/root")
;; (phw-vc-cvs-root-remote-p ":ext:berndl@cvs.sourceforge.net:C:/local/root")
;; (phw-vc-cvs-root-remote-p "berndl@cvs.sourceforge.net:C:/local/root")
;; (phw-vc-cvs-root-remote-p ":ext:cvs.sourceforge.net:C:/local/root")
;; (phw-vc-cvs-root-remote-p "cvs.sourceforge.net:C:/local/root")
;; The following has to return nil!
;; (phw-vc-cvs-root-remote-p "/local/root")
;; (phw-vc-cvs-root-remote-p ":local:C:/local/root")
;; The following is allowed to return "C" because CVS forbids to use
;; windows-path as root without keyword :local:!
;; (phw-vc-cvs-root-remote-p "C:/local/root")

(defun phw-vc-dir-managed-by-CVS (directory)
  "Return 'CVS if DIRECTORY is managed by CVS. nil if not."
  (and (locate-library "vc-cvs")
       (phw-file-exists-p (concat directory "/CVS/"))
       (require 'vc)
       (require 'vc-cvs)
       'CVS))

(defun phw-vc-dir-managed-by-RCS (directory)
  "Return 'RCS if DIRECTORY is managed by RCS. nil if not."
  (and (locate-library "vc-rcs")
       (phw-file-exists-p (concat directory "/RCS/"))
       (require 'vc)
       (require 'vc-rcs)
       'RCS))

(defun phw-vc-dir-managed-by-SVN (directory)
  ...
  (and (locate-library "vc-svn")
       (let ((admin-dir (cond ((and (memq system-type '(cygwin windows-nt ms-dos))
                               ...
                               "_svn")
                              (t ".svn"))))
         (phw-file-exists-p (concat directory "/" admin-dir "/")))
       (require 'vc)
       (require 'vc-svn)
       'SVN))

(defun phw-vc-dir-managed-by-SCCS (directory)
  "Return 'SCCS if DIRECTORY is managed by SCCS. nil if not."
  (and (locate-library "vc-sccs")
       (or (phw-file-exists-p (concat directory "/SCCS/"))
           ;; Remote SCCS project
           (let ((proj-dir (getenv "PROJECTDIR")))
             (if proj-dir
                 (phw-file-exists-p (concat proj-dir "/SCCS")))))
       (require 'vc)
       (require 'vc-sccs)
       'SCCS))
...
;; `phw-vc-supported-backends'.

(defun phw-vc-dir-managed-by-HG (directory)
...
(defun phw-vc-dir-managed-by-GIT (directory)
...
;; (defun phw-vc-dir-managed-by-GIT (directory)
;;   (let* ((sourcedir (phw-fix-filename (file-truename directory)))
;;          (gitdir (concat sourcedir "/.git/")))
;;     (if (and (phw-file-exists-p gitdir)
;;              (locate-library "vc-git"))
;;         'GIT
;;       (if (equal sourcedir (phw-fix-filename "/"))
;;           nil
;;         (phw-vc-dir-managed-by-GIT (concat sourcedir "/../"))))))

;;(phw-vc-dir-managed-by-GIT default-directory)

;; Monotone support

(defun phw-vc-dir-managed-by-MTN (directory)
...
;; phw-vc-dir-managed-by-CC and phw-vc-check-CC-state to
;; `phw-vc-supported-backends'
...
(defun phw-vc-dir-managed-by-CC (directory)
...
(defun phw-vc-check-CC-state (file)
...
(defphw-advice clearcase-sync-from-disk after phw-vc-advices
  "Ensures that the PHW-cache is reset and the entry for the currently
checked-in/out or added file is cleared. Does nothing if the function
`phw-vc-dir-managed-by-CC' is not contained in `phw-vc-supported-backends'!"
  (when (assoc 'phw-vc-dir-managed-by-CC phw-vc-supported-backends)
    (phw-vc-cache-remove (phw-fix-filename (ad-get-arg 0)))
    (phw-vc-reset-vc-stealthy-checks)))



;; This function should work for all backends supported by vc
(defun phw-vc-state (file)
...
`phw-vc-recompute-state'."
  (and (fboundp 'vc-file-clearprops) (vc-file-clearprops file))
  (vc-state file))

(defalias 'phw-vc-recompute-state 'vc-recompute-state)

(defun phw-vc-get-state-fcn-for-dir (directory)
...
each element of `phw-vc-supported-backends' and return the cdr of the first
...
checked for VC-states!"
  (let* ((norm-dir (phw-fix-filename directory))
         (cache-val (phw-vc-cache-get norm-dir)))
    (case cache-val
      (NO-VC nil)
      ((nil) ;; not cached or outdated
       (let ((vc-backend-fcn
              (catch 'found
                (dolist (elem phw-vc-supported-backends)
         ...
         (phw-vc-cache-add-dir norm-dir (or vc-backend-fcn 'NO-VC))
         vc-backend-fcn))
      (otherwise cache-val))))

(defalias 'phw-vc-managed-dir-p 'phw-vc-get-state-fcn-for-dir)

(defun phw-vc-generate-node-name (name state)
...
used otherwise an ascii-icon. The text-property 'phw-vc-ascii-icon-length is
...
of NAME."
  (let* ((ascii-icon (phw-vc-get-ascii-icon-for-vc-state state))
    ...
    (put-text-property 0 (length node-name)
                       'phw-vc-ascii-icon-length (1+ (length ascii-icon))
                       node-name)
    (phw-generate-node-name node-name (1+ (length ascii-icon))
                            (phw-vc-get-image-name-for-vc-state state)
                            ;; even in the history- or the directories-buffers
                            ;; we use the icons of the sources-buffer because
                            ;; they are the same!
                            phw-sources-buffer-name)))

;; (insert (phw-vc-generate-node-name "test-name" 'needs-merge))

(defun phw-get-sourcename-of-nodename (nodename)
  "Extract the name of the source from the node-name NODENAME.
This is for nodenames generated by `phw-vc-generate-node-name' or
`phw-generate-node-name' and it return exactly this part of NODENAME without
all the preceding stuff added by one of these functions."
  (let* ((vc-ascii-icon-length (get-text-property
                                0
                                'phw-vc-ascii-icon-length
                                nodename))
...
(defun phw-stealthy-vc-check--dir/history (state)
  "Check for all sourcefile-nodes either in the directories- or the
history-buffer the VC-state. This function does the real job and is is only
for use by a stealthy function defined with `defphw-stealthy'! STATE is the
initial state-value the stealthy-function has when called. Return the new
state-value."
  (if (or (not phw-vc-needed-vc-package-available-p)
          (not phw-vc-enable-support))
      'done
    (if (not (or (string= (buffer-name (current-buffer))
      ...
      (let ((lines-of-buffer (count-lines (point-min) (point-max)))
            ...
            (vc-state-fcn nil)
            (new-state nil)
            (node-types-to-check (list phw-history-nodetype-filebuffer
                                       phw-history-nodetype-indirect-filebuffer
                                       phw-directories-nodetype-sourcefile)))
        (phw-exit-on-input 'vc-check-dir-hist-stealthy
          (save-excursion
            (while (<= state lines-of-buffer)
              (phw-throw-on-input 'lines-of-buffer-loop)
              (phw-goto-line state)
              ...
              (when (member (tree-node->type curr-node) node-types-to-check)
                (setq curr-dir (phw-file-name-directory
                                (phw-source-get-filename (tree-node->data curr-node))))
                (when (and (phw-vc-directory-should-be-checked-p curr-dir)
                           (phw-file-exists-p (phw-source-get-filename
                                               (tree-node->data curr-node))))
                  (setq vc-state-fcn (phw-vc-get-state-fcn-for-dir curr-dir))
                  (when vc-state-fcn ;; file is under VC-control
                    (setq new-name (tree-node->name curr-node))
                    (setq new-state
                          (phw-vc-check-state (phw-source-get-filename
                                               (tree-node->data curr-node))
                                              (buffer-name (current-buffer))
                                              vc-state-fcn))
                    ;; we update the node only if the state has changed 
                    (when (not (equal 'unchanged new-state))
                      (setq new-name (phw-vc-generate-node-name new-name new-state))
                      (tree-buffer-update-node
                       nil new-name
...
(defun phw-stealthy-vc-check--sources (state)
...
defined with `defphw-stealthy'! STATE is the initial state-value the
stealthy-function has when called. Return the new state-value."
  (if (or (not phw-vc-needed-vc-package-available-p)
          (not (phw-vc-directory-should-be-checked-p phw-path-selected-directory)))
      'done
    (let ((vc-state-fcn (phw-vc-get-state-fcn-for-dir phw-path-selected-directory)))
      (if (null vc-state-fcn)
        ...
        (let ((lines-of-buffer (count-lines (point-min) (point-max)))
          ...
          (phw-exit-on-input 'vc-check-sources-stealthy
            (save-excursion
              (while (<= state lines-of-buffer)
                (phw-throw-on-input 'lines-of-buffer-loop)
                (phw-goto-line state)
                (setq curr-node (tree-buffer-get-node-at-point))
                (if (phw-file-exists-p (tree-node->data curr-node))
                    (progn
                      (setq new-name (tree-node->name curr-node))
                      (setq new-state
                            (phw-vc-check-state (tree-node->data curr-node)
                                                (buffer-name (current-buffer))
                                                vc-state-fcn))
                      ;; we update the node only if the state has changed 
                      (when (not (equal 'unchanged new-state))
                        ...
                        (or update-performed-for-dir
                            (setq update-performed-for-dir phw-path-selected-directory))
                        (tree-buffer-update-node
                         curr-node new-name
                  ...
                  (phw-files-and-subdirs-cache-remove phw-path-selected-directory)
                  ...
                  (if (= lines-of-buffer 1)
                      (phw-directory-empty-cache-remove phw-path-selected-directory))
          ...
          (when update-performed-for-dir
            (phw-vc-update-sources-cache update-performed-for-dir))
          (if (> state lines-of-buffer)
              (setq state 'done)))
        state))))

(defphw-stealthy phw-stealthy-vc-check-in-history-buf
  "Check for all entries in the history-buffer their VC-state and
display an appropriate icon in front of the item."
  (when (equal 'window-not-visible
               (phw-exec-in-window phw-history-buffer-name
                 (setq state
                       (phw-stealthy-vc-check--dir/history state))))
    (setq state 'done)))


(defphw-stealthy phw-stealthy-vc-check-in-sources-buf
  "Check for all sourcefile-nodes in the sources-buffer their VC-state and
display an appropriate icon in front of the file."
  (when (equal 'window-not-visible
               (phw-exec-in-window phw-sources-buffer-name
                 (setq state
                       (phw-stealthy-vc-check--sources state))))
    (setq state 'done)))

(defphw-stealthy phw-stealthy-vc-check-in-directories-buf
...
(defun phw-vc-reset-vc-stealthy-checks ()
  ...
  ;; read-only-state!
  (phw-stealthy-function-state-init 'phw-stealthy-ro-check-in-directories-buf)
  (phw-stealthy-function-state-init 'phw-stealthy-ro-check-in-sources-buf)
  (phw-stealthy-function-state-init 'phw-stealthy-vc-check-in-sources-buf)
  (phw-stealthy-function-state-init 'phw-stealthy-vc-check-in-directories-buf)
  (phw-stealthy-function-state-init 'phw-stealthy-vc-check-in-history-buf)
...
(defvar phw-checkedin-file nil
  "Stored the filename of the most recent checked-in file. Is only set by the
after-advice of `vc-checkin' and `phw-vc-checkin-hook' \(resets it to nil).
Evaluated only by `phw-vc-checkin-hook'.

This is the communication-channel between `vc-checkin' and
`phw-vc-checkin-hook' so this hook-function gets the filename of the
checked-in file.")

(defphw-advice vc-checkin after phw-vc-advices
  "Simply stores the filename of the checked-in file in `phw-checkedin-file'
so it is available in the `vc-checkin-hook'."
  (ignore-errors
    (setq phw-checkedin-file (phw-fix-filename (ad-get-arg 0)))))

(defun phw-vc-checkin-hook ()
  "Ensures that the PHW-cache is reset and the entry for the most recent
checkedin file is cleared. Uses `phw-checkedin-file' as last checked-in file."
  (when phw-checkedin-file
    (phw-vc-cache-remove phw-checkedin-file)
    (phw-vc-reset-vc-stealthy-checks)
    (setq phw-checkedin-file nil)))

(defun phw-vc-after-revert-hook ()
  "Ensures that the PHW-cache is reset and the entry for the currently
reverted file-buffer is cleared."
  (let ((file (ignore-errors (phw-fix-filename (phw-buffer-file-name)))))
    (when (and file (phw-file-exists-p file))
      (phw-vc-cache-remove file)
      (phw-vc-reset-vc-stealthy-checks))))

(defun phw-vc-enable-internals (arg)
  "Enable or disable \(if ARG < 0) all settings needed by the VC-support."
  (if (< arg 0)
      (progn
        (remove-hook 'after-revert-hook 'phw-vc-after-revert-hook)
        (remove-hook 'write-file-hooks 'phw-vc-reset-vc-stealthy-checks)
        (remove-hook 'vc-checkin-hook 'phw-vc-checkin-hook)
        (phw-disable-advices 'phw-vc-advices))
    (add-hook 'after-revert-hook 'phw-vc-after-revert-hook)
    (add-hook 'write-file-hooks 'phw-vc-reset-vc-stealthy-checks)
    (add-hook 'vc-checkin-hook 'phw-vc-checkin-hook)
    (phw-enable-advices 'phw-vc-advices)))

;; -- end of vc-support ---------------

;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: Maybe we should run directly
;; `phw-stealthy-updates' from within this after-update-hooks?!

(defun phw-stealth-tasks-after-directories-update ()
  "After update hook for the directories-buffer. Runs directly after
performing a `tree-buffer-update' for this buffer."
  (phw-stealthy-function-state-init 'phw-stealthy-empty-dir-check)
  (phw-stealthy-function-state-init 'phw-stealthy-ro-check-in-directories-buf)
  (phw-stealthy-function-state-init 'phw-stealthy-vc-check-in-directories-buf)
  )

(defun phw-stealth-tasks-after-sources-update ()
...
(defun phw-stealth-tasks-after-history-update ()
...
(defun phw-tree-node-add-files
  ...
  (let* ((no-vc-state-display
          ...
          (and (equal (buffer-name) phw-directories-buffer-name)
               (= type phw-directories-nodetype-directory)))
         (dir-managed-by-vc (if (or no-vc-state-display
                                    (not (phw-vc-directory-should-be-checked-p path)))
                                nil
                              (phw-vc-managed-dir-p path))))
    (dolist (file files)
      (let* ((filename (phw-fix-filename path file))
             (file-1 (if include-extension
                         file
                       (phw-file-name-sans-extension file))))
        (tree-node-add-children
         node
         (phw-new-child
          old-children
          (if no-vc-state-display
            ...
            (if dir-managed-by-vc
                (phw-vc-generate-node-name file-1
                                           (nth 0 (phw-vc-cache-get filename)))
              (phw-generate-node-name file-1 -1 "leaf"
                                      phw-sources-buffer-name)))
          type filename
          (or not-expandable
              (= type phw-directories-nodetype-sourcefile)
              ;; The empty-dir-check is performed stealthy
              nil ;;(phw-check-emptyness-of-dir filename)
              )
          (if phw-truncate-long-names 'end)))))))

(defun phw-new-child (old-children name type data
...
(defun phw-add-source-path (&optional dir alias no-prompt-for-future-session)
  "Add a directory to the `phw-source-path'."
  (interactive)
  ;; we must manually cut a filename because we must not add filenames to
  ;; `phw-source-path'!
  (let* ((use-dialog-box nil)
         (my-dir (phw-fix-filename
                  (or dir
                      (phw-file-name-directory (read-file-name "Add source path: ")))
                  nil t))
         (my-alias (or alias
                       (read-string (format "Alias for \"%s\" (empty = no alias): "
                                            my-dir)))))
    (setq phw-source-path (append phw-source-path
    ...
    (if (and (not no-prompt-for-future-session)
        ...
        (phw-customize-save-variable 'phw-source-path phw-source-path)
      (customize-set-variable 'phw-source-path phw-source-path))))

(tree-buffer-defpopup-command phw-add-source-path-node
  "Runs `phw-add-source-path' from popup."
  (call-interactively 'phw-add-source-path))


(tree-buffer-defpopup-command phw-node-to-source-path
  "Add this node to the source-path."
  (phw-add-source-path (tree-node->data node)))


(defun phw-delete-s (child children sources)
  (when children
    (if (eq child (car children))
	(cdr sources)
      (cons (car sources) (phw-delete-s child (cdr children) (cdr sources))))))


(tree-buffer-defpopup-command phw-delete-source-path
  "Delete this source-path via popup."
  (let ((path (tree-node->data node)))
    (when (phw-confirm (concat "Really delete source-path " path "?"))
      (setq phw-source-path (phw-delete-s
...
(defun phw-remove-dir-from-caches (dir)
  "Remove DIR from the caches SUBDIR, EMPTY-DIR and SOURCES."
  (phw-files-and-subdirs-cache-remove dir)
  (phw-directory-empty-cache-remove dir)
  (phw-sources-cache-remove dir))

(defun phw-directory-update-speedbar (dir)
  "Update the integrated speedbar if necessary."
  (and (phw-speedbar-active-p)
       ;; depending on the value of `phw-directory-update-speedbar' we have to
       ;; check if it is senseful to update the speedbar.
       (or (equal phw-directories-update-speedbar t)
           (and (equal phw-directories-update-speedbar 'auto)
                (not (or (get-buffer-window phw-sources-buffer-name phw-frame)
                         (member phw-layout-name
                                 phw-show-sources-in-directories-buffer))))
           (and (not (equal phw-directories-update-speedbar 'auto))
                (functionp phw-directories-update-speedbar)
                (funcall phw-directories-update-speedbar dir)))
       (phw-speedbar-update-contents)))

(defphw-tree-buffer-callback phw-directory-or-source-clicked phw-directories-buffer-name select nil
...
Directory- and sources nodes are handled appropriately."
  (if (= 3 (tree-node->type node))
    ...
    (phw-update-directory-node node)
    (if shift-mode
        (phw-mouse-over-directory-node node nil nil 'force))
    (if (or (= phw-directories-nodetype-directory
               (tree-node->type node))
            (= phw-directories-nodetype-sourcepath
               (tree-node->type node)))
        (progn
          ;; prevent from automatically hiding the phw-windows when meta-mode
          ;; is not nil
          ...
          (if (= 2 phw-button)
            ...
            ;; subdirs)
            (when shift-mode
              (phw-remove-dir-from-caches (tree-node->data node))
              (phw-directory-empty-cache-remove-all (tree-node->data node))
              ;; a powerclick should remove all vc-caches of contained files
              (phw-vc-cache-remove-files-of-dir (tree-node->data node))
              )
            ...
            ;; buffer does not mean that there are no other phw-buffers visible!!!

            (when (and (phw-buffer-is-maximized-p phw-directories-buffer-name)
                       (not (phw-show-sources-in-directories-buffer-p))
                       (phw-buffer-is-phw-buffer-of-current-layout-p
                        phw-sources-buffer-name))
              (if (phw-member-of-symbol/value-list
                   phw-directories-buffer-name
                   phw-maximize-next-after-maximized-select)
                  (progn
                    (phw-maximize-phw-buffer phw-sources-buffer-name)
                    (phw-window-select phw-sources-buffer-name))
                (phw-undo-maximize-phw-buffer t)))
            
            (phw-set-selected-directory (tree-node->data node) shift-mode)
            ;; if we have running an integrated speedbar we must update the
            ;; speedbar 
            (phw-directory-update-speedbar (tree-node->data node))))
      
      (phw-source-item-clicked node phw-button edit-window-nr shift-mode meta-mode)
      )))

(defun phw-source-item-clicked (node phw-button edit-window-nr shift-mode meta-mode)
  ...
  ;; phw-directory-or-source-clicked
  (when (and (phw-buffer-is-maximized-p (buffer-name))
             (phw-buffer-is-phw-buffer-of-current-layout-p
    ...
    (if (phw-member-of-symbol/value-list
         ...
         phw-maximize-next-after-maximized-select)
        (progn
          (phw-maximize-phw-buffer phw-methods-buffer-name)
          (phw-window-select phw-methods-buffer-name))
      (phw-undo-maximize-phw-buffer t)))
  (phw-set-selected-source (tree-node->data node)
                           (phw-combine-phw-button/edit-win-nr phw-button edit-window-nr)
			   shift-mode)
  )

(defphw-tree-buffer-callback phw-source-clicked phw-sources-buffer-name select nil
  "Does all necessary when a user clicks onto a node in the sources-buffer."
  (if shift-mode
      (phw-mouse-over-source-node node nil nil 'force))
  (phw-source-item-clicked node phw-button edit-window-nr shift-mode meta-mode))

(defphw-tree-buffer-callback phw-history-clicked phw-history-buffer-name select nil
  "Does all necessary when a user clicks onto a node in the history-buffer."
  (if shift-mode
      (phw-mouse-over-history-node node nil nil 'force))
  (if (= (tree-node->type node) phw-history-nodetype-bucket)
...
(defun phw-expand-directory-nodes (level)
  "Set the expand level of the nodes in the PHW-directories-buffer.
For argument LEVEL see `phw-expand-methods-nodes'.
...
can last a long time - depending of machine- and disk-performance."
  ...
  (phw-exec-in-window phw-directories-buffer-name
    ...
    (tree-buffer-update))
  (phw-basic-buffer-sync 'force))


(defun phw-get-file-info-text (file)
  "Return a file-info string for a file in the PHW sources buffer"
  (let ((attrs (phw-file-attributes file)))
    (format "%s %8s %4d %10d %s %s"
	    ...
	    (format-time-string "%Y/%m/%d %H:%M" (nth 5 attrs))
            (if (equal (cdr phw-sources-show-node-info) 'file-info-full)
                file
              (phw-file-name-nondirectory file)))
    ))


(defun phw-mouse-over-directory-node (node &optional window no-message click-force)
...
`phw-directories-show-node-info'. NODE is the node for which help text should
be displayed, WINDOW is the related window, NO-MESSAGE defines if the
help-text should be printed here."
  (if (= (tree-node->type node) phw-directories-nodetype-sourcefile)
      (phw-mouse-over-source-node node window no-message click-force)
    (if (not (= (tree-node->type node) 3))
        (let ((str (when (or click-force
                             (phw-show-minibuffer-info node window
                                                       (car phw-directories-show-node-info))
                             (and (not (equal (car phw-directories-show-node-info)
                                              'never))
                                  (not (phw-string= (tree-node->data node)
                                                    (tree-node->name node)))
                                  (eq (tree-node->parent node)
                                      (tree-buffer-get-root))))
                     (if (equal (cdr phw-directories-show-node-info) 'name)
                         (tree-node->name node)
                       (tree-node->data node)))))
          (prog1 str
            (unless no-message
...
(defun phw-mouse-over-source-node (node &optional window no-message click-force)
  "Displays help text if mouse moves over a node in the sources buffer or if
CLICK-FORCE is not nil and always with regards to the settings in
`phw-sources-show-node-info'. NODE is the node for which help
...
the help-text should be printed here."
  (let ((str (ignore-errors ;; For buffers that hasn't been saved yet
               (when (or click-force
                         (phw-show-minibuffer-info node window
                                                   (car phw-sources-show-node-info)))
                 (if (equal (cdr phw-sources-show-node-info) 'name)
                     (tree-node->name node)
                   (phw-get-file-info-text (tree-node->data node)))))))
    (prog1 str
      (unless no-message
        (phw-nolog-message str)))))


(defun phw-mouse-over-history-node (node &optional window no-message click-force)
  "Displays help text if mouse moves over a node in the history buffer or if
CLICK-FORCE is not nil and always with regards to the settings in
`phw-history-show-node-info'. NODE is the node for which help
text should be displayed, WINDOW is the related window, NO-MESSAGE defines if
the help-text should be printed here."
  (let ((str (ignore-errors ;; For buffers that hasn't been saved yet
               (when (or click-force
                         (phw-show-minibuffer-info node window
                                                   (car phw-history-show-node-info)))
                 (if (= (tree-node->type node) phw-history-nodetype-bucket)
                     (tree-node->data node)
                   (case (cdr phw-history-show-node-info)
                     (name (tree-node->name node))
                     (path (phw-source-get-filename (tree-node->data node)))
                     (name-path (format "%s (%s)" (tree-node->name node)
                                        (phw-source-get-filename (tree-node->data node))))))))))
    (prog1 str
      (unless no-message
        (phw-nolog-message str)))))
...
;; needs methods
(tree-buffer-defpopup-command phw-create-source
  "Creates a new sourcefile in current directory."
  (let* ((use-dialog-box nil)
         (dir (phw-fix-filename
               (funcall (if (phw-file-directory-p (tree-node->data node))
                            'identity
                          'phw-file-name-directory)
                        (tree-node->data node))))
         (filename (phw-file-name-nondirectory
                    (read-file-name "Source name: " (concat dir "/")))))
    (phw-select-edit-window)
    (if (save-match-data (string-match "\\.java$" filename))
        (phw-jde-gen-class-buffer dir filename)
      (find-file (concat dir "/" filename)))
    (when (= (point-min) (point-max))
      (set-buffer-modified-p t)
      (let ((phw-auto-update-methods-after-save nil))
        (save-buffer))
      (phw-rebuild-methods-buffer-with-tagcache nil nil t))
    (phw-remove-dir-from-caches dir)
    (phw-set-selected-directory dir t)
    (phw-basic-buffer-sync)))


(defun phw-grep-directory-internal (node find)
  (phw-select-edit-window)
  (let* ((node-data-file (phw-source-get-filename (tree-node->data node)))
         (default-directory (concat (phw-fix-filename
                                     (if (phw-file-directory-p node-data-file)
                                         node-data-file
                                       (phw-file-name-directory node-data-file)))
                                    (phw-directory-sep-string node-data-file))))
    (call-interactively (if find
                            (or (and (fboundp phw-grep-recursive-function)
                                     phw-grep-recursive-function)
                                'grep-find)
                          (or (and (fboundp phw-grep-function)
                                   phw-grep-function)
                              'grep)))))


(tree-buffer-defpopup-command phw-grep-find-directory
  "Runs grep-find for current directory."
  (phw-grep-directory-internal node t))


(tree-buffer-defpopup-command phw-grep-directory
  "Runs grep for current directory."
  (phw-grep-directory-internal node nil))


(defun phw-create-directory (parent-node)
  (make-directory (concat (tree-node->data parent-node) "/"
                          (read-from-minibuffer "Directory name: ")))
  (phw-update-directory-node parent-node)
  (tree-buffer-update))


(tree-buffer-defpopup-command phw-delete-directory
  "Delete current directory."
  (let ((dir (tree-node->data node)))
    (when (phw-confirm (concat "Really delete directory" dir "? "))
      (delete-directory (tree-node->data node))
      (phw-update-directory-node (tree-node->parent node))
      (tree-buffer-update))))


(defun phw-dired-directory-internal (node &optional other)
  (phw-select-edit-window)
  (let* ((node-data-file (phw-source-get-filename (tree-node->data node)))
         (dir (phw-fix-filename
               (funcall (if (phw-file-directory-p node-data-file)
                            'identity
                          'phw-file-name-directory)
                        node-data-file))))
    (funcall (if other
...
(tree-buffer-defpopup-command phw-dired-directory
  "Run dired for this directory."
  (phw-dired-directory-internal node))


(tree-buffer-defpopup-command phw-dired-directory-other-window
  "Run dired for this directory in the other window."
  (phw-dired-directory-internal node 'other))


(defun phw-dir-run-cvs-op (node op op-arg-list)
  (let ((dir (tree-node->data node)))
    (funcall op dir op-arg-list)))


(tree-buffer-defpopup-command phw-dir-popup-cvs-status
  "Check status of directory \(and below) in pcl-cvs mode."
  (phw-dir-run-cvs-op node 'cvs-status '("-v")))


(tree-buffer-defpopup-command phw-dir-popup-cvs-examine
  "Examine directory \(and below) in pcl-cvs mode."
  (phw-dir-run-cvs-op node 'cvs-examine '("-d" "-P")))


(tree-buffer-defpopup-command phw-dir-popup-cvs-update
  "Update directory \(and below) in pcl-cvs mode."
  (phw-dir-run-cvs-op node 'cvs-update '("-d" "-P")))


(defvar phw-common-directories-menu nil)


(setq phw-common-directories-menu
      '(("Grep"
         (phw-grep-directory "Grep Directory")
         (phw-grep-find-directory "Grep Directory recursive"))
        ;;("---")
        ("Dired"
         (phw-dired-directory "Open in Dired")
         (phw-dired-directory-other-window "Open in Dired other window"))
        ("---")
	(phw-create-source "Create Sourcefile")
	(phw-create-directory "Create Child Directory")
	(phw-delete-directory "Delete Directory")
        ("---")
	(phw-add-source-path-node "Add Source Path")))


(defvar phw-directories-menu nil
  "Built-in menu for the directories-buffer for directories which are not a
source-path of `phw-source-path'.")
(setq phw-directories-menu
      (append
       phw-common-directories-menu
       '((phw-node-to-source-path "Make This a Source Path")
         ("---")
         (phw-maximize-phw-window-menu-wrapper "Maximize window"))))


(defvar phw-directories-menu-title-creator
  (function (lambda (node)
              (let ((node-type (tree-node->type node))
                    (node-name (tree-node->name node)))
                    (cond ((= node-type phw-directories-nodetype-directory)
                           (format "%s  (Directory)" node-name))
                          ((= node-type phw-directories-nodetype-sourcefile)
                           (format "%s  (File)" node-name))
                          ((= node-type phw-directories-nodetype-sourcepath)
...
(tree-buffer-defpopup-command phw-open-source-in-editwin1
  "Open current source-file the 1. edit-window."
  ;; We can use `phw-source-clicked' for history-buffer too because shift-mode
  ;; is nil.
  (phw-source-clicked node 3 1 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin2
  "Open current source-file the 2. edit-window."
  (phw-source-clicked node 3 2 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin3
  "Open current source-file the 3. edit-window."
  (phw-source-clicked node 3 3 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin4
  "Open current source-file the 4. edit-window."
  (phw-source-clicked node 3 4 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin5
  "Open current source-file the 5. edit-window."
  (phw-source-clicked node 3 5 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin6
  "Open current source-file the 6. edit-window."
  (phw-source-clicked node 3 6 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin7
  "Open current source-file the 7. edit-window."
  (phw-source-clicked node 3 7 nil nil))
(tree-buffer-defpopup-command phw-open-source-in-editwin8
  "Open current source-file the 8. edit-window."
  (phw-source-clicked node 3 8 nil nil))

(defun phw-dir/source/hist-menu-editwin-entries ()
  ...
  (let ((edit-win-list (phw-canonical-edit-windows-list))
    ...
    (when (> (length edit-win-list) 1)
      (dotimes (i (min 8 (length edit-win-list)))
        (setq result
              (append result
                      (list (list (intern (format "phw-open-source-in-editwin%d" (1+ i)))
...
(defun phw-directories-menu-creator (tree-buffer-name node)
  ...
  (let ((dyn-user-extension
         (and (functionp phw-directories-menu-user-extension-function)
              (funcall phw-directories-menu-user-extension-function
                       tree-buffer-name node)))
        (dyn-builtin-extension (phw-dir/source/hist-menu-editwin-entries)))
    (list (cons phw-directories-nodetype-directory
                (funcall (or phw-directories-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-directories-menu-user-extension
                                 phw-directories-menu)))
          (cons phw-directories-nodetype-sourcefile
                (funcall (or phw-sources-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-sources-menu-user-extension
                                 phw-sources-menu
                                 dyn-builtin-extension)))
          (cons phw-directories-nodetype-sourcepath
                (funcall (or phw-directories-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-directories-menu-user-extension
                                 phw-source-path-menu))))))

;; source-path tokens

(defvar phw-source-path-menu nil
  "Built-in menu for the directories-buffer for directories which are elements of
`phw-source-path'.")
(setq phw-source-path-menu
      (append
       phw-common-directories-menu
       '((phw-delete-source-path "Delete Source Path")
         ("---")
         (phw-maximize-phw-window-menu-wrapper "Maximize window"))))


(tree-buffer-defpopup-command phw-delete-source
  "Deletes current sourcefile."
  (let* ((file (phw-source-get-filename (tree-node->data node)))
         (dir (phw-fix-filename (phw-file-name-directory file))))
    (when (phw-confirm (concat "Really delete " (phw-file-name-nondirectory file) "? "))
      ...
      (phw-delete-file file)
      (phw-remove-dir-from-caches dir)
      (phw-set-selected-directory dir t))))


(tree-buffer-defpopup-command phw-file-popup-ediff-revision
  "Diff file against repository with ediff."
  (let ((file (phw-source-get-filename (tree-node->data node))))
    (ediff-revision file)))


(tree-buffer-defpopup-command phw-file-popup-vc-next-action
  "Checkin/out file."
  (phw-display-source (tree-node->data node) nil)
  (call-interactively 'vc-next-action))


(tree-buffer-defpopup-command phw-file-popup-vc-log
  "Print revision history of file."
  (phw-display-source (tree-node->data node) nil)
  (call-interactively 'vc-print-log))


(tree-buffer-defpopup-command phw-file-popup-vc-annotate
  "Annotate file"
  (phw-display-source (tree-node->data node) nil)
  (call-interactively 'vc-annotate))

(tree-buffer-defpopup-command phw-file-popup-vc-diff
  "Diff file against last version in repository."
  (phw-display-source (tree-node->data node) nil)
  (call-interactively 'vc-diff))

(tree-buffer-defpopup-command phw-file-popup-vc-refresh-file
  "Recompute the VC-state for this file."
  (let ((file (phw-source-get-filename (tree-node->data node))))
    (phw-vc-cache-remove file)
    (phw-vc-reset-vc-stealthy-checks)))

(tree-buffer-defpopup-command phw-file-popup-vc-refresh-dir
  "Recompute the VC-state-values for the whole directory."
  (let ((dir (phw-fix-filename
              (phw-file-name-directory
               (phw-source-get-filename (tree-node->data node))))))
    (phw-vc-cache-remove-files-of-dir dir)
    (phw-vc-reset-vc-stealthy-checks)))

(defvar phw-sources-menu nil
  "Built-in menu for the sources-buffer.")

(setq phw-sources-menu
      '(("Grep"
         (phw-grep-directory "Grep Directory")
         (phw-grep-find-directory "Grep Directory recursive"))
        ("Dired"
         (phw-dired-directory "Open Dir in Dired")
         (phw-dired-directory-other-window "Open Dir in Dired other window"))
        ("Filter"
         (phw-popup-sources-filter-by-ext "Filter by extension")
         (phw-popup-sources-filter-by-regexp "Filter by a regexp")
         (phw-popup-sources-filter-none "No filter"))
        ("---")        
	(phw-create-source "Create Sourcefile")
        (phw-delete-source "Delete Sourcefile")
        ("---")
        (phw-maximize-phw-window-menu-wrapper "Maximize window")))


(defvar phw-sources-menu-title-creator
  (function (lambda (node)
              (phw-file-name-nondirectory (tree-node->data node))))
  "The menu-title for the sources menu. See
`phw-directories-menu-title-creator'.")

(defun phw-sources-menu-creator (tree-buffer-name node)
  "Creates the popup-menus for the sources-buffer."
  (setq phw-layout-prevent-handle-phw-window-selection t)
  (let ((dyn-user-extension
         (and (functionp phw-sources-menu-user-extension-function)
              (funcall phw-sources-menu-user-extension-function
                       tree-buffer-name node)))
        (dyn-builtin-extension (phw-dir/source/hist-menu-editwin-entries)))
    (list (cons phw-sources-nodetype-sourcefile
                (funcall (or phw-sources-menu-sorter
                             'identity)
                         (append dyn-user-extension
                                 phw-sources-menu-user-extension
...
(tree-buffer-defpopup-command phw-popup-history-filter-by-ext
  "Filter history entries by extension."
  (let ((ext-str (read-string "Insert the filter-extension without leading dot: "
                              (and node
                                   (phw-file-name-extension
                                    (phw-source-get-filename (tree-node->data node)))))))
    (phw-history-filter-by-ext ext-str)))

(defun phw-history-filter-by-regexp (&optional regexp filter-display)
  "Filter history entries by REGEXP. If the first optional argument REGEXP is
nil then it asks for a regexp. If second argument FILTER-DISPLAY is not nil
then it is displayed in the modeline of the history-buffer for current
...
(tree-buffer-defpopup-command phw-popup-history-filter-by-regexp
  "Filter history entries by regexp by popup."
  (phw-history-filter-by-regexp))
  
(tree-buffer-defpopup-command phw-popup-history-filter-all-existing
  "No history filter, i.e. add all existing file-buffers to the history."
  (phw-add-all-buffers-to-history))
  
(tree-buffer-defpopup-command phw-file-popup-vc-refresh-all-files
  "Recompute the VC-state for the whole history."
  (when (equal (buffer-name) phw-history-buffer-name)
...
(tree-buffer-defpopup-command phw-popup-history-bucketize-by-dir
  "Bucketize the history by directory.

It changes temporaly the option `phw-history-make-buckets' to the value
'directory."
  (customize-set-variable 'phw-history-make-buckets 'directory))

(tree-buffer-defpopup-command phw-popup-history-bucketize-by-dir-with-sp
  "Bucketize the history by directory with source-path substitution.

It changes temporaly the option `phw-history-make-buckets' to the value
'directory-with-source-path."
  (customize-set-variable 'phw-history-make-buckets 'directory-with-source-path))

(tree-buffer-defpopup-command phw-popup-history-bucketize-by-mode
  "Bucketize the history by major-mode.

It changes temporaly the option `phw-history-make-buckets' to the value
'mode."
  (customize-set-variable 'phw-history-make-buckets 'mode))

(tree-buffer-defpopup-command phw-popup-history-bucketize-by-ext
  "Bucketize the history by file-extension.

It changes temporaly the option `phw-history-make-buckets' to the value
'extension."
  (customize-set-variable 'phw-history-make-buckets 'extension))

(tree-buffer-defpopup-command phw-popup-history-bucketize-by-reg
  "Bucketize the history by regular expressions.

It opens the customize-buffer for the option `phw-history-make-buckets' so the
regular expressions can be inserted."
  (customize-option 'phw-history-make-buckets))

(tree-buffer-defpopup-command phw-popup-history-bucketize-never
  "Remove all bucketizing in the history - display it \"flat\".

It changes temporaly the option `phw-history-make-buckets' to the value
'never."
  (customize-set-variable 'phw-history-make-buckets 'never))

(defun phw-history-filter ()
...
- No filter: This means to display an entry for all currently living
  file-buffers."
  (interactive)
  (let ((choice (phw-query-string "Filter history by:"
                                  '("extension" "regexp" "no filter"))))
    (cond ((phw-string= choice "extension")
           (phw-history-filter-by-ext
            (read-string "Insert the filter-extension without leading dot: ")))
          ((phw-string= choice "regexp")
           (phw-history-filter-by-regexp))
          (t (phw-add-all-buffers-to-history)))))

(defvar phw-history-common-menu nil
  "Common menu entries for history-nodes")
(setq phw-history-common-menu
      '((phw-popup-history-filter-all-existing "Exactly all living file-buffers")
        ("---")
        ("Bucketize"
         (phw-popup-history-bucketize-never "No bucketizing")
         (phw-popup-history-bucketize-by-dir "by directory")
         (phw-popup-history-bucketize-by-dir-with-sp "by directory with source-path")
         (phw-popup-history-bucketize-by-mode "by major-mode")
         (phw-popup-history-bucketize-by-ext "by file-extension")
         (phw-popup-history-bucketize-by-reg "by reg. expr."))
        ("---")
        (phw-maximize-phw-window-menu-wrapper "Maximize window")))        


(defvar phw-history-menu nil
  "Built-in menu for the history-buffer.")
(setq phw-history-menu
      (append
       '(("Grep"
          (phw-grep-directory "Grep Directory")
          (phw-grep-find-directory "Grep Directory recursive"))
         ...
         ("Dired"
          ...
          (phw-dired-directory-other-window "Open Dir in Dired other window"))
         ("Filter"
          (phw-popup-history-filter-by-ext "Filter by extension")
          (phw-popup-history-filter-by-regexp "Filter by regexp")
          (phw-popup-history-filter-all-existing "No filter"))
         ("---")
         (phw-history-kill-buffer "Kill Buffer")
         (phw-delete-source "Delete Sourcefile"))
       phw-history-common-menu))

(defvar phw-history-menu-title-creator
  (function (lambda (node)
              (tree-node->name node)))
  "The menu-title for the history menu. See
`phw-directories-menu-title-creator'.")

(defun phw-history-menu-creator (tree-buffer-name node)
  "Creates the popup-menus for the history-buffer."
  (setq phw-layout-prevent-handle-phw-window-selection t)
  (let ((dyn-user-extension
         (and (functionp phw-history-menu-user-extension-function)
              (funcall phw-history-menu-user-extension-function
                       tree-buffer-name node)))
        (dyn-builtin-extension (phw-dir/source/hist-menu-editwin-entries)))
    (list (cons phw-history-nodetype-filebuffer
                (funcall (or phw-history-menu-sorter 'identity)
                         (append dyn-user-extension
                                 phw-history-menu-user-extension
                                 phw-history-menu
                                 dyn-builtin-extension)))
          (cons phw-history-nodetype-bucket
                (funcall (or phw-history-menu-sorter 'identity)
                         (append dyn-user-extension
                                 phw-history-common-menu)))
          (cons phw-history-nodetype-indirect-filebuffer
                (funcall (or phw-history-menu-sorter 'identity)
                         (append dyn-user-extension
                                 phw-history-menu-user-extension
                                 phw-history-menu
...
(defphw-tree-buffer-creator phw-create-directories-tree-buffer
    phw-directories-buffer-name
  ...
  (tree-buffer-create
   ...
   :node-mouse-over-fn 'phw-mouse-over-directory-node
   :mouse-highlight-fn t ;; highlight each node when moving mouse over it
   :node-data-equal-fn 'equal
   :maybe-empty-node-types (list phw-directories-nodetype-directory
                                 phw-directories-nodetype-sourcepath)
   ;; Now no longer tree-buffer decides if a node is displayed as leave but
   ;; now the file-browser does it in the function `phw-tree-node-add-files' -
   ;; Reason: We have now to deal with the VC-support
   :leaf-node-types nil ;;(list phw-directories-nodetype-sourcefile)
   :menu-creator 'phw-directories-menu-creator
   :menu-titles (list (cons phw-directories-nodetype-directory
                            phw-directories-menu-title-creator)
                      (cons phw-directories-nodetype-sourcefile
                            phw-directories-menu-title-creator)
                      (cons phw-directories-nodetype-sourcepath
   ...
   :trunc-lines (phw-member-of-symbol/value-list phw-directories-buffer-name
                                                 phw-tree-truncate-lines)
   :read-only t
   :tree-indent phw-tree-indent
   :incr-search-p phw-tree-incremental-search
   :incr-search-additional-pattern phw-vc-incr-searchpattern-node-prefix
   ...
   :additional-images-dir (phw-member-of-symbol/value-list phw-directories-buffer-name
   ...
   :type-facer (list (cons phw-directories-nodetype-sourcefile
                           phw-source-in-directories-buffer-face))
   :expand-symbol-before-p phw-tree-expand-symbol-before
   :highlight-node-face phw-directory-face
   :general-face phw-directories-general-face
   ;; we add an after-create-hook to the tree-buffer
   :after-create-hook (append
                       (list (function (lambda ()
                                         (local-set-key [f2] 'phw-customize)
                                         (local-set-key [f3] 'phw-show-help)
                                         (local-set-key [f4] 'phw-add-source-path)
                                         (phw-common-after-tree-buffer-create-actions))))
                       phw-common-tree-buffer-after-create-hook
                       phw-directories-buffer-after-create-hook)
   :after-update-hook 'phw-stealth-tasks-after-directories-update
   ))

(defphw-tree-buffer-creator phw-create-sources-tree-buffer phw-sources-buffer-name
  ...
  (tree-buffer-create
   ...
   :node-expanded-fn 'phw-tree-buffer-node-expand-callback
   :node-collapsed-fn 'phw-tree-buffer-node-collapsed-callback
   :node-mouse-over-fn 'phw-mouse-over-source-node
   :mouse-highlight-fn t ;; highlight each node when moving mouse over it
   :node-data-equal-fn 'equal
   ...
   :leaf-node-types nil ;; (list phw-sources-nodetype-sourcefile)
   :menu-creator 'phw-sources-menu-creator
   :menu-titles (list (cons phw-sources-nodetype-sourcefile
                            phw-sources-menu-title-creator))
   :modeline-menu-creator 'phw-common-tree-buffer-modeline-menu-creator
   :sticky-parent-p nil
   :sticky-parent-fn nil
   :trunc-lines (phw-member-of-symbol/value-list phw-sources-buffer-name
                                                 phw-tree-truncate-lines)
   :read-only t
   :tree-indent phw-tree-indent
   :incr-search-p phw-tree-incremental-search
   ...
   (lambda (search full-search-regexp)
     (if (phw-apply-filter-to-sources-buffer (and search
                                                  (> (length search) 0)
                                                  full-search-regexp)
                                             search)
         search
   ...
   :hor-scroll-step phw-tree-easy-hor-scroll
   :default-images-dir (car phw-tree-image-icons-directories)
   :additional-images-dir (phw-member-of-symbol/value-list phw-sources-buffer-name
                                                           (cdr phw-tree-image-icons-directories)
                                                           'car 'cdr)
   :image-file-prefix "phw-"
   :tree-style phw-tree-buffer-style
   ...
   :type-facer nil
   :expand-symbol-before-p phw-tree-expand-symbol-before
   :highlight-node-face phw-source-face
   :general-face phw-sources-general-face
   :after-create-hook (append
                       (list (function (lambda ()
                                         (phw-common-after-tree-buffer-create-actions))))
                       phw-common-tree-buffer-after-create-hook
                       phw-sources-buffer-after-create-hook)
   :after-update-hook 'phw-stealth-tasks-after-sources-update))

(defphw-tree-buffer-creator phw-create-history-tree-buffer phw-history-buffer-name
  ...
  (tree-buffer-create
   ...
   :node-expanded-fn 'phw-tree-buffer-node-expand-callback
   :node-collapsed-fn 'phw-tree-buffer-node-collapsed-callback
   :node-mouse-over-fn 'phw-mouse-over-history-node
   :mouse-highlight-fn t ;; highlight each node when moving mouse over it
   :node-data-equal-fn 'equal
   :maybe-empty-node-types nil
   :leaf-node-types nil
   :menu-creator 'phw-history-menu-creator
   :menu-titles (list (cons phw-history-nodetype-filebuffer
                            phw-history-menu-title-creator)
                      (cons phw-history-nodetype-indirect-filebuffer
                            phw-history-menu-title-creator)
                      (cons phw-history-nodetype-bucket
                            phw-history-menu-title-creator))
   ...
   :sticky-indent-string phw-tree-stickynode-indent-string
   :sticky-parent-fn nil
   :trunc-lines (phw-member-of-symbol/value-list phw-history-buffer-name
                                                 phw-tree-truncate-lines)
   :read-only t
   :tree-indent phw-tree-indent
   :incr-search-p phw-tree-incremental-search
   ...
   :hor-scroll-step phw-tree-easy-hor-scroll
   :default-images-dir (car phw-tree-image-icons-directories)
   :additional-images-dir (phw-member-of-symbol/value-list phw-history-buffer-name
                                                           (cdr phw-tree-image-icons-directories)
                                                           'car 'cdr)
   :image-file-prefix "phw-"
   :tree-style phw-tree-buffer-style
   ...
   :type-facer nil
   :expand-symbol-before-p phw-tree-expand-symbol-before
   :highlight-node-face phw-history-face
   :general-face phw-history-general-face
   :after-create-hook (append
                       (list (function (lambda ()
                                         (phw-common-after-tree-buffer-create-actions))))
                       phw-common-tree-buffer-after-create-hook
                       phw-history-buffer-after-create-hook)
   :after-update-hook 'phw-stealth-tasks-after-history-update))

(silentcomp-provide 'phw-file-browser)

;;; phw, 2015, 2015-file-browser.el ends here
