...
;; phw-add-tags runs not with method-buffer as current tree-buffer)...we can
...
See Info node `(phw)tree-buffer' for all details of using tree-nodes."
...
;;   (let ((id (phw-overlay-get
;;              (phw--semantic-tag-overlay
;;               (phw--semantic-current-tag)) 'PHW-tree-node-id)))
...
;;   (save-excursion
;;     (set-buffer phw-methods-buffer-name)
;;     (let ((reslist (tree-node-map-subtree
;;                     (tree-buffer-get-root)
;;                     (function
;;                      (lambda (node)
;;                        (when (= (tree-node->type node)
;;                                 phw-methods-nodetype-tag)
;;                          (phw--semantic-tag-name (tree-node->data node))))))))
;;       reslist)))
...
;;     (when (and (equal (buffer-name) phw-history-buffer-name)
;;                (= phw-history-nodetype-bucket (tree-node->type node)))
...
generated command is called by PHW via the popup-mechanism \(or the
...
\(tree-buffer-defpopup-command phw-my-special-dir-popup-function
  ...
  `(eval-and-compile
     (defun ,name (&optional node &rest rest-arg-list)
       ,(concat docstring
                "\n\n"
                "This is a generated command intended to be called via the popup-menu of PHW.\n"
                "The arguments NODE and REST-ARG-LIST can not be inserted in an interactive\n"
                "way by the end-user but are passed from PHW to this command.\n"
       ...
       (interactive)
       (let ((node (if (and (phw-interactive-p) (null node))
                       (tree-buffer-get-node-at-point)
                     node)))
         (when node
           ,@body)))))
...
See Info node `(phw, 2015, 2015)tree-buffer' for all details of using tree-buffers."
