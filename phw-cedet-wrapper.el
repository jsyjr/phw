;;; phw-cedet-wrapper.el -- define wrappers for all cedet funcs/vars
...
;; by PHW independent which cedet version is used. So the PHW-code is
...
;; which cedet version is loaded. PHW only uses the functions exported from
;; phw-cedet-wrapper.el!
;;
;; In addition this file defines all requirements PHW needs of CEDET.
...
(defconst phw-cedet-old-sourcetree-structure-detected-p
  (locate-library "semantic-ctxt")
  "Not nil if old cedet sourcetree-structure is detected.")

;; Additonal cedet libraries needed by phw must be added here!!
...
(defconst phw-cedet-lib-registry '((cedet . cedet)
                                   (semantic . semantic)
...
ALL CEDET-LIBRARIES NEEDED BY PHW MUST BE REGISTERED HERE!")

(defconst phw-cedet-required-version-min '(1 0 2 6)
  "Minimum version of cedet needed by PHW.
...
;; The constraint on a max version of PHW has been removed.

(defun phw-cedet-missing-libs ()
  "Return a list of names of missing cedet-libs.
If no cedet libs are missing nil is returned."
  (let ((missing-libs-list nil)
        (lib nil))
    (dolist (l-elem phw-cedet-lib-registry)
      (setq lib (symbol-name (if phw-cedet-old-sourcetree-structure-detected-p
                                 (car l-elem)
                               (cdr l-elem))))
...
(defun phw-cedet-require (old-style-lib)
...
All cedet libaryies needed by PHW must be loaded with this function! Do not
use `require' for looading a cedet-library into Emacs!"
  (require (if phw-cedet-old-sourcetree-structure-detected-p
               old-style-lib
             (cdr (assoc old-style-lib phw-cedet-lib-registry)))))
...
;;   needed by PHW.
(ignore-errors (phw-cedet-require 'cedet))

(defconst phw-cedet-missing-libraries
  (phw-cedet-missing-libs)
  "List of library-names of CEDET missed by PHW.
Nil if all libs needed by PHW are found.")

(unless phw-cedet-missing-libraries
  (phw-cedet-require 'semantic)
  (phw-cedet-require 'semantic-ctxt)
  (phw-cedet-require 'semantic-analyze)
  (phw-cedet-require 'semanticdb)
  (phw-cedet-require 'semanticdb-find)
  (phw-cedet-require 'semanticdb-mode)
  (phw-cedet-require 'eieio))

(defconst phw-compiled-in-semantic-version
  (eval-when-compile (ignore-errors semantic-version))
  "Semantic-version used for byte-compiling PHW. Either nil when no semantic
is loaded or the value of `semantic-version' at PHW-compilation time.")

(defconst phw-compiled-in-cedet-version
  (eval-when-compile (ignore-errors cedet-version))
  "Cedet-version used for byte-compiling PHW. Either nil when no semantic
is loaded or the value of `cedet-version' at PHW-compilation time.")

(defun phw-check-semantic-load ()
  "Checks if cedet is correctly loaded if semantic 2.X is used and if the same
semantic-version has been used for byte-compiling PHW and loading into Emacs.
If PHW detects a problem it is reported and then an error is thrown."
  (when (boundp 'semantic-version)
    (let ((err-msg
           (cond ;; Different semantic-version used for byte-compiling PHW and
            ;; loading into Emacs.
            ((not (string= semantic-version phw-compiled-in-semantic-version))
             (concat "PHW has been byte-compiled with another semantic-version than currently\n"
                     "loaded into Emacs:\n"
                     (format "  + Semantic used for byte-compiling PHW: %s\n"
                             phw-compiled-in-semantic-version)
                     (format "  + Semantic currently loaded into Emacs: %s\n"
                             semantic-version)
                     "Please ensure that PHW is byte-compiled with the same semantic-version as you\n"
                     "you load into your Emacs. Check if you have byte-compiled PHW with the cedet-\n"
                     "suite but loaded old semantic 1.X into Emacs or vice versa.\n\n"
                     "In general it is recommended to start PHW first-time not byte-compiled\n"
                     "and then call the command `phw-byte-compile'. This ensures you byte-compile PHW\n"
      ...
      (unless (= 0 (length err-msg)) 
        (with-output-to-temp-buffer "*PHW semantic-load problems*"
          (princ "Currently PHW can not be activated cause of the following reason:\n\n")
          (princ err-msg)
          (princ "\n\nPlease fix the reported problem and restart Emacs\n"))
        (phw-error "Please fix the reported problem and restart Emacs!")))))

(defun phw-check-cedet-load ()
  "Checks if cedet is correctly loaded if semantic 2.X is used and if the same
semantic-version has been used for byte-compiling PHW and loading into Emacs.
If PHW detects a problem it is reported and then an error is thrown."
  (when (boundp 'cedet-version)
    (let ((err-msg
           (cond ;; cedet was not compiled into PHW
            ((null phw-compiled-in-cedet-version)
             (concat (format "Currently CEDET %s is loaded but PHW has been byte-compiled without\n"
                             cedet-version)
                     "any CEDET. Please either use PHW un-byte-compiled \(remove all *.elc\n"
                     "files from the PHW-directory) or byte-compile PHW correctly with CEDET!\n"
                     "In the later case it is recommended to start PHW first-time not byte-compiled\n"
                     "and then call the command `phw-byte-compile'. This ensures you byte-compile PHW\n"
            ...
            ;; Different cedet-version used for byte-compiling PHW and
            ;; loading into Emacs.
            ((not (string= cedet-version phw-compiled-in-cedet-version))
             (concat "PHW has been byte-compiled with another cedet-version than currently\n"
                     "loaded into Emacs:\n"
                     (format "  + CECET used for byte-compiling PHW: %s\n"
                             phw-compiled-in-cedet-version)
                     (format "  + CEDET currently loaded into Emacs: %s\n"
                             cedet-version)
                     "Please ensure that PHW is byte-compiled with the same cedet-version as you\n"
            ...
            (t ""))))
      (unless (= 0 (length err-msg)) 
        (with-output-to-temp-buffer "*PHW cedet-load problems*"
          (princ "Currently PHW can not be activated cause of the following reason:\n\n")
          (princ err-msg)
          (princ "\n\nPlease fix the reported problem and restart Emacs\n"))
        (phw-error "Please fix the reported problem and restart Emacs!")))))

(defconst phw-semantic-2-loaded (ignore-errors
                                  (string-match "^2" semantic-version)))

;; -- getter functions for all variables of cedet currently used by PHW ---
...
;; -- an alias for all functions of cedet currently used by PHW ---

(defconst phw--cedet-function-list
  '(
...
(defconst phw--semantic-format-function-list
  '(
...
;; cedet >= 1.0
(unless phw-cedet-missing-libraries
  (dolist (f-elem  (append phw--cedet-function-list phw--semantic-format-function-list))
    (defalias (intern (concat "phw--" (symbol-name f-elem))) f-elem)))

(silentcomp-provide 'phw-cedet-wrapper)

;;; phw-cedet-wrapper.el end here
