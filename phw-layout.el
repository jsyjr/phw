...
these modes/variables is restored."
  (if arg
      (progn
        ...
        (or (get 'phw-enable-temp-buffer-shrink-to-fit
...
(defcustom phw-compile-window-height nil
  "*Height of the persistent compilation-window of PHW.
If you want a compilation window shown at the bottom of the PHW-layout
then set here the height of it \(Default is a height of 5). If you redraw the
current layout with `phw-redraw-layout' then the compilation window (if any)
...
Beware: If you set a persistent compilation window then PHW displays all buffers
for which `phw-compilation-buffer-p' returns not nil in that persistent
compilation window. If a buffer which should being displayed there is not
displayed there then try to modify the options `phw-compilation-buffer-names',
`phw-compilation-major-modes' or `phw-compilation-predicates' \(in this
sequence).

See also the options `phw-compile-window-temporally-enlarge' and
`phw-enlarged-compilation-window-max-height' and also the command
`phw-toggle-compile-window-height'!

PHW offers the functionality of such a persistent compile-window regardless if
the special PHW-windows are visible or not \(see the command
...
layout with `phw-redraw-layout'"
  ...
  ;; value of `phw-windows-hidden-state'! Same for `phw-compile-window-width'. If
  ;; this is necessary for other options too then we should
  ;; `phw-layout-option-set-function' to a function with an additional
  ;; parameter which decides if phw-window-hidden should be used for
  ;; NO-PHW-WINDOWS or not.
  :set (function (lambda (symbol value)
                   ...
                   (when (and (boundp 'phw-minor-mode)
                              phw-minor-mode
                              (frame-live-p phw-frame))
                     (phw-enable-temp-buffer-shrink-to-fit value)
                     (let ((curr-frame (selected-frame)))
                       (unwind-protect
                           (progn
                             (select-frame phw-frame)
...
This option has only an effect if `phw-compile-window-height' is not nil!

The following values are possible:
- 'after-display: After displaying a \"compilation-buffer\" \(in the sense of
  `phw-compilation-buffer-p'!) in the compile-window of PHW. For the max.
  height of the enlarged compile-window see the option
  `phw-enlarged-compilation-window-max-height'.

- 'after-selection: selecting the `phw-compile-window' auto. enlarges it and
  de-selecting \(means leaving `phw-compile-window') auto. shrinks it.
  Enlarging and shrinking the `phw-compile-window' is done with
  `phw-toggle-compile-window-height'. See also the documentation of this
  function!

- 'both: The combination of 'after-display and 'after-selection.

- nil: PHW fixes always the height of the `phw-compile-window' at the value of
...
(defcustom phw-maximize-phw-window-after-selection nil
...
(defcustom phw-maximize-next-after-maximized-select nil
  "*Maximizes the next logical tree-window after a maximized node-selection.
Selecting a node in a maximized tree-window is handled very smart by PHW:
...
`phw-tree-do-not-leave-window-after-select' \(see also the tree-buffer-command
`phw-toggle-do-not-leave-window-after-select' which is bound to `C-t' in each
tree-buffer) then PHW does *not* maximize the next logical tree-window but
...
used \(e.g. `phw-directories-buffer-name', `phw-sources-buffer-name' or
...
;; below phw-compile-window-height and it is also not worth. IMHO preventing
...
(defcustom phw-compile-window-prevent-shrink-below-height t
  "*Allow the compile-window to be shrunken below its height.
A non nil value means PHW prevents the compile-window from being shrunken
below the threshold of `phw-compile-window-height' by displaying temp-buffers
\(e.g. *Help* etc.) or after running compilations or greps. But interactively
it is always allowed to shrink it to every height!

If nil then PHW does nothing to prevent being shrunken below the value of
`phw-compile-window-height'.
...
(defcustom phw-enlarged-compilation-window-max-height 'best
  "*The max height of the compile-window after enlarging it.
The max height of the compilation window after enlarged by
`phw-toggle-compile-window-height'. The following values are allowed:

'best:

PHW fits the height of the compile-window exactly to the size of its current
contents but never shrinks below the value of `phw-compile-window-height' or
enlarges over the half of the frame-height of the PHW-frame. The values of the
...
1/2 the frame-height of the PHW-frame
...
(defcustom phw-scroll-other-window-scrolls-compile-window nil
...
- always when PHW is active - that means no special-display-handling of
  buffers when PHW is active
...
(defsubst phw-ignore-special-display ()
  (or (equal phw-ignore-special-display 'always)
      (and (equal phw-ignore-special-display 'compile-window)
           phw-compile-window-height)))

(defcustom phw-ignore-pop-up-frames 'compile-window
...
(defsubst phw-ignore-pop-up-frames ()
  (or (equal phw-ignore-pop-up-frames 'always)
      (and (equal phw-ignore-pop-up-frames 'compile-window)
           phw-compile-window-height)))

(defcustom phw-ignore-display-buffer-function 'always
...
`display-buffer-function' when called for the `phw-frame'. If this variable
...
smart PHW-logic will help to deal best with the PHW-window-layout! You can
define if and when `display-buffer-function' should be ignored:
- only when persistent compile window is used - i.e. if
  `phw-compile-window-height' is not nil
- always when PHW is active - that means ignore when PHW is active otherwise
...
(defsubst phw-ignore-display-buffer-function ()
  (or (equal phw-ignore-display-buffer-function 'always)
      (and (equal phw-ignore-display-buffer-function 'compile-window)
           phw-compile-window-height)))

(defcustom phw-split-edit-window-after-start 'before-deactivation
  "*Sets how and if the edit window should be splitted after PHW-start.
But be aware: This option determines only if and how the edit-window should be
splitted at start-time of PHW. There are five different values allowed for
this option:
- nil: Do not split the edit-area of PHW after activation, i.e. there will be
  only one edit-window after starting PHW.
...
- 'before-activation: Split the edit-area as before the PHW-start, i.e. the
  edit-area will have after start a window-layout as the whole frame had
  before the start of PHW.
- 'before-deactivation: Split the edit-area into a window-layout PHW had in
  its edit-area direct before the PHW-deactivation. This value preserves the
  full state between activations of PHW, i.e. the visibility of the
  PHW-windows, the visibility of a compile-window and also the full
  ...
  `phw-layout-name', `phw-compile-window-height', `phw-compile-window-width',
  `phw-windows-width' and `phw-windows-height'.

Default value is 'before-deactivation.

Some remarks to the value 'before-activation: If this value has been set then
PHW needs four permanent adivces even when PHW is deactivated: `split-window',
...
in an internal PHW-variable the facts that a window has been splitted or
...
(defcustom phw-fix-window-size nil
  "*Fix size of the PHW-windows/buffers even after frame-resizing.
...
'height and t. Therefore this option takes no effect \(means all phw-windows
have always unfixed sizes) with Emacs < 22 if `phw-compile-window-height' is
not nil.
...
(defun phw-get-window-fix-type (layout-name)
  "Determine which value of `window-size-fixed' we must set in all phw-buffers
of layout LAYOUT-NAME."
  (if (symbolp phw-fix-window-size)
      (if (equal phw-fix-window-size 'auto)
          (if (equal (phw-get-layout-type phw-layout-name) 'top)
              'height
            'width)
        phw-fix-window-size)
...
PHW will cycle through all windows of the PHW-frame or scroll simply
the next window in the PHW-frame, means it behaves like the original
`other-window' rsp. the original `other-window-for-scrolling'.

'only-edit:

PHW will only cycle through the edit-windows of PHW or only
...
With this setting PHW tries to choose the `other-window'-destination or the
...
If one of the special phw-windows is selected then always the \"next\"
phw-window is choosen \(whereas the next phw-window of the last phw-window is
the first phw-window). In the context of an `other-window'-call the ARG of
`other-window' will be taken into account. If there is only one phw-window
then PHW considers also the edit-windows!
...
Regardless of the allowed values above PHW handles the situation of an active
minibuffer during a call to `other-window' or `scroll-other-window' like
follows:

If the minibuffer-window is selected then PHW always chooses the window
...
1. A canonical list of all currently visible windows of the `phw-frame'
2. A canonical list of all currently visible edit-windows
3. A canonical list of all currently visible phw-windows
4. The window-object of the compile-window if there is any.
5. The minibuffer-window of the PHW-frame if there is an active minibuffer.
5. The result of the function `phw-where-is-point' - see the documentation
...
(defcustom phw-advice-window-functions-signal-error nil
...
phw-tree-window or if one tries to switch to another buffer in one of the
phw-tree-windows. For details see the documentation of each of the adviced
...
If we are in an PHW special buffer (methods, directories, etc), and any of the
adviced windowing functions is called interactively, we will select first an
edit-window according to the value of `phw-mouse-click-destination'. This is
...
`phw-advice-window-functions-signal-error'); see the documentation of the
...
(defcustom phw-layout-window-sizes nil
  "*Specifies the sizes of the PHW windows for each layout.
...
`phw-store-window-sizes'. Next time the layout is redrawn the values stored in
this option will be used.

If `phw-store-window-sizes' is used then the windows sizes are stored per
default as fractions of current frame-width and -height of the phw-frame, so
the stored values will \"work\" for other frame sizes too. But if you call
`phw-store-window-sizes' with a prefix-argument then the fixed values of
current width and height are stored!

If this option is set \"by hand\" \(i.e. not by `phw-store-window-sizes') then
...
- It is also recommended to use buffer-name-symbols instead of plain
  buffer-names \(e.g. phw-history-buffer-name instead of \" *PHW History*\")
- There must be an entry for each special phw-buffer of that layout for which
...
(defcustom phw-redraw-layout-quickly nil
...
(defcustom phw-left-right-layout-hide-sequence '(left-side all right-side none)
  ...
  :type '(repeat (choice :tag "Hidden windows"
                         ...
                         (const :tag "Only right side phw-windows hidden" :value right-side)
                         (const :tag "All phw-windows hidden" :value all)
                         (const :tag "No phw-window hidden (ie. all displayed)" :value none))))

(defcustom phw-hide-phw-windows-before-hook nil
  "*Hook run direct before the PHW windows will be hidden.
Hiding is done either by `phw-toggle-phw-windows' or `phw-hide-phw-windows'.
This means that at runtime of this hook all the PHW-tree-windows of current
layout are visible.

IMPORTANT: Showing the hidden PHW-windows is internally done by calling
`phw-redraw-layout' and therefore also the hooks
...
(defcustom phw-hide-phw-windows-after-hook nil
  "*Hooks run direct after the PHW windows have been hidden.
Hiding was done either by `phw-toggle-phw-windows' or `phw-hide-phw-windows'.
...
(defcustom phw-show-phw-windows-before-hook nil
  "*Hooks run direct before the PHW windows will be shown.
Showing is done either by `phw-toggle-phw-windows' or `phw-show-phw-windows'.
This means that at runtime of this hook the PHW-windows are still hidden.

IMPORTANT: Showing the hidden PHW-windows is internally done by calling
`phw-redraw-layout' and therefore also the hooks
`phw-redraw-layout-before-hook' and `phw-redraw-layout-after-hook' are
evaluated. So there is the following sequence of hooks during the process of
showing the hidden PHW-windows:
1. `phw-show-phw-windows-before-hook'
2. `phw-redraw-layout-before-hook'
3. <redrawing the layout to show the hidden PHW-windows>
...
(defcustom phw-show-phw-windows-after-hook nil
  "*Hooks run direct before the PHW windows will be shown.
Showing has been done either by `phw-toggle-phw-windows' or
`phw-show-phw-windows'. This means that at runtime of this hook the
PHW-windows are already visible.
...
problems to display buffers in the compile-window of PHW \(e.g. buffers which
...
1. Set `phw-layout-debug-mode' to not nil
...
(defvar phw-frame nil
  "Frame where PHW runs. This frame is only set if this variable is nil or the
value points to a dead frame. Deactivation and activation of PHW does not set
this variable to nil!")

(defvar phw-edit-window nil
  "Only used internally by `phw-redraw-layout-full'. Do not refer to this
variable because the value is not predictable!")

(defvar phw-last-edit-window-with-point nil
  "The edit-window of PHW which had the point before an emacs-command is
done.")

(defvar phw-last-source-buffer nil
  "The source-buffer of `phw-last-edit-window-with-point'.")

(defvar phw-last-compile-buffer-in-compile-window nil
  "The buffer in the compile-window before an emacs-command is done.")

(defvar phw-compile-window nil
...
;; `phw-compile-window-height' will often be unequal to that height drawn by
;; `phw-redraw-layout-full' ==> we store and use the height drawn by this
;; function.
(defvar phw-compile-window-height-lines nil
  "Contains always the absolute number of lines \(incl. modeline) of the
compile-window direct after a full redraw. Only set by
`phw-redraw-layout-full' and evaluated and used by *all* mechanisms of PHW
which shrink back the compile-window to its specified height without doing a
full redraw!!")

(defvar phw-compile-window-was-selected-before-command nil
  "Not nil only if the `phw-compile-window' was selected before most recent
command.")

(defvar phw-layout-default-window-sizes nil
  "Contains the the sizes of the phw-windows of the current layout exactly as
drawn by the layout-function \(see `phw-redraw-layout-full').")

;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: remove this later - and all usages.
(defvar phw-windows-hidden nil
  "Used with `phw-toggle-phw-windows'. If true the PHW windows are hidden. Do
not change this variable!")

(defconst phw-windows-hidden-all-value 'all)
(defconst phw-windows-hidden-none-value 'none)

(defvar phw-windows-hidden-state phw-windows-hidden-none-value
  "Information which PHW windows are hidden.
The value is one of the symbols left-side, right-side, top-side,
none or all which indicates which phw-windows are hidden \(ie.
all phw-windows on this frame-side are hidden).
...
`phw-windows-all-hidden', `phw-windows-all-displayed' or
`phw-windows-hidden-state-list'!")
...
;; -- later)
(defun phw-windows-all-hidden (&optional hidden-state)
  "Return not nil if all phw-windows are hidden.
If HIDDEN-STATE is not nil then it must be of the same type as
`phw-windows-hidden-state'. If nil then the value of
`phw-windows-hidden-state' is used.
This function has a \"binary\" semantic: a return value of nil
means that some or all phw-windows of current layout are displayed and a
return value of not nil means that all phw-windows are hidden."
  t)

(defun phw-windows-all-displayed (&optional hidden-state)
  "Return not nil if all phw-windows are displayed.
If HIDDEN-STATE is not nil then it must be of the same type as
`phw-windows-hidden-state'. If nil then the value of
`phw-windows-hidden-state' is used. This function has a
\"binary\" semantic: a return value of not nil means that all
phw-windows of current layout are displayed and a return value of
nil means that either no or only some but not all phw-windows are
displayed."
  nil)

(defun phw-windows-hidden-state-list (&optional hidden-state)
  "Return the hidden-state of the phw-windows as list.
If HIDDEN-STATE is not nil then it must be of the same type as
`phw-windows-hidden-state'. If nil then the value of
`phw-windows-hidden-state' is used.

The return value is either nil if all phw-windows are displayed
\(i.e. no phw-windows are hidden) or a list which sides of the
phw-frame are hidden. This list contains an appropriate set of
...
(defun phw-windows-toggled-hidden-state (&optional hidden-state)
...
'phw-left-right-layout-hide-sequence': It is the successor of the
...
the toggled state is the first element of that list."
  (let ((state (or hidden-state phw-windows-hidden-state)))
    (if (equal (phw-get-layout-type) 'left-right)
        (phw-next-listelem phw-left-right-layout-hide-sequence state)
      (if (phw-windows-all-displayed)
          'all
        'none))))

(defvar phw-special-phw-buffers-of-current-layout nil
  "The list of special phw-buffers of current-layout.
This variable is only set by `phw-redraw-layout-full'!")

(defvar phw-phw-buffer-name-selected-before-command nil
  "Not nil only if a special phw-window was selected before most recent
command. If not nil it contains the buffer-name of this special phw-buffer.")

(defvar phw-layout-prevent-handle-phw-window-selection nil
...
These are all windows in the `phw-frame' which are not identical to the
compile-window and not identical to one of the visible PHW-windows and which
are not dedicated."
    ...
    (delq nil (mapcar (function (lambda (elem)
                                  (if (and (or (member elem
                                           ...
                                           (or (not (equal comp-win-state 'visible))
                                               (not (equal elem phw-compile-window))))
                                      elem)))
                      windows-list))))

(defun phw-canonical-residual-windows-list (&optional winlist)
  "Return a list of all current residual windows \(starting from the left-most
top-most window) in the order `other-window' would walk through these windows.
These are all windows in the `phw-frame' which are not identical to the
compile-window and not identical to one of the visible PHW-windows.
...
(defun phw-initialize-layout ()
  ...
  (setq phw-edit-window nil
        phw-last-edit-window-with-point nil
        phw-last-source-buffer nil
        phw-last-compile-buffer-in-compile-window nil
        phw-current-maximized-phw-buffer-name nil
        phw-cycle-phw-buffer-state nil
        phw-special-phw-buffers-of-current-layout nil
        phw-windows-hidden-state phw-windows-hidden-none-value
        phw-compile-window nil
        phw-layout-prevent-handle-compile-window-selection nil
        phw-layout-prevent-handle-phw-window-selection nil
        phw-phw-buffer-name-selected-before-command nil
        phw-compile-window-was-selected-before-command nil
        phw-compile-window-height-lines nil)
  (phw-window-config-cache-clear))
...
(defun phw-compile-window-live-p (&optional display-msg)
...
user is displayed in the echo-area if no compile-window is visible."
  (if (and phw-compile-window-height
           phw-compile-window
           (window-live-p phw-compile-window))
      t
    (if display-msg
        (message "No compile-window visible in current PHW-layout!"))
    nil))

(defun phw-get-compile-window-buffer ()
  "Return the buffer currently displayed in the compile-window or nil if there
is no compile-window displayed."
  (if (phw-compile-window-live-p)
      (window-buffer phw-compile-window)))

;; Klaus Berndl <klaus.berndl@sdm.de>: This function is only there for
;; backward compatibility and is not needed for PHW-versions > 2.11
(defun phw-edit-window-live-p ()
...
to proceed. If yes then PHW will be deactivated before deleting FRAME. If PHW
is not activated or FRAME is not equal the PHW-frame then this advice is
...
;; and `phw-toggle-compile-window-height'!
(defphw-advice compilation-set-window-height around phw-layout-basic-adviced-functions
  "Makes the function compatible with PHW."
  (if (not (equal (window-frame (ad-get-arg 0)) phw-frame))
      (phw-with-original-basic-functions
       ad-do-it)
    (if (and (equal (ad-get-arg 0) phw-compile-window)
      ...
      ;; `phw-compile-window-height'
      (let* ((comp-win-height-value (phw-buffer-local-value
                                     'compilation-window-height
                                     (window-buffer (ad-get-arg 0))))
             (compilation-window-height (if (and phw-compile-window-prevent-shrink-below-height
        ...
        (and compilation-window-height
             ...
             (or (equal (ad-get-arg 0) phw-compile-window)
                 (phw-edit-window-splitted))
             ...
             (save-excursion
               (let ((w (selected-window)))
                 (phw-layout-debug-error "compilation-set-window-height: window: %s, cur-win: %s, cur-height: %d"
                                         (ad-get-arg 0) w
...
;; `other-window-for-scrolling'.
(defphw-advice scroll-other-window around phw-layout-basic-adviced-functions
...
details which window will be scrolled."
  (if (or (not phw-minor-mode)
          ...
          (and (equal (selected-window) (minibuffer-window phw-frame))
    ...
    (let* ((o-w (other-window-for-scrolling))
           ...
           (other-window-scroll-buffer (if (or (equal (current-buffer) o-w-s-b)
                                               (equal (minibuffer-window phw-frame)
                                                      o-w))
                                           nil
                                         o-w-s-b)))
      ad-do-it)))
...
;; window-resizing!
;; (defadvice find-file (around phw)
...
;; the phw-windows exactly as before this command."
;;   (if (and phw-minor-mode
...
(defun phw-toggle-scroll-other-window-scrolls-compile (&optional arg)
  "Toggle the state of `phw-scroll-other-window-scrolls-compile-window'.
...
`other-window-for-scrolling'."
  ...
  (unless (or (not phw-minor-mode)
              (not (equal (selected-frame) phw-frame)))
    (setq phw-scroll-other-window-scrolls-compile-window
          (if (null arg)
              (not phw-scroll-other-window-scrolls-compile-window)
...
`phw-canonical-edit-windows-list'). If EDIT-WINDOWS-LIST is nil then a new
edit-window-list is computed via `phw-canonical-edit-windows-list'."
  (let ((edit-win-list (or edit-windows-list (phw-canonical-edit-windows-list))))
    (cond ((null edit-win-list)
           (phw-error "Internal error - redraw the layout!"))
          ((= (length edit-win-list) 1)
           nil)
          ((= (length edit-win-list) 2)
           (if (= (car (phw-window-edges (car edit-win-list)))
                  (car (phw-window-edges (cadr edit-win-list))))
               'vertical
             'horizontal))
          (t 'splitted))))

(defvar phw-temp-buffer-shrink-to-fit nil
...
;; properly within `phw-display-buffer-xemacs' and
...
(defun phw-display-buffer-xemacs (buffer &optional not-this-window-p
                                         override-frame
                                         shrink-to-fit)
...
Returns the window displaying BUFFER."
  ...
  (phw-layout-debug-error "phw-display-buffer-xemacs for %s %s %s %s [%s]"
                          buffer not-this-window-p override-frame
                          shrink-to-fit phw-temp-buffer-shrink-to-fit)
  ...
  ;; `phw-temp-buffer-shrink-to-fit' is only set by
  ...
  ;; there. This is done be a let-binding so `phw-temp-buffer-shrink-to-fit'
  ;; has always nil after finishing this function! Only if
  ;; `phw-temp-buffer-shrink-to-fit' is not nil here we override the value of
  ;; SHRINK-TO-FIT. And because  `phw-temp-buffer-shrink-to-fit' is only set
  ...
  ;; `display-buffer'!
  (setq shrink-to-fit (or phw-temp-buffer-shrink-to-fit shrink-to-fit))
        ...
        (result
         ...
         (catch 'done
           (let (window old-frame target-frame explicit-frame shrink-it)
             ...
             (setq buffer (phw-buffer-obj buffer))
...
(defun phw-temp-buffer-show-function-emacs (buf)
  ...
  (when (phw-compilation-buffer-p buf)
    (phw-layout-debug-error "phw-temp-buffer-show-function-emacs: comp-buffer: %s"
                            buf)
    (when (and (equal (selected-frame) phw-frame)
               (equal 'hidden (phw-compile-window-state))
               ;; calling this from minibuffer (e.g. completions)
               ;; seems to cause problems
               (not (equal (minibuffer-window phw-frame) (selected-window))))
      (phw-layout-debug-error "phw-temp-buffer-show-function-emacs: comp-win will toggled")
      (phw-toggle-compile-window 1)))
  (save-selected-window
    (save-excursion
      ;; this call to `display-buffer' runs the adviced version of PHW which
      ;; always handles all the compile-window stuff if buf is a
      ;; compile-buffer in the sense of `phw-compilation-buffer-p'.
      (let ((win (display-buffer buf)))
        (phw-layout-debug-error "phw-temp-buffer-show-function-emacs: win: %s, buf: %s,dedi:%s"
                                win buf (window-dedicated-p win))
        (select-window win)
        (setq minibuffer-scroll-window win)
        (set-buffer buf)
        (run-hooks 'temp-buffer-show-hook)
        (phw-layout-debug-error "phw-temp-buffer-show-function-emacs: sel-win: %s, win-buf: %s,dedi%s"
                                (selected-window)
                                (window-buffer (selected-window))
                                (window-dedicated-p (selected-window)))
;;         (if (and (boundp 'help-window)
;;                  (equal (selected-window) phw-compile-window))
;;             (setq help-window phw-compile-window))
        ))))

(defvar phw-temp-buffer-show-function-old nil)

(defun phw-enable-own-temp-buffer-show-function (arg)
  "If ARG then set `temp-buffer-show-function' to the right function so PHW
works correct. Store the old value. If ARG is nil then restore the old value
of `temp-buffer-show-function'."
  (if arg
      (progn
        (setq phw-temp-buffer-show-function-old
              temp-buffer-show-function)
        (setq temp-buffer-show-function
              (if phw-running-xemacs
                  'show-temp-buffer-in-current-frame
                'phw-temp-buffer-show-function-emacs)))
    (setq temp-buffer-show-function
          phw-temp-buffer-show-function-old)))


;; =========== intelligent window function advices ===================

(defmacro phw-with-original-basic-functions (&rest body)
  "Evaluates BODY with all adviced basic-functions of PHW deactivated \(means
with their original definition). Restores always the previous state of the PHW
adviced basic-functions, means after evaluating BODY it activates the advices
of exactly the functions in `phw-layout-basic-adviced-functions'!"
  `(phw-with-original-adviced-function-set 'phw-layout-basic-adviced-functions
     ,@body))

(defphw-advice-set phw-permanent-adviced-layout-functions
  "Permanent adviced layout functions.
This means these function will remain adviced after deactivating phw!"
  t)

(defmacro phw-with-original-permanent-layout-functions (&rest body)
  "Evaluates BODY with all adviced permanent-functions of PHW deactivated
\(means with their original definition). Restores always the previous state of
the PHW adviced permanent-functions, means after evaluating BODY it activates
the advices of exactly the functions in `phw-permanent-adviced-layout-functions'!"
  `(phw-with-original-adviced-function-set 'phw-permanent-adviced-layout-functions
                                           ,@body))

(defun phw-where-is-point (&optional win-list)
  "Return a cons-cell with the exact location of point in the phw-frame.
The car is one of 'phw \(point in a special phw-window), 'edit
...
which is not one of the special phw-windows - some packages like
...
the number of the window in the full window-list of the phw-frame \(without
minibuffer-window).

If the compile-window or the minibuffer is the selected window then the window
number is always 1.

If the phw-frame is not the selected frame then nil is returned.
...
If point stays in the second special-window list, then \('phw . 2) is
...
\('compile . 1) rsp. \(minibuf . 1) is returned."
  (when (equal (selected-frame) phw-frame)
    (let ((win-list (or win-list (phw-canonical-windows-list)))
          (type (cond ((memq (selected-window)
                             (phw-canonical-phw-windows-list win-list))
                       'phw)
                      ((memq (selected-window)
                             (phw-canonical-edit-windows-list win-list))
                       'edit)
                      ((equal (selected-window) (minibuffer-window phw-frame))
                       'minibuf)
                      ((phw-point-in-compile-window)
                       'compile)
                      (t 'other-dedicated))))
           (cons type (case type
                        (phw (phw-point-in-phw-window-number
                              (phw-canonical-phw-windows-list win-list)))
                        (edit (phw-point-in-edit-window-number
                               (phw-canonical-edit-windows-list win-list)))
                        (compile 1)
                        (minibuf 1)
                        (otherwise (phw-window-in-window-list-number win-list)))))))
...
(defun phw-point-in-phw-window-number (&optional phw-windows-list)
  "Return nil if point stays not in an special phw-window otherwise return 1
if point is in the left/topmost phw-window or 2 if in the next phw-window and
so on. Return the number of the phw-window \(if point is in an phw-window) in
the order `walk-windows' would go through the phw-windows. If PHW-WINDOWS-LIST
is not nil then it must be a current list of phw-windows \(got by
`phw-canonical-phw-windows-list'). If PHW-WINDOWS-LIST is nil then a new
phw-window-list is computed via `phw-canonical-phw-windows-list'."
  (when (equal (selected-frame) phw-frame)
    (ignore-errors (phw-window-in-window-list-number
                    (or phw-windows-list (phw-canonical-phw-windows-list))))))


(defun phw-point-in-edit-window-number (&optional edit-windows-list)
...
edit-window-list is computed via `phw-canonical-edit-windows-list'."
  (when (equal (selected-frame) phw-frame)
    (ignore-errors (phw-window-in-window-list-number
                    (or edit-windows-list (phw-canonical-edit-windows-list))))))

(defmacro phw-when-point-in-edit-window-phw-windows-visible (&rest body)
  "Evaluate BODY if an edit-window is selected and phw-windows are visible."
  `(when (and phw-minor-mode
              (not (phw-windows-all-hidden))
              (phw-point-in-edit-window-number))
     ,@body))

(defun phw-display-source (source other-edit-window)
  "Display SOURCE in the correct edit-window.
What the correct window is depends on the setting in
`phw-mouse-click-destination' and the value of OTHER-EDIT-WINDOW
\(for this see `phw-combine-phw-button/edit-win-nr').
...
name of an indirect-buffer."
  (select-window (phw-get-edit-window other-edit-window))
  (phw-nav-save-current)
  (switch-to-buffer (phw-source-get-buffer source))
  (phw-nav-add-item (phw-nav-file-history-item-new)))

(defun phw-get-edit-window-by-number (edit-win-nr &optional edit-win-list)
  "Return that edit-window with number EDIT-WIN-NR. If EDIT-WIN-LIST is set
then get it from that list otherwise from `phw-canonical-edit-windows-list'.
EDIT-WIN-NR must be an integer between 1 and length of EDIT-WIN-LIST \(rsp.
`phw-canonical-edit-windows-list')."
  (nth (1- edit-win-nr) (or edit-win-list (phw-canonical-edit-windows-list))))

(defun phw-get-window-by-number (win-nr &optional win-list)
  "Return that window with number WIN-NR. If WIN-LIST is set
then get it from that list otherwise from `phw-canonical-windows-list'.
...
- t if PHW-BUTTON is 2 and the edit-area of PHW is splitted.
- EDIT-WINDOW-NR if PHW-BUTTON is 3."
  (case phw-button
    (1 nil)
    (2 (phw-edit-window-splitted))
    (3 edit-window-nr)))

(defun phw-get-edit-window (other-edit-window)
...
`phw-combine-phw-button/edit-win-nr') and `phw-mouse-click-destination'.
- OTHER-EDIT-WINDOW is nil: Get the edit-window according to the option
  `phw-mouse-click-destination'.
...
(defun phw-point-in-compile-window ()
  "Return not nil iff point is in the compile-window of PHW"
...
(defun phw-goto-phw-window (phw-buffer-name)
  "Select that phw-window displaying the special phw-buffer PHW-BUFFER-NAME.
Only buffer-names defined for the current layout \(see function
`phw-buffer-is-phw-buffer-of-current-layout-p') or the buffer-name of the
integrated speedbar are accepted. If such a window can not be selected then
probably because another phw-window of current layout is currently maximized;
...
described conditions or if the phw-windows are hidden or PHW is not active. If
necessary the `phw-frame' will be first raised."
  (when (and phw-minor-mode
             (not (phw-windows-all-hidden))
             (or (equal phw-buffer-name phw-speedbar-buffer-name)
                 (phw-buffer-is-phw-buffer-of-current-layout-p phw-buffer-name)))
    (raise-frame phw-frame)
    (select-frame phw-frame)
    (or (phw-window-select phw-buffer-name)
        ;; the window is not visible because another one is maximized;
        ;; therefore we first redraw the layout
        (progn
          (phw-redraw-layout-full nil nil nil nil)
          ;; now we can go to the window
          (phw-window-select phw-buffer-name)))))

(defun phw-goto-window-edit-last ()
  "Make the last selected edit-window window the current window. This is the
same as if `phw-mouse-click-destination' is set to 'last-point."
  (interactive)
  (when phw-minor-mode
    (raise-frame phw-frame)
    (select-frame phw-frame)
...
(defun phw-goto-window-edit1 ()
  ...
  (when phw-minor-mode
    (raise-frame phw-frame)
    (select-frame phw-frame)
    (phw-select-edit-window 1)))

(defun phw-goto-window-edit2 ()
  "Make the second edit-window \(if available) window the current window."
  (interactive)
  (when phw-minor-mode
    (raise-frame phw-frame)
    (select-frame phw-frame)
    (phw-select-edit-window t)))

(defun phw-goto-window-edit-by-smart-selection (&optional use-immediate-completion)
...
special phw-windows or in the compile-window).
...
and point stays either in one of the special phw-windows or in the
compile-window).
    ...
    (let* ((destination-list (if (member (selected-window) win-list)
      ...
      (when choices-buffer-name-alist
        (if (= (length choices-buffer-name-alist) 1)
            ...
            (phw-window-select (cdar choices-buffer-name-alist))
          ...
          (if use-immediate-completion
              (phw-window-select
               (cdr (assoc (phw-offer-choices "Select a window: "
                                              (mapcar 'car choices-buffer-name-alist))
                           choices-buffer-name-alist)))
            (phw-window-select
             (cdr (assoc (phw-query-string "Select a window: "
                                           (mapcar 'car choices-buffer-name-alist))
                         choices-buffer-name-alist)))))))))

(defun phw-goto-window-compilation ()
  "Goto the phw compilation window `phw-compile-window'."
  (interactive)
  (when (and phw-minor-mode
...
(defun phw-select-phw-frame ()
  "Selects the `phw-frame' if PHW is activated - otherwise reports an error."
  (interactive)
  (if phw-minor-mode
      (progn
        (raise-frame phw-frame)
        (select-frame phw-frame))
    (phw-error "PHW is not avtive in any frame!")))

(defun phw-select-edit-window (&optional edit-window-number)
...
(defun phw-layout-window-sync (&optional phw-window-list)
...
Runs all functions registered in `phw-autocontrol/sync-fcn-register'.
Functions registered with a phw-buffer run only if that buffer is currently
displayed in an phw-window.

If PHW-WINDOWS-LIST is not nil then this list of phw-windows is used otherwise
it will be computed."
  (when (and phw-minor-mode
             (not (phw-windows-all-hidden))
             (phw-point-in-edit-window-number))
    ...
    (let ((visible-phw-buffers (phw-get-current-visible-phw-buffers phw-window-list)))
      (dolist (elem phw-autocontrol/sync-fcn-register)
        (when (or (null (cdr elem))
                  (member (phw-buffer-obj (symbol-value (cdr elem)))
                          visible-phw-buffers))
          (funcall (car elem) t))))))
...
`phw-last-compile-buffer-in-compile-window' correct so other functions
can use these variables."
  (when (and phw-minor-mode
             (equal (selected-frame) phw-frame))
    ;; We MUST not use here `phw-point-in-edit-window-number' because this
    ...
    (when (and (not (phw-point-in-dedicated-special-buffer))
               (not (equal (minibuffer-window phw-frame)
                           (selected-window)))
               (not (phw-point-in-compile-window)))
      (setq phw-last-edit-window-with-point (selected-window))
      (setq phw-last-source-buffer (current-buffer)))
    (if (phw-point-in-compile-window)
        (progn
          (setq phw-compile-window-was-selected-before-command t)
          (setq phw-last-compile-buffer-in-compile-window
                (current-buffer)))
      (setq phw-compile-window-was-selected-before-command nil)
      (setq phw-last-compile-buffer-in-compile-window nil))
...
- handling of `phw-maximize-phw-window-after-selection'"
  (if phw-layout-prevent-handle-compile-window-selection
      (setq phw-layout-prevent-handle-compile-window-selection nil)
    (when (and phw-minor-mode
               (equal (selected-frame) phw-frame)
               (member phw-compile-window-temporally-enlarge
               ...
               (phw-compile-window-live-p)
               (= (minibuffer-depth) 0))
      (cond ((and (phw-point-in-compile-window)
                  (not phw-compile-window-was-selected-before-command))
             (phw-layout-debug-error "phw-layout-post-command-hook: enlarge")
             (phw-toggle-compile-window-height 1)
             ;; now we change the window-start, so we see autom. more text
             ;; after the enlargement of the window.
;;              (let ((height-before (phw-window-full-height))
;;                    (height-after (phw-toggle-compile-window-height 1)))
;;                (set-window-start phw-compile-window
;;                                  (save-excursion
...
;;                                  t))
            ...
            ((and phw-compile-window-was-selected-before-command
                  (not (phw-point-in-compile-window)))
             (phw-layout-debug-error "phw-layout-post-command-hook: shrink")
             (phw-toggle-compile-window-height -1)))))
  (if phw-layout-prevent-handle-phw-window-selection
      (progn
        (setq phw-layout-prevent-handle-phw-window-selection nil))
    (when (and phw-minor-mode
               phw-maximize-phw-window-after-selection
               ...
               ;; first phw-window - so it seems that Emacs automatically
               ;; "clicks" into the first phw-window which then maximizes this
               ;; window which is definitively very annoying and a bug (with
               ;; right-layouts there is no problem, because the top/left-most
               ;; window is not a phw-tree-window and has therefore no special
               ;; mouse-1 binding!). But it's currently not sure if it is a
               ;; bug somewhere in PHW or in GNU Emacs - but for the moment i
               ...
               ;; report it - maybe we have to write a small PHW-independent
               ;; code-example which reproduces this bug/behavior?!!

               ;; In the meanwhile we allow automatic maximizing only when
               ;; `phw-tree-mouse-action-trigger' is 'button-press!
               (or phw-running-xemacs
                   ;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: rename this so
                   ;; the name reflects the logic >=22!!!
                   phw-running-gnu-emacs-version-22
                   (equal phw-tree-mouse-action-trigger 'button-press))
               (equal (selected-frame) phw-frame)
               (= (minibuffer-depth) 0))
      (cond ((and (not (equal phw-phw-buffer-name-selected-before-command
                              (buffer-name)))
                  (phw-point-in-dedicated-special-buffer))
             (phw-maximize-phw-buffer (buffer-name) t))
            ((and phw-phw-buffer-name-selected-before-command
...
;; automatically taken into account by phw-check-for-special-buffer because
...
(defun phw-check-for-special-buffer (buffer-or-name)
...
`special-display-regexps'."
  (let ((result
         (if (phw-ignore-special-display)
             nil
           (catch 'done
             (let ((buf-name (phw-buffer-name buffer-or-name)))
                  ...
                  (while tem
                    (let ((car (car tem)))
                      (if (or
                          ...
                          (throw 'done t)))
                    (setq tem (cdr tem)))))))))
     (phw-layout-debug-error "phw-check-for-same-window-buffer for %s: %s"
                             buffer-or-name result)
     result))
...
`phw-canonical-edit-windows-list' and `phw-canonical-phw-windows-list'. This
...
(when-phw-running-emacs-22
 (defphw-advice get-largest-window before phw-layout-basic-adviced-functions
   "When called from within the `phw-frame' then DEDICATED is always set to nil.
So never a dedicated window is returned during activated PHW."
   (phw-layout-debug-error "get-largest-window for frame:%s, dedicated:%s"
                           (ad-get-arg 0) (ad-get-arg 1))
   (and (boundp 'phw-minor-mode)
        phw-minor-mode
 ...
 (defphw-advice get-lru-window before phw-layout-basic-adviced-functions
   "When called from within the `phw-frame' then DEDICATED is always set to nil.
So never a dedicated window is returned during activated PHW."
   (phw-layout-debug-error "get-lru-window for frame:%s, dedicated:%s"
                           (ad-get-arg 0) (ad-get-arg 1))
   (and (boundp 'phw-minor-mode)
        phw-minor-mode
        (eq (selected-frame) phw-frame)
...
(defphw-advice display-buffer around phw-layout-basic-adviced-functions
  "Makes this function compatible with PHW if called in or for the phw-frame.
It displays all buffers which are \"compilation-buffers\" in the sense of
`phw-compilation-buffer-p' in the compile-window of PHW. If the compile-window
is temporally hidden then it will be displayed first.

If there is no compile-window \(`phw-compile-window-height' is nil) then it
...
`phw-compilation-buffer-p' will be displayed in one of the edit-area and
...
`phw-ignore-special-display'. The values of `same-window-buffer-names' and
`same-window-regexps' are supported as well.

See the value of the option `phw-ignore-display-buffer-function'!

If called for other frames it works like the original version."
  (if phw-running-xemacs
      (phw-layout-debug-error "display-buffer entered with: %s %s %s %s"
                              (ad-get-arg 0)
                              (ad-get-arg 1)
                              (ad-get-arg 2)
                              (ad-get-arg 3))
    (phw-layout-debug-error "display-buffer entered with: %s %s %s"
                            (ad-get-arg 0)
                            (ad-get-arg 1)
                            (ad-get-arg 2)))
  (if (and phw-minor-mode
           (or (and (ad-get-arg 2)
                    (framep (ad-get-arg 2))
                    (equal (ad-get-arg 2) phw-frame))
               (and (or (null (ad-get-arg 2))
                        (equal (ad-get-arg 2) t)
                        (equal (ad-get-arg 2) 0))
                    (equal (selected-frame) phw-frame)))
           (not (phw-check-for-special-buffer (ad-get-arg 0)))
           (not (and (boundp 'display-buffer-function)
                     (fboundp display-buffer-function)
                     (not (phw-ignore-display-buffer-function)))))
      (let ((special-display-function (if (phw-ignore-special-display)
                                          nil
                                        special-display-function)))
        (cond ((phw-compilation-buffer-p (ad-get-arg 0))
               (phw-layout-debug-error "display-buffer for a comp-buffer: %s"
                                       (ad-get-arg 0))
               ;; we have to display the buffer in the compile-window if a
               ;; compile-window was set but currently hidden --> then we have
               ;; to show it now. `phw-toggle-compile-window' preserves always
               ;; the selected window!
               (when (and (equal 'hidden (phw-compile-window-state))
                          ;; calling this from minibuffer (e.g. completions)
                          ;; seems to cause problems
                          (not (equal (minibuffer-window phw-frame) (selected-window))))
                 (phw-layout-debug-error "display-buffer: comp-win will be toggled.")
                 (save-excursion (phw-toggle-compile-window 1)))
               (if (phw-compile-window-live-p)
                   ;; now we have to make the edit-window(s) dedicated
                   (let ((edit-window-list (phw-canonical-edit-windows-list))
                         (pop-up-frames (if (phw-ignore-pop-up-frames)
                                            nil
                                          pop-up-frames)))
                     (phw-layout-debug-error "display-buffer: buffer %s has to be displayed in the alive compile-window: %s"
                                             (ad-get-arg 0) phw-compile-window)
                     (unwind-protect
                         (progn
                           (mapc (function (lambda (w)
                                             (set-window-dedicated-p w t)))
                                 edit-window-list)
                           (setq phw-layout-temporary-dedicated-windows
                                 edit-window-list)
                           ...
                           ;; make the phw-frame unsplittable - but here we
                       ...
                       (mapc (function (lambda (w)
                             ...
                             edit-window-list)
                       ;;(modify-frame-parameters phw-frame '((unsplittable . nil)))
                       (setq phw-layout-temporary-dedicated-windows nil)) ;; end unwind-protect

                     ;; if called interactively we run now our
                     ;; `phw-toggle-compile-window-height' to set the height of
                     ;; the compile-window according to the value of
                     ;; `phw-enlarged-compilation-window-max-height'. If called
                     ...
                     ;; algorithm of `phw-toggle-compile-window-height' fails
                 ...
                 (let ((pop-up-frames (if (phw-ignore-pop-up-frames)
                                          nil
                                        pop-up-frames)))
                   ...
                   (when (and (not phw-running-gnu-emacs-version-23)
                              ...
                              (not pop-up-frames)
                              (not (phw-edit-window-splitted))
                              (not (phw-check-for-same-window-buffer (ad-get-arg 0))))
                     (phw-layout-debug-error "display-buffer for comp-buffer %s - split edit-window:"
                                             (ad-get-arg 0))
                     (split-window (car (phw-canonical-edit-windows-list))))
               ...
               (let ((edit-win-list (phw-canonical-edit-windows-list))
			    ...
			    (or pop-up-windows (ad-get-arg 1))
                            ...
                            (not (and (not (ad-get-arg 1))
                                      (member (get-buffer-window (ad-get-arg 0) phw-frame)
                                              edit-win-list)))
                            ...
                            (phw-point-in-edit-window-number edit-win-list)
                            (not (phw-check-for-same-window-buffer (ad-get-arg 0))))
                   (phw-layout-debug-error "display-buffer for normal-buffer %s - split edit-window:"
                                           (ad-get-arg 0))
                   (split-window (car edit-win-list)))
                 (if (phw-compile-window-live-p)
                     (unwind-protect
                         (progn
                           (set-window-dedicated-p phw-compile-window t)
                           ;; now we perform the original `display-buffer' but
                           ;; now the only not dedicated window(s) are the
                           ;; edit-window(s)
                           (setq phw-layout-temporary-dedicated-windows
                                 (list phw-compile-window))
                           ad-do-it)
...
(defun phw-get-other-window-minibuf-active (win-list
                                            edit-win-list
                                            phw-win-list
                                            comp-win
                                            minibuf-win
    ...
    (if (equal (car point-loc) 'minibuf)
        (if (= nth-win 1)
            (or (if (and minibuffer-scroll-window
                ...
                comp-win
                phw-last-edit-window-with-point)
          (phw-next-listelem (append edit-win-list
                                     (if comp-win
                                         (list comp-win))
                                     (if minibuf-win
                                         (list minibuf-win)))
...
(defun phw-get-other-window-smart (win-list
                                   ...
                                   point-loc
                                   nth-window)
  "Implements the smart-setting of `phw-other-window-behavior'."
  (if minibuf-win
      ;; if we have an active mini-buffer we delegate this to
      ;; `phw-get-other-window-minibuf-active'
      (phw-get-other-window-minibuf-active win-list
                                           edit-win-list
                                           phw-win-list
                                           comp-win
                                           minibuf-win
    ...
    (let ((nth-win (or nth-window 1)))
      (case (car point-loc)
        (phw
         (phw-next-listelem (if (and phw-win-list
                                     (= 1 (length phw-win-list)))
                                (append phw-win-list edit-win-list)
                              phw-win-list)
                            (selected-window) nth-win))
        (compile
         (if (= nth-win 1)
             (or (and phw-last-edit-window-with-point
                      (window-live-p phw-last-edit-window-with-point)
                      phw-last-edit-window-with-point)
                 (car edit-win-list))
           (phw-next-listelem (append edit-win-list
...
(defun phw-get-other-window (nth-window)
  "Return the \"other window\" according to `phw-other-window-behavior'.
Returns the window NTH-WINDOW steps away from the current window. If
NTH-WINDOW is nil then it is treated as 1."
  (let* ((nth-win (or nth-window 1))
         (windows-list (phw-canonical-windows-list))
         (edit-win-list (phw-canonical-edit-windows-list windows-list))
         (phw-win-list (phw-canonical-phw-windows-list windows-list))
         (point-loc (phw-where-is-point windows-list))
         (compwin-state (phw-compile-window-state))
         (minibuf-win (if (> (minibuffer-depth) 0)
                          (minibuffer-window phw-frame))))
    (if (functionp phw-other-window-behavior)
        (let ((other-win (funcall phw-other-window-behavior
                                  windows-list
                                  edit-win-list
                                  phw-win-list
          ...
          (if (and other-win (window-live-p other-win))
              other-win
            (phw-next-listelem (append windows-list (list minibuf-win))
                               (selected-window) nth-win)))
      (if minibuf-win
          (phw-get-other-window-minibuf-active windows-list
                                               edit-win-list
                                               phw-win-list
                                               (if (equal compwin-state 'visible)
                                                   phw-compile-window)
        ...
        (case phw-other-window-behavior
          (all
           (phw-next-listelem windows-list
                              (selected-window) nth-win))
          (only-edit
           (if (not (equal 'edit (car point-loc))) ;; point not in an edit-window
             ...
             (phw-next-listelem edit-win-list
                                (selected-window) nth-win)))
          (edit-and-compile
           (if (member (car point-loc) '(phw other-dedicated))
               (phw-next-listelem windows-list
                                  (selected-window) nth-win)
             ...
             (phw-next-listelem (append edit-win-list
                                ...
                                nth-win)))
          (otherwise ;; = 'smart
           (phw-get-other-window-smart windows-list
                                       edit-win-list
                                       phw-win-list
                                       (if (equal compwin-state 'visible)
                                           phw-compile-window)
...
;; (phw-get-other-window-smart (phw-canonical-windows-list) (phw-canonical-edit-windows-list) (phw-canonical-phw-windows-list) nil nil '(other-dedicated . 2) 1)


(defphw-advice other-window around phw-layout-basic-adviced-functions
  "The PHW-version of `other-window'. Works exactly like the original function
with the following PHW-adjustment: The behavior depends on
`phw-other-window-behavior'."
  (if (or (not phw-minor-mode)
          (not (equal (selected-frame) phw-frame)))
      (phw-with-original-basic-functions
    ...
    (let* ((count (if (ad-get-arg 0)
                      (ad-get-arg 0)
                    1))
           (o-w (phw-get-other-window count)))
      (select-window o-w))))


(defphw-advice delete-windows-on around phw-layout-basic-adviced-functions
  "The PHW-version of `delete-windows-on'. Works exactly like the original
function with the following PHW-adjustment:

An error is reported if BUFFER is a special PHW-buffer. These windows are not
allowed to be deleted."
  (let ((curr-frame (selected-frame))
        (buf-name (phw-buffer-name (ad-get-arg 0)))
...
(defsubst phw-edit-area-creators-init ()
  (setq phw-edit-area-creators nil))

(defsubst phw-edit-area-creators-add (creator)
  (setq phw-edit-area-creators (append phw-edit-area-creators (list creator))))

(defun phw-restore-edit-area ()
  (dolist (elem phw-edit-area-creators)
    (let ((edit-win-list (phw-canonical-edit-windows-list)))
      (select-window (nth (car elem) edit-win-list))
      (funcall (cdr elem)))))

(defun phw-edit-area-creators-number-of-edit-windows ()
...
been deleted. This is done even when PHW is deactivated so PHW can later
          ...
          (when edit-win-number
            (if (or (= (length edit-win-list) 1)
              ...
              (if (= (length edit-win-list) 2)
                ...
                ;; one edit-window.
                (phw-edit-area-creators-add (cons edit-win-number
                                                  'delete-window))))))
      (error (phw-warning "Before-advice delete-window (error-type: %S, error-data: %S)"
                          (car oops) (cdr oops))))))

(defphw-advice delete-window around phw-layout-basic-adviced-functions
  "The PHW-version of `delete-window'. Works exactly like the original
function with the following PHW-adjustment:
...
another one. If called in the compile-window of PHW then the compile-window
will be hidden \(like with `phw-toggle-compile-window'). If called in an
unsplitted edit-window then nothing is done. If called in an phw-window of the
current PHW-layout there are two alternatives:
- If the function is contained in `phw-layout-always-operate-in-edit-window'
  the right edit-window is selected \(depends on the value of the option
  `phw-mouse-click-destination') and does then itﾴs job.
- Otherwise the behavior depends on the value of the option
  `phw-advice-window-functions-signal-error'.
...
`phw-advice-window-functions-signal-error'."
  (if (or (not phw-minor-mode)
          ...
          ;; deletion.
          (and (phw-windows-all-hidden)
               (not (phw-compile-window-live-p)))
          ;; if all windows are dedicated (i.e. there is no edit-window left)
          ;; we allow deletion of all other windows (incl. phw-windows and
          ;; compile-window
          ;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: not perfect but...
          (null (phw-canonical-edit-windows-list))
          )
      (phw-with-original-basic-functions
       ad-do-it)
...
have been deleted. This is done even when PHW is deactivated so PHW can later
restore the window-layout as if before activation. Normally there can not
occur an error in this advice but nevertheless this advice is constructed
error-save, i.e. if an error occurs during this before-advice then it will be
reported but `delete-window' will be executed correctly."
  (when (and (equal (window-frame (or (ad-get-arg 0) (selected-window)))
                    phw-frame)
             ;; The case when phw is active is handled completely by
             ;; delete-window!
             (not phw-minor-mode))
    (condition-case oops
        (let* ((edit-win-list (phw-canonical-edit-windows-list))
          ...
          (when edit-win-number
            ...
            (phw-edit-area-creators-init)))
      (error (phw-warning "Before-advice delete-other-windows (error-type: %S, error-data: %S)"
                          (car oops) (cdr oops))))))

(defphw-advice delete-other-windows around phw-layout-basic-adviced-functions
  "The PHW-version of `delete-other-windows'. Works exactly like the
...
phw-windows nor the compile-window will be destroyed!
- If called in an unsplitted edit-window then either the compile-window will
  be hidden \(if there is one) or nothing is done.
- If called in one of the phw-windows then the current one is maximized, i.e.
  the other phw-windows \(not the edit-windows!) are deleted.
- If called in the compile window there are two
  alternatives:
  + If the function is contained in `phw-layout-always-operate-in-edit-window'
    the right edit-window is selected \(depends on the value of the option
    `phw-mouse-click-destination') and then it does itﾴs job.
  + Otherwise the behavior depends on the value of the option
  `phw-advice-window-functions-signal-error'.
...
If called in any other window of the current PHW-layout it stops with an error
...
(defphw-advice split-window-vertically around phw-layout-basic-adviced-functions
  "The PHW-version of `split-window-vertically'. Works exactly like the
original function with the following PHW-adjustment:
...
(defphw-advice split-window before phw-permanent-adviced-layout-functions
  "Does nothing special but only storing the fact that the window is splitted.
This is done even when PHW is deactivated so PHW can later restore the
...
with the following PHW-adjustment:

If called for a not-edit-window in the `phw-frame' it stops with an error if
`split-window' is not contained in the option
`phw-layout-always-operate-in-edit-window'! Besides this \(e.g. called for a
window in another frame than the `phw-frame') it behaves like the original
version."
  (phw-layout-debug-error "split-window entered for window: %s" (ad-get-arg 0))
  (if (or (not phw-minor-mode)
          (not (equal (window-frame (or (ad-get-arg 0) (selected-window)))
                      phw-frame)))
      (phw-with-original-basic-functions
    ...
    (let* ((window (or (ad-get-arg 0) (selected-window)))
           (edit-win-number (phw-position window
                                          (phw-canonical-edit-windows-list))))
      (if edit-win-number
          ad-do-it
        (if phw-advice-window-functions-signal-error
            (phw-error "Only the edit-windows of PHW are split-able!")
          (setq ad-return-value (selected-window)))))))

(defphw-advice switch-to-buffer-other-window around phw-layout-basic-adviced-functions
  "The PHW-version of `switch-to-buffer-other-window'. Works exactly like the
original but with some adaptions for PHW so this function works in a
\"natural\" way:

If called in any special phw-window of the current PHW-layout then it goes
always to an edit-window \(which one depends on the setting in
`phw-mouse-click-destination') and then goes on as if called from this
edit-window.

If a compile-window is used \(i.e. `phw-compile-window-height' is not nil)
then \"compilation-buffers\" in the sense of `phw-compilation-buffer-p' are
...
for compilation-buffers \(if a compile-window is used, see above)."
  (if (or (not phw-minor-mode)
      ...
      (phw-with-original-basic-functions
       ad-do-it)
    (if (equal (car (phw-where-is-point)) 'phw)
        (phw-select-edit-window))
    (let ((pop-up-windows t)
...
(defphw-advice switch-to-buffer around phw-layout-basic-adviced-functions
  "The PHW-version of `switch-to-buffer'. Works exactly like the original but
with the following enhancements for PHW:

\"compilation-buffers\" in the sense of `phw-compilation-buffer-p' will be
displayed always in the compile-window of PHW \(if `phw-compile-window-height'
...
`phw-compilation-buffer-names', `phw-compilation-major-modes' or
`phw-compilation-predicates'.

If called for non \"compilation-buffers\" \(s.a.) from outside the edit-area
of PHW it behaves as if called from an edit-window if `switch-to-buffer' is
contained in the option `phw-layout-always-operate-in-edit-window'. Otherwise
an error is reported."
  (if (or (not phw-minor-mode)
          (not (equal (selected-frame) phw-frame)))
      (phw-with-original-basic-functions
       ad-do-it)
    (phw-layout-debug-error "switch-to-buffer buffer: %s" (ad-get-arg 0))
    (cond ((phw-compilation-buffer-p (ad-get-arg 0))
           ...
           ;; therefore point can still stay in an phw-window
           (if (equal (car (phw-where-is-point)) 'phw)
          ...
          (t ;; normal buffers
           (if (member (car (phw-where-is-point)) '(phw compile))
               (if (member 'switch-to-buffer phw-layout-always-operate-in-edit-window)
                   (phw-select-edit-window)
                 (phw-error "switch-to-buffer: Can only switch to %s in an edit-window!"
                            (ad-get-arg 0))))))
    ;; now we stay in the correct window
    (phw-with-original-basic-functions
     ad-do-it)
    (phw-layout-debug-error "switch-to-buffer curr-buffer: %s" (current-buffer))
    (when (phw-point-in-compile-window)
      (phw-layout-debug-error "switch-to-buffer curr-buffer: %s, curr window %s"
                              (current-buffer) (selected-window))
      ;; we set the height of the compile-window according to
      ;; `phw-enlarged-compilation-window-max-height'
      (phw-set-compile-window-height))))

(defphw-advice other-window-for-scrolling around phw-layout-basic-adviced-functions
...
If the option `phw-scroll-other-window-scrolls-compile-window' is not nil
\(maybe set by `phw-toggle-scroll-other-window-scrolls-compile') and a
compile-window is visible then always the current buffer in the compile-window
is scrolled!

Otherwise it depends completely on the setting in `phw-other-window-behavior'."
  (if (or (not phw-minor-mode)
          (not (equal (selected-frame) phw-frame))
          (and (equal (selected-window) (minibuffer-window phw-frame))
               minibuffer-scroll-window))
    ...
    (let* ((o-w-s-b 
            (if (and phw-scroll-other-window-scrolls-compile-window
                     ...
                     (equal 'visible (phw-compile-window-state)))
                (window-buffer phw-compile-window)
              (window-buffer (phw-get-other-window nil))))
...
called for other frames than for the `phw-frame' then act like the original.
This adviced version of `walk-windows' is not for direct usage therefore it is
added to `phw-always-disabled-advices' and therefore always disabled; use the
macro `phw-with-phw-advice' instead if you need this adviced version of
`walk-windows'!"
  (if (and phw-minor-mode
           (or (equal (ad-get-arg 2) phw-frame)
               (and (null (ad-get-arg 2))
                    (equal (selected-frame) phw-frame))))
      (progn
        (let ((phw-walk-windows-advice-proc (ad-get-arg 0)))
          (ad-with-originals 'walk-windows
            (walk-windows (function (lambda (w)
                                      (if (or (phw-buffer-is-phw-buffer-of-current-layout-p
                                               (window-buffer w))
                                              (equal w phw-compile-window))
                                          nil
                                        ;; for an edit-window we call the
                                        ;; original PROC
                                        (funcall phw-walk-windows-advice-proc w))))
                          (ad-get-arg 1)
                          (ad-get-arg 2)))))
    ad-do-it))

(defphw-advice balance-windows around phw-layout-basic-adviced-functions
  "When called in the `phw-frame' then only the edit-windows are balanced."
  (if (and phw-minor-mode
           (equal (selected-frame) phw-frame)
           (not (phw-windows-all-hidden)))
      (if phw-running-gnu-emacs-version-22
...
(defun phw-split-hor (amount &optional dont-switch-window use-frame)
...
multiplied with the current window-width \(frame-width if USE-FRAME is not nil)."
  (let ((abs-amout (phw-normalize-number amount (if use-frame
                                                    (frame-width)
                                                  (window-width)))))
    (phw-split-hor-abs abs-amout dont-switch-window)
    abs-amout))

(defun phw-split-hor-abs (amount &optional dont-switch-window)
  (split-window-horizontally amount)
  (if (not dont-switch-window)
      (select-window (next-window))))

(defun phw-split-ver (amount &optional dont-switch-window use-frame)
...
with the current window-height \(frame-height if USE-FRAME is not nil)."
  (let ((abs-amout (phw-normalize-number amount (if use-frame
                                                    (1- (frame-height))
                                                  (phw-window-full-height)))))
    (phw-split-ver-abs abs-amout dont-switch-window)
    abs-amout))

(defun phw-split-ver-abs (amount &optional dont-switch-window)
...
(defun phw-set-minor-mode-text ()
  (setq phw-minor-mode-text
        (if (phw-windows-all-hidden)
            (or (phw-option-get-value 'phw-minor-mode-text 'saved-value)
                (phw-option-get-value 'phw-minor-mode-text 'standard-value))
          "")))

;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: Remove this and rename
;; phw-toggle-phw-windows-new to phw-toggle-phw-windows when maximizing has
;; been rewritten with new mechanism and new hidding takes account of
;; maximized windows.
(defun phw-toggle-phw-windows (&optional arg)
  "Toggle visibility of the PHW-windows.
With prefix argument ARG, make visible if positive, otherwise invisible.
This has nothing to do with \(de)activating PHW but only affects the
visibility of the PHW windows. PHW minor mode remains active!"
  (interactive "P")
  (unless (or (not phw-minor-mode)
              (not (equal (selected-frame) phw-frame)))
    (let ((new-state (if (null arg)
      ...
      (if (not new-state)
          (progn
            (run-hooks 'phw-show-phw-windows-before-hook)
            (if (phw-show-any-node-info-by-mouse-moving-p)
                (tree-buffer-activate-follow-mouse))
            ;; if `phw-buffer-is-maximized-p' is not nil then this means we
            ;; should only restore this one maximized buffer!
            (let ((compwin-hidden (equal 'hidden
                                         (phw-compile-window-state))))
              (if (phw-buffer-is-maximized-p)
                   (phw-maximize-phw-buffer (phw-maximized-phw-buffer-name))
                (phw-redraw-layout-full))
              (if compwin-hidden
                  (phw-toggle-compile-window -1)))
            (run-hooks 'phw-show-phw-windows-after-hook)
            (message "PHW windows are now visible."))
        (unless (phw-windows-all-hidden)
          (run-hooks 'phw-hide-phw-windows-before-hook)
          (tree-buffer-deactivate-follow-mouse)
          (let ((compwin-hidden (equal 'hidden
                                       (phw-compile-window-state))))
            (phw-redraw-layout-full nil nil nil phw-windows-hidden-all-value)
            (if compwin-hidden
                (phw-toggle-compile-window -1)))
          (run-hooks 'phw-hide-phw-windows-after-hook)
          (message "PHW windows are now hidden."))))))

(defun phw-toggle-phw-windows-new (&optional arg)
  "Toggle visibility of the PHW-windows.
For layout-type left-right the toggle sequence depends on the
value of the option `phw-left-right-layout-hide-sequence'. For
all other layout types toggling follows the intuitive way. With
prefix argument ARG you will be asked which phw-windows to hide
...
visibility of the PHW windows. PHW minor mode remains active!"
  ...
  (unless (or (not phw-minor-mode)
              (not (equal (selected-frame) phw-frame)))
    (let ((new-state (if (null arg)
                         (phw-windows-toggled-hidden-state)
                       (if (not (equal (phw-get-layout-type) 'left-right))
                           (phw-windows-toggled-hidden-state)
                         ;; ask for the new state
                         (let ((possible-hide-options
                                (mapcar (function (lambda (e)
                                                    (symbol-name e)))
                                        (phw-delete-first-occurence-from-list
                                         ;; phw-delete-first-occ... is destructive!
                                         (phw-copy-list '(none all left-side right-side))
                                         phw-windows-hidden-state))))
                           (intern (phw-query-string "PHW-windows to hide:"
                                                     possible-hide-options)))))))
      (phw-hide-phw-windows-internal new-state))))

(defun phw-hide-phw-windows-internal (new-state)
  "Make PHW-windows visible or invisible.
NEW-STATE must be one of the symbols none, all, left-side or right-side."
  ...
  (unless (or (not phw-minor-mode)
    ...
    (let ((old-state phw-windows-hidden-state)
          (compwin-hidden (equal 'hidden
                                 (phw-compile-window-state))))
      (unless (equal old-state new-state)
        (when (equal old-state phw-windows-hidden-all-value)
          ;; before all phw-windows were hidden, now we display at least some
          ;; phw-windows
          (run-hooks 'phw-show-phw-windows-before-hook)
          (if (phw-show-any-node-info-by-mouse-moving-p)
              (tree-buffer-activate-follow-mouse)))
        (when (equal new-state phw-windows-hidden-all-value)
          ;; before at least some phw-windows were displayed, now we hide all
          (run-hooks 'phw-hide-phw-windows-before-hook)
          (tree-buffer-deactivate-follow-mouse))
        (phw-redraw-layout-full nil nil nil new-state)
...
(defun phw-hide-phw-windows ()
  "Hide the PHW windows if not already hidden."
  (interactive)
  (phw-toggle-phw-windows 0))
;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: use this when new mechanism is ready
  ;; (phw-hide-phw-windows-internal phw-windows-hidden-all-value))

(defun phw-show-phw-windows ()
  "Make the PHW windows visible."
  (interactive)
  (phw-toggle-phw-windows 1))
;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: use this when new mechanism is ready
  ;; (phw-hide-phw-windows-internal phw-windows-hidden-none-value))


(defvar phw-current-maximized-phw-buffer-name nil
  "If not nil then it contains the buffer-name of the current maximized
phw-buffer. If nil then this means currently there is no phw-buffer maximized.

Do not set this variable. It is only set by `phw-redraw-layout-full' and
`phw-maximize-phw-buffer'.")

(defun phw-redraw-layout-preserving-compwin-state ()
  "Redraw current layout with all PHW-windows visible."
  (interactive)
  (let ((compwin-state (phw-compile-window-state)))
    (phw-redraw-layout-full)
    (if (equal compwin-state 'hidden)
        (phw-toggle-compile-window -1))))

(defun phw-toggle-maximize-phw-window-with-mouse ()
  "Mouse-wrapper for `phw-toggle-maximize-phw-window'."
  (interactive "@")
  (phw-toggle-maximize-phw-window))

(defun phw-toggle-maximize-phw-window (&optional phw-buffer-name)
  "Toggle maximizing the special phw-window of PHW-BUFFER-NAME.
If there is a maximizied phw-window then it will be \"minimized\", i.e. all
phw-windows of current layout will be displayed. If there is no phw-window
maximized then that of PHW-BUFFER-NAME will be maximized. If PHW-BUFFER-NAME
is nil then the current buffer-name is used. Nothing will be done if the
caller tries to maximize a non-phw-window."
  (if (phw-buffer-is-maximized-p)
      (phw-undo-maximize-phw-buffer)
    (let ((buf-name (or phw-buffer-name (buffer-name))))
      (when (phw-buffer-is-phw-buffer-of-current-layout-p buf-name)
        (phw-maximize-phw-buffer buf-name t)))))

(defun phw-undo-maximize-phw-buffer (&optional preserve-selected-window)
  "Undo the maximizing of an phw-buffer.
If optional arg PRESERVE-SELECTED-WINDOW is not nil then the currently
selected window does not change. Otherwise after displaying all phw-windows the
current edit-window is selected."
  (when (equal (selected-frame) phw-frame)
    (let ((curr-loc (phw-where-is-point))
          (prev-buffer-name (buffer-name)))
      (phw-redraw-layout-preserving-compwin-state)
      ...
      (when preserve-selected-window
        (case (car curr-loc)
          (phw
           (phw-window-select prev-buffer-name))
          (compile
           (phw-window-select phw-compile-window))))
      (phw-info-message "Maximizing has been undone."))))

(defun phw-maximized-phw-buffer-name ()
  "Return the currently maximized special phw-buffer-name or nil if there is none."
  phw-current-maximized-phw-buffer-name)

(defun phw-buffer-is-maximized-p (&optional phw-buffer-name)
  "Not nil if PHW-BUFFER-NAME is currently maximized.
If PHW-BUFFER-NAME is not nil and not part of the current layout then nil is
returned. If PHW-BUFFER-NAME is nil then not nil is returned if any special
buffer of current layout is maximized otherwise nil."
  (if phw-buffer-name
      (and (phw-buffer-is-phw-buffer-of-current-layout-p phw-buffer-name)
           (equal phw-buffer-name
                  (phw-maximized-phw-buffer-name)))
    (phw-maximized-phw-buffer-name)))

;; TODO: Klaus Berndl <klaus.berndl@sdm.de>:
;; change maximizing:
;; - throw away the creator-fcn stuff, now we do maximizing via deleting all
;;   other phw-windows of the same column (left, right, left-right) or row
;;   (top)..
;; - maximizing can be done in each column
;; - phw-maximized-phw-buffer-name can now hold a list of maximized
;;   buffer-names!!! CHECK WHERE THIS IS SET OR EVALUATED - adapt the code!!
;; - maximizing: get the list of phw-windows of that column (row) the window
...
;;   this list. Then set phw-maximized-phw-buffer-name and voila, done!
;; - undo-maximize can be done for each side separately in left-right-layouts!
;; - phw-hide-phw-windows should handle hidding only one side with left-right
;;   layouts.
;; - phw-toggle-maximize-phw-window-with-mouse should be checked
;; - phw-toggle-phw-windows should hanlde the new possibilities (hidding left
;;   --> hidding right too --> displaying all e.g.)
;; - calling delete-window (C-x 0) in a phw-window can allow deleting them -
...
3. Data of all edit-windows in form of a list: Everey listelement is a list
   ...
   `window-point' and fourth subelement is the result of `phw-get-window-size'
...
4. Data of the compile window or nil \(if there is no compile-window visible):
   ...
   compile-window.
5. The window sizes of the phw-windows as returned by
   `phw-get-phw-window-sizes'"
  (let* ((win-list (phw-canonical-windows-list))
         (edit-win-list (phw-canonical-edit-windows-list win-list))
         (phw-win-list (phw-canonical-phw-windows-list win-list))
         (point-pos (phw-where-is-point win-list))
         (edit-area-size (phw-get-edit-area-size win-list)))
    (list (if (equal 'edit (car point-pos)) (cdr point-pos))
          ...
          (mapcar (function (lambda (win)
                              (list (window-buffer win)
                                    (window-start win)
                                    (window-point win)
                                    (phw-get-window-size win nil edit-area-size))))
                  edit-win-list)
          (if (equal 'visible (phw-compile-window-state))
              (list (window-buffer phw-compile-window)
                    (if (equal (car point-pos) 'compile) (point))
                    (phw-window-full-height phw-compile-window)))
          (phw-get-phw-window-sizes nil phw-win-list)
          )))

;; =================== Helper functions ==================================

(defmacro phw-with-dedicated-window (&rest body)
...
;; 1. Phw-windows on the left side (include layouts with left and right side
;;    PHW-windows)
;; 2. Phw-windows on the right side
;; 3. Phw-windows on the top
...
;; 1. Phw-windows on the left side

(defun phw-delete-window-phw-windows-left (window edit-win-list)
  (let ((phw-win-width-before (window-width (frame-first-window phw-frame)))
        (curr-edit-win-width (phw-window-full-width window)))
    (delete-window window)
    (when (/= (window-width (frame-first-window phw-frame))
              phw-win-width-before)
...
(defalias 'phw-delete-window-phw-windows-left-right
  'phw-delete-window-phw-windows-left)

;; 2. Phw-windows on the right side

(defun phw-delete-window-phw-windows-right (window edit-win-list)
  (delete-window window))

;; 3. Phw-windows on the top

(defun phw-delete-window-phw-windows-top (window edit-win-list)
  (let ((phw-win-height-before (phw-window-full-height (frame-first-window phw-frame)))
        (curr-edit-win-height (phw-window-full-height window)))
    (delete-window window)
    (when (/= (phw-window-full-height (frame-first-window phw-frame))
              phw-win-height-before)
      (save-selected-window
        (select-window (phw-next-listelem edit-win-list window))
        (enlarge-window curr-edit-win-height)))))


(defconst phw-layout-types '(left right top left-right))

(defun phw-layout-type-p (type &optional err)
  (if (not (member type phw-layout-types))
      (if err
          (error "Only left, right, top and left-right are allowed as types!")
        nil)
    t))

(defvar phw-available-layouts nil
  "List of all current available layout names. Do not change this variable!
This variable is only modified by `phw-available-layouts-add' and
`phw-available-layouts-remove'. These functions are only called by
`phw-layout-define' and `phw-layout-undefine'!")

;; Accessors for `phw-available-layouts':
(defun phw-available-layouts-of-type (type)
  "Return a list of all layout-names for given type TYPE. Type must be an
element of `phw-layout-types' or nil \(then return all layout-names
regardless of the type)."
  (if type (phw-layout-type-p type t))
  (delete nil (mapcar (function (lambda (elem)
                                  (if (or (not type)
                                          (equal (cdr elem) type))
                                      (car elem))))
                      phw-available-layouts)))

(defun phw-available-layouts-member-p (layout-name)
  "Return a non nil value iff LAYOUT-NAME is the name of a layout of
`phw-available-layouts'."
  (member layout-name (phw-available-layouts-of-type nil)))

(defun phw-available-layouts-add (name type)
  "Add layout with NAME and TYPE to `phw-available-layouts'. NAME is a string
and TYPE must be an element of `phw-layout-types'."
  (add-to-list 'phw-available-layouts (cons name type))
  (setq phw-available-layouts
        (sort phw-available-layouts
              (function (lambda (l r)
                          (phw-string< (car l) (car r)))))))

(defun phw-available-layouts-remove (name)
  "Remove layout with NAME from `phw-available-layouts'."
  (let ((elem (assoc name phw-available-layouts)))
    (when elem
      (setq phw-available-layouts
            (sort (delete elem phw-available-layouts)
                  (function (lambda (l r)
                              (phw-string< (car l) (car r)))))))))

(defun phw-get-layout-type (&optional name)
  "Return the type of current layout or of layout NAME."
  (let ((n (or name phw-layout-name)))
    (cdr (assoc n phw-available-layouts))))

(defun phw-layout-left-p (&optional name)
  "Return not nil if current layout or layout NAME is of type left."
  (equal 'left (phw-get-layout-type name)))

(defun phw-layout-leftright-p (&optional name)
  "Return not nil if current layout or layout NAME is of type left-right."
  (equal 'left-right (phw-get-layout-type name)))

(defun phw-layout-right-p (&optional name)
  "Return not nil if current layout or layout NAME is of type right."
  (equal 'right (phw-get-layout-type name)))

(defun phw-layout-top-p (&optional name)
  "Return not nil if current layout or layout NAME is of type top."
  (equal 'top (phw-get-layout-type name)))

;; Macro for easy defining new layouts
(defmacro phw-layout-define (name type doc &rest create-code)
  "Creates a new PHW-layout with name NAME which must be a string. TYPE is the
...
\"phw-layout-function-<name>\". CREATE-CODE is all the lisp code which is
necessary to define the PHW-windows/buffers. This macro adds the layout with
NAME and TYPE to the internal variable `phw-available-layouts'.
...
1. Current frame is splitted at least in one edit-window and the column\(s)
   ...
   for the special PHW-windows/buffers. Depending on the value of the option
   `phw-compile-window-height' there is also a compile window at the bottom of
   the frame which is stored in `phw-compile-window'.
...
4. All PHW-advices for the functions in `phw-layout-basic-adviced-functions' are
   disabled!

Things CREATE-CODE has to do:
1. Splitting the PHW-windows-column\(s)/row \(s.a.) in all the PHW-windows the
   layout should contain \(e.g. directories, sources, methods and history).
   The split must not be done with other functions than `phw-split-hor' and
   `phw-split-ver'! It is recommended not to to use a \"hard\" number of
...
2. Making each special PHW-window a dedicated window. This can be done with
   ...
   registered at PHW and called by PHW to perform any arbitrary code in
   current window and makes the window autom. dedicated at the end. This can
   be used by third party packages like JDEE to create arbitrary PHW-windows
   besides the standard tree-windows.

   To make a special PHW-window a dedicated window for an
   phw-buffer either one of the seven functions above must be used
   or a new \"window-dedicator\"-function has to be defined with
   `defphw-window-dedicator-to-phw-buffer' and must be used within the
   layout-definition.

3. Every\(!) special PHW-window must be dedicated as described in 2.

4. CREATE-CODE must work correctly regardless if there is already a
   compile-window \(stored in `phw-compile-window') or not
   \(`phw-compile-window' is nil)

Things CREATE-CODE can do or can use:
1. Using the values of `phw-compile-window-height', `phw-windows-width',
   `phw-windows-height' and `phw-compile-window-width'.
...
3. Deleting the edit-window, the compile-window \(if there is any) or the
   PHW-windows-column\(s)/row \(see Precondition 1.)
4. Referring to the values of `phw-edit-window' or `phw-compile-window'
   because these values are always nil or undefined during CREATE-CODE.
5. Using the function `phw-compile-window-live-p'.
...
2. Every window besides the edit-window \(and the compile-window) must be
   a dedicated window \(e.g. a PHW-tree-window)."
  `(progn
     (phw-layout-type-p (quote ,type) t)
     (eval-and-compile
       (defun ,(intern (format "phw-layout-function-%s" name)) (&optional create-code-fcn)
         ,doc
         ;; Klaus Berndl <klaus.berndl@sdm.de>: creating the compile-window is
         ;; now done in `phw-redraw-layout-full'!
         ;; (when (and phw-compile-window-height
         ;;            (or (equal phw-compile-window-width 'frame)
         ;;                (equal (phw-get-layout-type phw-layout-name) 'top)))
         ;;   (phw-split-ver (- phw-compile-window-height) t t)
         ;;   (setq phw-compile-window (next-window)))
         ,(cond ((equal type 'left)
                 '(phw-split-hor phw-windows-width t))
                ((equal type 'right)
                 '(phw-split-hor (- phw-windows-width) nil))
                ((equal type 'top)
                 '(phw-split-ver phw-windows-height t))
                ((equal type 'left-right)
                 '(progn
                    (phw-split-hor (- phw-windows-width) t)
                    (phw-split-hor phw-windows-width t t))))
         ...
         ;; leftmost-topmost phw-window-column/bar is selected.
         (if (and create-code-fcn
                  (not (equal (phw-get-layout-type phw-layout-name) 'left-right)))
             (progn
       ...
       (defalias (quote ,(intern
                          (format "phw-delete-window-in-editwindow-%s"
                                  name)))
         (quote ,(intern
...
;;   )
(put 'phw-layout-define 'lisp-indent-function 2)

(defun phw-layout-undefine (name)
  "Unbind phw-layout-function-<NAME>, phw-delete-window-phw-windows-<NAME>,
phw-delete-other-windows-phw-windows-<NAME> and remove NAME from
`phw-available-layouts'."
  (fmakunbound (intern (format "phw-layout-function-%s" name)))
  (fmakunbound (intern (format "phw-delete-window-phw-windows-%s" name)))
  (phw-available-layouts-remove name))


(defun phw-choose-layout-name (layout-list require-match)
...
(defun phw-layout-switch (name)
  "Switch to layout with layout-name NAME."
  (let ((comp-win-state (phw-compile-window-state)))
    (customize-set-variable 'phw-layout-name name)
...
(defvar phw-window-config-cache-size 200)
(defvar phw-window-config-cache nil
  "A ring-cache for phw-window-configurations. Max. size is
`phw-window-config-cache-size'. If a new element is added and the cache is
already full then the oldest item is dumped to make room.")

(defun phw-window-config-cache-add (phw-window-config)
  "Add PHW-WINDOW-CONFIG to the `phw-window-config-cache'-ring. The new config
will be added as the newest \(last) item. If the cache is full \(see
`phw-window-config-cache-size'), dump the oldest item to make room."
  (phw-window-config-cache-init)
  (phw-ring-insert phw-window-config-cache phw-window-config))

(defun phw-window-config-cache-get (emacs-window-config)
...
If the cache does not contain such an element then nil is returned."
  (if (and (phw-ring-p phw-window-config-cache)
           (not (phw-ring-empty-p phw-window-config-cache)))
      (let ((configs (phw-ring-elements phw-window-config-cache)))
        (assq emacs-window-config configs))))

(defun phw-window-config-cache-get-latest ()
  "Get the newest \(last) cache-item."
  (if (and (phw-ring-p phw-window-config-cache)
           (not (phw-ring-empty-p phw-window-config-cache)))
      (phw-ring-ref phw-window-config-cache 0)))

(defun phw-window-config-cache-init ()
  "Initialize the cache as a ring of size `phw-window-config-cache-size' if
not already initialized."
  (or (phw-ring-p phw-window-config-cache)
      (setq phw-window-config-cache
            (phw-make-ring phw-window-config-cache-size))))

(defun phw-window-config-cache-clear ()
  "Clear the cache."
...
`current-window-configuration'."
  (not (equal (nth 3 window-config)
              (list phw-frame phw-layout-name phw-compile-window-height
                    phw-compile-window-width
                    phw-windows-width phw-windows-height))))


(defphw-advice current-window-configuration after phw-layout-basic-adviced-functions
  "Stores some additional informations about the window-configurations needed
by PHW."
  (condition-case oops
      (let ((f (or (ad-get-arg 0) (selected-frame))))
        (when (equal f phw-frame)
          (phw-window-config-cache-add
           (list ad-return-value
                 (if (phw-windows-all-hidden)
                     nil
                   (phw-get-current-visible-phw-buffers))
                 (if (phw-compile-window-live-p)
                     (phw-position phw-compile-window
                                   (phw-canonical-windows-list)))
                 ;; We add here as first element `phw-frame' and also in the
                 ;; check of `phw-window-configuration-invalidp'! Then a
                 ;; phw-window-config made from a frame which is now deleted
                 ...
                 ;; `phw-window-configuration-invalidp'!
                 (list phw-frame phw-layout-name phw-compile-window-height
                       phw-compile-window-width
                       phw-windows-width phw-windows-height)
                 phw-edit-area-creators
                 phw-windows-hidden-state
                 (phw-window-configuration-data)))))
    (error
     (phw-layout-debug-error "advice of current-window-configuration failed: (error-type: %S, error-data: %S)"
                             (car oops) (cdr oops))))
  ad-return-value)


(defphw-advice set-window-configuration after phw-layout-basic-adviced-functions
  "Resets some internal window-configuration-states needed by PHW. These
internal PHW-states were stored by `current-window-configuration' in a
ring-cache as add-on to CONFIGURATION."
  (condition-case oops
      (when (equal (selected-frame) phw-frame)
        (let ((config (phw-window-config-cache-get (ad-get-arg 0))))
          (when (and config
                     (not (phw-window-configuration-invalidp config)))
            ...
            ;; In addition we have to reset phw-compile-window and also to set
            ;; phw-windows-hidden-state correctly
            (and (nth 1 config)
                 (phw-set-windows-dedicated-state (nth 1 config) t))
            (when (nth 2 config)
              (let ((win-list (phw-canonical-windows-list)))
                (and phw-compile-window-height
                     (setq phw-compile-window (nth (nth 2 config) win-list)))))
            ;; (nth 3 config) is not used and needed within this function!
            (setq phw-edit-area-creators (nth 4 config))
            (setq phw-windows-hidden-state (nth 5 config))
            (phw-set-minor-mode-text))))
    (error
     (phw-layout-debug-error "advice of set-window-configuration failed: (error-type: %S, error-data: %S)"
                             (car oops) (cdr oops))))
  ad-return-value)

(when-phw-running-xemacs
 (defphw-advice set-window-configuration/mapping after phw-layout-basic-adviced-functions
   "If `set-window-configuration' changes the values of `phw-edit-window',
`phw-last-edit-window-with-point' or `phw-compile-window', this advice reset
them to the new values to allow phw to run at all in XEmacs 21.5"
   (let ((edit-window-changed (assq phw-edit-window ad-return-value))
         (last-edit-window-with-point-changed (assq phw-last-edit-window-with-point ad-return-value))
         (compile-window-changed (assq phw-compile-window ad-return-value)))
     (if edit-window-changed
         (setq phw-edit-window (cdr edit-window-changed)))
     (if last-edit-window-with-point-changed
         (setq phw-last-edit-window-with-point (cdr last-edit-window-with-point-changed)))
     (if compile-window-changed
         (setq phw-compile-window (cdr compile-window-changed)))))
 )


(defun phw-current-window-configuration ()
  "Return the current phw-window-configuration"
  (progn
    (current-window-configuration)
    (phw-window-config-cache-get-latest)))

(defun phw-set-window-configuration (phw-window-config)
  "Sets the window-configuration of PHW-WINDOW-CONFIG. The additional
informations needed by PHW will be set by the adviced version of
`set-window-configuration'."
  (set-window-configuration (car phw-window-config)))

(defmacro phw-save-window-excursion (&rest body)
  "Same as `save-window-excursion' but it takes care of the PHW-needs."
  (let ((current-window-config (make-symbol "curr-win-conf")))
    `(let ((,current-window-config (phw-current-window-configuration)))
       (unwind-protect
           (progn
             ,@body)
         (phw-set-window-configuration ,current-window-config)))))
...
;;     (phw-with-original-basic-functions
;;      (delete-other-windows))
;;     ;; set phw-compile-window and phw-edit-area-creators to some crap. If the
;;     ;; test is ok, then the adviced set-window-configuration must reset these
;;     ;; variables correctly!
;;     (setq phw-compile-window nil)
;;     (setq phw-edit-area-creators '(1 2 3 4))
;;     (set-window-configuration config))

;; run `phw-test-store' when some splitted layout is active. Then do anything
;; what you want to change the split-state of the layout and/or the
;; visibility-state of the compile-window. Then run `phw-test-restore': This
;; has to bring back completely the stored configuration!
;;   (defvar phw-test-config nil)
;;   (defun phw-test-store ()
;;     (interactive)
;;     (setq phw-test-config (current-window-configuration)))
;;   (defun phw-test-restore ()
;;     (interactive)
;;     (set-window-configuration phw-test-config))


;; redrawing the layout

(defun phw-redraw-layout (&optional arg)
  "Redraw the PHW screen.

Do not call this command from elisp-program but only interactively!

Called without a prefix-argument the state of the PHW-frame-layout will
preserved. This means:
- The state of compile-window \(hidden or visible) will be preserved but if
  visible then the height will be as specified in `phw-compile-window-height'.
- The state of the PHW-windows will be preserved \(hidden or visible) but if
  visible then the sizes will be as specified in the layout \(and with the
  options `phw-windows-width' and `phw-windows-height') or as stored with
  `phw-store-window-sizes'.

If called with ONE prefix-argument \(\[C-u]) then the layout will be drawn
with all PHW-windows and also with a visible compile-window \(when
`phw-compile-window-height' is not nil). The splitting-state of the edit-area
will be preserved.
...
If the variable `phw-redraw-layout-quickly' is not nil then the redraw is done
by the `phw-redraw-layout-quickly' function, otherwise by
`phw-redraw-layout-full'.
  ...
  (when (and phw-minor-mode
    ...
    (let ((compwin-hidden (equal (phw-compile-window-state) 'hidden)))
      ...
      (if (and phw-redraw-layout-quickly
          ...
          (condition-case nil
              (phw-redraw-layout-quickly)
            (error (message "PHW: Quick redraw failed...full redraw will be performed!")
...
(defun phw-repair-only-phw-window-layout ()
  ...
  ;; 2. The phw-windows are not hidden
  ;; 3. there is no phw-window maximized
...
If PHW-WINDOW is nil then the location of the selected window is returned
otherwise the location of PHW-WINDOW. If RESIDUAL-WINDOW is not nil it must be
one of the windows `phw-canonical-residual-windows-list' would compute. If nil
then it will be computed.

Caution: This function does not check if PHW-WINDOW \(rsp. the selected
window) is an phw-window! But only in this case the returned value is reliable!"
  (let* ((phw-win (or phw-window (selected-window)))
         (layout-type (phw-get-layout-type))
         (res-win (or residual-window
                      (car (phw-canonical-residual-windows-list)))))
    (if (and (windowp phw-win)
             (equal phw-frame (window-frame phw-win))
             (windowp res-win))
        (case layout-type
          (left 'left-side)
          (right 'right-side)
          (top 'top-side)
          (left-right (if (> (nth 0 (phw-window-edges res-win))
...
`phw-compile-window-height-lines'. If HEIGHT is not nil then the
compile-window will drawn with height HEIGHT otherwise
`phw-compile-window-height' is used."
  (phw-split-ver (- phw-compile-window-height) t t)
  (setq phw-compile-window (next-window))
  ...
  ;; specified height.
  (setq phw-compile-window-height-lines
        (phw-window-full-height phw-compile-window))
  ...
  ;; is called so the phw-windows will be created also with correct
...
(defun phw-redraw-layout-full (&optional no-buffer-sync phw-windows-creator
                                         window-configuration-data
                                         no-phw-windows emergency)
  "Redraw the PHW screen according to the layout set in `phw-layout-name'. After
this function the edit-window is selected which was current
before redrawing. If NO-BUFFER-SYNC is not nil then the
phw-buffers will not be synchronized. If PHW-WINDOWS-CREATOR is
...
be an object returned by `phw-window-configuration-data' and will
be used for restoring the layout. If set NO-PHW-WINDOWS must be
of the same type as `phw-windows-hidden-state' - see the
...
contain the buffer before the emergency-redraw."
  (when (and phw-minor-mode
    ...
    (let* ((config (or window-configuration-data (phw-window-configuration-data)))
           ...
           (phw-buffers-before-redraw (phw-get-current-visible-phw-buffers))
           (edit-area-size nil)
           (edit-win-list-after-redraw nil))

      (phw-layout-debug-error "phw-redraw-layout-full: config: %s, hidden-state: %s, curr-buff: %s, last-source-buff: %s"
                              config phw-windows-hidden-state (current-buffer)
                              phw-last-source-buffer)

      (unless no-phw-windows
        (setq no-phw-windows phw-windows-hidden-none-value))
       ...
       (phw-with-original-permanent-layout-functions
        ...
        (if emergency
            (progn
              ...
              (setq phw-windows-hidden nil)
              (setq phw-windows-hidden-state phw-windows-hidden-none-value))
          ;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: remove this later
          (setq phw-windows-hidden no-phw-windows)
          (setq phw-windows-hidden-state no-phw-windows))
        (phw-set-minor-mode-text)

        ;; Now all the windows must be created and the editing window must not
        ;; be splitted! In addition the variables `phw-edit-window' and
        ;; `phw-compile-window' must be set to the correct windows.

        ;; The following when-expression is added for better relayouting the
        ;; chosen layout if we have a compilation-window.
        (when phw-compile-window-height
          (if (not (phw-compile-window-live-p))
              (error "Compilation-window not set in the layout-function"))
          (set-window-buffer
           phw-compile-window
           (or (and compile-buffer-before-redraw
                    (phw-compilation-buffer-p compile-buffer-before-redraw))
               (phw-some (function (lambda (buf)
                                     (and (not (phw-string= "*Completions*"
                                                            (phw-buffer-name buf)))
                                          (not (phw-check-for-special-buffer buf))
                                          (phw-compilation-buffer-p buf))))
                         (buffer-list phw-frame))
               (get-buffer-create "*scratch*"))))

        ;; Only the function `phw-redraw-layout-full' is allowed to refer and
        ;; set the variable `phw-edit-window' but not after the following
        ;; code-line. The only exception is that every layout-function must set
        ;; `phw-edit-window' and `phw-compile-window' at its end (but because
        ;; most layout-function will be generated with `phw-layout-define' this
        ;; is not really an exception).
        (select-window phw-edit-window))) ;; end of phw-with-original-basic-functions,
                                          ;; phw-with-original-permanent-layout-functions
          ...
          (dotimes (i (length edit-win-data-before-redraw))
            (let ((win (nth i edit-win-list-after-redraw))
              ...
              (if (> (length edit-win-list-after-redraw) 1)
                  (phw-set-window-size win (nth 3 data) edit-area-size))
              )))
      ...
      (when (or emergency
                (and (null phw-windows-creator)
                     (not (phw-windows-all-hidden no-phw-windows))))
        (phw-restore-window-sizes))

      (setq phw-last-source-buffer (current-buffer))
      (setq phw-last-edit-window-with-point (selected-window))

      ;; updating and synchronizing of the phw-windows
      (let ((current-phw-buffers (phw-get-current-visible-phw-buffers)))
        ;; phw-special-phw-buffers-of-current-layout must no be set if some
...
`phw-activated-window-configuration'."
  (when (and phw-minor-mode
             (equal (selected-frame) phw-frame))
    (run-hooks 'phw-redraw-layout-before-hook)
    (phw-set-window-configuration phw-activated-window-configuration)
    (run-hooks 'phw-redraw-layout-after-hook)))


(defvar phw-toggle-layout-state 0
  "Internal state of `phw-toggle-layout'. Do not change it!")
(defun phw-toggle-layout (&optional last-one)
  "Toggles between the layouts defined in `phw-toggle-layout-sequence'.
...
`phw-toggle-layout-sequence'. The last selected layout is always that layout
which was current direct before the most recent layout-switch. So now a user
can switch to another layout via `phw-change-layout' and always come back to
his previous layout via \[C-u] `phw-toggle-layout'."
  (interactive "P")
  (if (and last-one
           (stringp phw-last-selected-layout))
      (phw-layout-switch phw-last-selected-layout)
    (let ((layout-name (nth phw-toggle-layout-state phw-toggle-layout-sequence))
          (next-index (if (< (1+ phw-toggle-layout-state)
                             (length phw-toggle-layout-sequence))
                          (1+ phw-toggle-layout-state)
                        0)))
      (when (and layout-name (not (= phw-toggle-layout-state next-index)))
        (setq phw-toggle-layout-state next-index)
        (phw-layout-switch layout-name)))))

(defun phw-store-window-sizes (&optional fix)
  "Stores the sizes of the PHW windows for the current layout.
The size of the PHW windows will be set to their stored values when
`phw-redraw-layout' or `phw-restore-window-sizes' is called. To reset the
window sizes to their default values call `phw-restore-default-window-sizes'.
Please read also the documentation of `phw-layout-window-sizes'!

The windows sizes are stored per default as fractions of current frame-width
and -height of the phw-frame, so the stored values will \"work\" for other
frame sizes too. If a permanent compile-window is visible then PHW will tell
...
has its standard-height as specified in `phw-compile-window-height'!. If FIX
is not nil \(means called with a prefix argument) then always the fixed values
of current width and height are stored!"
  (interactive "P")
  (when (equal (selected-frame) phw-frame)
    (if (or (phw-buffer-is-maximized-p)
            (not (phw-windows-all-displayed)))
         (phw-error "Sizes can only be stored if the complete layout is visible!")
      (if (or (not (phw-compile-window-live-p))
              (y-or-n-p "Window-sizes should be stored with hidden compile-window! Proceed? "))
          (let ((a (phw-find-assoc phw-layout-name phw-layout-window-sizes)))
            (unless a
              (setq a (cons phw-layout-name nil))
              (setq phw-layout-window-sizes
                    (phw-add-assoc a phw-layout-window-sizes)))
            (setcdr a (phw-get-phw-window-sizes fix))
            (phw-customize-save-variable 'phw-layout-window-sizes
                                         phw-layout-window-sizes))))))


(defun phw-restore-window-sizes ()
  "Sets the sizes of the PHW windows to their stored values."
  (interactive)
  (when (equal (selected-frame) phw-frame)
    (phw-set-phw-window-sizes (phw-find-assoc-value phw-layout-name
                                                    phw-layout-window-sizes))))

(defun phw-restore-default-window-sizes ()
  "Resets the sizes of the PHW windows to their default values."
  (interactive)
  (when (equal (selected-frame) phw-frame)
    (setq phw-layout-window-sizes
...
(defun phw-get-window-size (window &optional fix ref-size)
...
height) of the `phw-frame' unless FIX is not nil. Optional third argument
...
`frame-height' of the `phw-frame'."
  (when window
    (let ((ref-width (or (car ref-size) (frame-width phw-frame)))
          (ref-height (or (cdr ref-size) (frame-height phw-frame))))
      (cons (/ (phw-window-full-width window)
               (if fix
                   1
                 (* 1.0 ref-width)))
            (/ (phw-window-full-height window)
               (if fix
                   1
                 (* 1.0 ref-height)))))))


(defun phw-get-phw-window-sizes (&optional fix phw-win-list)
  "Get all window-sizes of current visible phw-windows. If FIX is not nil then
...
Uses PHW-WIN-LIST or - if nil - computes it with the function
`phw-canonical-phw-windows-list'.

Result is a list with an element for each visible phw-window whereas each
...
width and the cdr is the height of the window."
  (let ((ref-height (if (phw-compile-window-live-p)
    ...
    (mapcar (function
             (lambda (window)
               ...
               (cons (phw-phw-buffer-registry-get-symbol (buffer-name (window-buffer window)))
                     (phw-get-window-size window
                                          fix
                                          (cons ref-width ref-height)))))
            (or phw-win-list (phw-canonical-phw-windows-list)))))

;; Now possible to set fractional sizes; thanks to Geert Ribbers
;; [geert.ribbers@realworld.nl] for a first implementation.
(defun phw-set-window-size (window size &optional ref-size)
...
`frame-width' rsp. `frame-height' of the `phw-frame'."
...
(defun phw-set-phw-window-sizes (window-sizes)
  "Set the sizes of visible phw-windows to WINDOW-SIZES.
WINDOW-SIZES is a list where each element is a cons where the car
is either the buffer-name of an phw-buffer or the variable-symbol
which holds the buffer-name of an phw-buffer \(e.g. the symbol
`phw-history-buffer-name'). The cdr is a cons where car is the
...
(defun phw-get-edit-area-size (&optional win-list)
  (let ((layout-type (phw-get-layout-type phw-layout-name))
        (phw-win-list (phw-canonical-phw-windows-list win-list))
        (comp-win-height (if (equal (phw-compile-window-state) 'visible)
                             (phw-window-full-height phw-compile-window)
                           0)))
    (if (null phw-win-list)
        (cons (frame-width phw-frame)
              (- (frame-height phw-frame) comp-win-height))
      (case layout-type
        (top
         (cons (frame-width phw-frame)
               (- (frame-height phw-frame)
                  (phw-window-full-height (phw-first phw-win-list))
                  comp-win-height)))
        (left-right
         (cons (- (frame-width phw-frame)
                  (phw-window-full-width (phw-first phw-win-list))
                  (phw-window-full-width (phw-last phw-win-list)))
               (- (frame-height phw-frame)
                  comp-win-height)))
        (otherwise
         (cons (- (frame-width phw-frame)
...
;; (see `phw-repair-only-phw-window-layout') the following code and mechanism
;; is not used - but who knows, maybe we can need it later.................

;; Klaus Berndl <klaus.berndl@sdm.de>:
;; - this var must be set with the `current-window-configuration' in
;;   + `phw-restore-window-sizes' (`phw-store-window-sizes' and
;;     `phw-restore-default-window-sizes' uses the :set-function of
;;     `phw-layout-window-sizes' which in turn calles
;;     `phw-redraw-layout-full' - see below).
...
;;   + `phw-redraw-layout-full' (should cover all siutuation like
;;     hidden-phw-windows, visible-phw-windows, maximized-phw-windows etc...
;;   + `phw-toggle-compile-window-height' when the window has to be enlarged.
;;   Whenever a compile-window is visible and has its specified height
;;   (`phw-compile-window-height-lines') then the current window-config has to
;;   be stored into this variable. Otherwise this variable must be set to nil!
;; - This stored config is used by `phw-toggle-compile-window-height' to
;;   shrink back a compile-window to its specified height - so eventually
;;   be the compile-window-enlargement destroyed or resized phw-windows will
;;   be redrawn - we can not shrink the compile-window by a full redraw with
;;   phw-redraw-layout-full because this would damage some
;;   save-selected-window calls with `phw-toggle-compile-window-height
(defvar phw-compile-window-specified-height-config nil
  "Contains the most recent and valid phw-window-configuration \(a object
returned by `phw-current-window-configuration') or nil. If not nil then this
...
(defvar phw-use-window-config-for-compwin-shrink nil)

(defun phw-store-compile-window-specified-height-config ()
  "Store the current phw-window-configuration in
`phw-compile-window-specified-height-config' but do this only if a
compile-window is visible and if this compile-window has height
`phw-compile-window-height-lines'! Otherwise set this variable to nil."
  (setq phw-compile-window-specified-height-config
        (if (and phw-use-window-config-for-compwin-shrink
                 (phw-compile-window-live-p)
                 (equal (phw-window-full-height phw-compile-window)
                        phw-compile-window-height-lines))
            (phw-current-window-configuration))))

;; Klaus Berndl <klaus.berndl@sdm.de>: returns curently always nil because
;; currently phw-store-compile-window-specified-height-config is never called
;; - see comment above!
(defun phw-reset-compile-window-specified-height-config ()
  "Set the phw-window-configuration of
`phw-compile-window-specified-height-config' if it is a still valid
phw-window-configuration. If this had success return t otherwise nil."
  (let ((result
         (and phw-compile-window-specified-height-config
              (not (phw-window-configuration-invalidp phw-compile-window-specified-height-config))
              (phw-set-window-configuration phw-compile-window-specified-height-config))))
    (phw-layout-debug-error "phw-reset-compile-window-specified-height-config: %s"
                            result)
    result))

;; For backward-compatibility
(defalias 'phw-toggle-enlarged-compilation-window
  'phw-toggle-compile-window-height)

(defun phw-toggle-compile-window-height (&optional arg)
  "Toggle whether the `phw-compile-window' is enlarged or not.
If ARG > 0 then shrink or enlarge the the compile-window according to the
value of `phw-enlarged-compilation-window-max-height'. But never shrink below
the value of `phw-compile-window-height'. If ARG <= 0 then shrink
`phw-compile-window' to `phw-compile-window-height' and if ARG is nil then
...
if no compile-window is visible."
  ...
  (if (and phw-minor-mode
           (equal (selected-frame) phw-frame)
           (equal 'visible (phw-compile-window-state)))
      (let* ((height-before (phw-window-full-height phw-compile-window))
             (should-shrink (if (null arg)
                               (> height-before phw-compile-window-height-lines)
                             (<= (prefix-numeric-value arg) 0)))
...
;;                                               phw-compile-window))
        ...
        (save-excursion
          (save-selected-window
            ...
            (if should-shrink
                (progn
                  (phw-layout-debug-error "phw-toggle-compile-window-height: buffer: %s, lines: %d shrink down to compile-window-height"
                                          (current-buffer) number-of-lines)
                  (when (not (phw-reset-compile-window-specified-height-config))
                    (phw-layout-debug-error "phw-toggle-compile-window-height: call shrink-window!")
                    (shrink-window (max 0 (- (phw-window-full-height)
                                             phw-compile-window-height-lines)))
                    ...
                    ;; phw-window disappears, when doing completion etc..) we
                    ...
                    (ignore-errors (phw-restore-window-sizes))
                    ))
              (if (equal phw-enlarged-compilation-window-max-height 'best)
                  ;; With GNU Emacs we could use `phw-fit-window-to-buffer' but
                  ...
                  ;; compile-window as defined in `phw-compile-window-height'
                  ;; (in lines).
                  (progn
                    (setq max-height
                          (max (min (floor (/ (1- (frame-height)) 2))
                                    ...
                                    number-of-lines)
                               phw-compile-window-height-lines))
                    (phw-layout-debug-error "phw-toggle-compile-window-height: max-height: %s, curr-win-height: %s"
                                            max-height (phw-window-full-height))
                    (enlarge-window (- max-height (phw-window-full-height))))
                (setq max-height
                      (cond ((equal phw-enlarged-compilation-window-max-height
                                    'half)
                             (floor (/ (1- (frame-height)) 2)))
                            ((numberp phw-enlarged-compilation-window-max-height)
                             (phw-normalize-number
                              phw-enlarged-compilation-window-max-height
                              (1- (frame-height))))))
                (enlarge-window (- (max max-height phw-compile-window-height-lines)
                                   (phw-window-full-height))))
...
;;                 (set-window-start phw-compile-window (point-min)))
              )
            ;; return the new compile-window height
            (phw-window-full-height))))
    (if (phw-interactive-p)
        (phw-info-message "No compile-window in current PHW-layout!"))
    nil))
...
(defun phw-set-compile-window-height ()
  "Set the height of the compile-window according to
`phw-enlarged-compilation-window-max-height' and the value of
`temp-buffer-shrink-to-fit' \(XEmacs) and `temp-buffer-resize-mode' \(GNU
Emacs)."
  (if (and (phw-compile-window-live-p)
      ...
      (progn
        ...
        (phw-toggle-compile-window-height 1))
    (phw-layout-debug-error "phw-set-compile-window-height: shrink")
    (phw-toggle-compile-window-height -1)))


(defun phw-compile-window-state ()
  "Returns the state of the compile-window:
- 'no: No persistent compile-window, i.e. `phw-compile-window-height' is nil.
- 'visible: The compile-window is visible.
- 'hidden: A persistent compile-window is set but it is currently hidden."
  (if (null phw-compile-window-height)
      'no
    (if (phw-compile-window-live-p)
        'visible
      'hidden)))

(defun phw-toggle-compile-window (&optional arg)
  "Toggle the visibility of the compile-window of PHW.
...
`phw-compile-window-height'! If called and `phw-compile-window-height' is nil
then PHW asks for the height of the compile-window, sets this height as new
value of `phw-compile-window-height' and displays the compile-window \(so if
...
you have to quit with `C-g')."
  ...
  (unless (or (not phw-minor-mode)
    ...
    (let ((new-state (if (null arg)
                         (not (phw-compile-window-live-p))
                       (>= (prefix-numeric-value arg) 0)))
          (phw-buf (if (member (current-buffer)
                                (phw-get-current-visible-phw-buffers))
                       (current-buffer)))
          (new-win nil))
      (if new-state
          (let ((height (or phw-compile-window-height
                            (and (phw-interactive-p)
                                 (or (phw-option-get-value 'phw-compile-window-height
                                                           'saved-value)
                                     (phw-read-number "Insert height of the compile-window: " 6))))))
            (when height
              (customize-set-variable 'phw-compile-window-height height)
              ;; phw-redraw-layout-full only preserves point and selected window
              ;; if called from an edit- or compile-window. If called from an
              ;; PHW-window we have to restore it here.
              (when phw-buf
                (setq new-win (get-buffer-window phw-buf))
                (if (and new-win (window-live-p new-win)
                         (equal (window-frame new-win) phw-frame))
                    (select-window new-win)))))
        (when (phw-compile-window-live-p)
          (let ((point-location (phw-where-is-point)))
            (phw-with-original-basic-functions
             (phw-with-original-permanent-layout-functions
              (delete-window phw-compile-window)))
            (phw-restore-window-sizes)
...
;; LocalWords:  phw Jesper Nordenberg Berndl java splitted Elisp ChangeLog CVS
