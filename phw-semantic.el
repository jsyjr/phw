;;; phw-semantic.el -- define tag-utilities based on semantic
...
;; CEDET-suite) which are needed by PHW
...
(defun phw-semanticdb-find-result-nth-with-file (result n)
...
with a file, then the cdr of the result-cons is nil."
  (let ((result-nth (phw--semanticdb-find-result-nth result n)))
    (if (and (car result-nth)
             (phw--semantic-tag-with-position-p (car result-nth))
             (cdr result-nth))
        (cons (car result-nth)
              (phw--semanticdb-full-filename (cdr result-nth)))
      (cons (car result-nth) nil))))

(defun phw-get-definition-list-by-semanticdb (tag-name &optional tag-class)
...
as they match with TAG-NAME."
  (when (and (featurep 'semanticdb) (phw--semanticdb-minor-mode-p))
    ;; With semantic 2.X we do a full featured database-search.
    (let* ((search-result (phw--semanticdb-find-tags-by-name tag-name))
           (result-tags (and search-result
                             (phw--semanticdb-strip-find-results search-result)))
           (tag-numbers nil))
      (when (and result-tags
                 ;; some paranoia
                 (= (length result-tags)
                    (phw--semanticdb-find-result-length search-result)))
        ...
        (dotimes (i (length result-tags))
          (if (and (or (null tag-class) ;; we are interested in any tags regardless of class
                       (equal (phw--semantic-tag-class (nth i result-tags)) tag-class))
                   (phw--semantic-tag-with-position-p (nth i result-tags)))
        ...
        (delq nil
              (mapcar (function (lambda (n)
                                  (let ((r (phw-semanticdb-find-result-nth-with-file
                                            search-result n)))
                                    (if (and (cdr r)
                                             (stringp (cdr r))
...
(defun phw-search-tag-by-semanticdb (tag-name &optional tag-class)
...
The returned tag will contain the filename as its :file-attribute."
  (let ((definition-alist (phw-get-definition-list-by-semanticdb tag-name tag-class))
        (result-elem))
    (when definition-alist
      ...
      (setq result-elem
            (if (> (length definition-alist) 1)
                (assoc (phw-offer-choices "Select a definition-file: "
                                          (mapcar #'car definition-alist))
                       definition-alist)
              (car definition-alist)))
      ;; we add the filename to the tag, then all needed informations are
      ;; within the tag
      (phw--semantic--tag-put-property (cdr result-elem)
                                       :filename
                                       (car result-elem)))))

(defun phw-search-tag-by-semantic-analyzer (tag-name &optional tag-class)
...
it)."
  (let* ((scope (phw--semantic-calculate-scope)))
    (when scope
      (phw--semantic-analyze-find-tag tag-name tag-class scope))))

(silentcomp-provide 'phw-semantic)

;;; phw-semantic.el end here
