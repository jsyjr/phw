;;; phw-speedbar.el --- Integration of speedbar into PHW
...
;; This package provide speedbar integration and using for the PHW.
;;
;; There are two complete different aspects of integration/using speedbar for
;; PHW:
;;
;; 1. Integration the speedbar itself into the phw-frame:
;;
;;    This allows you to:
;;    
;;    - Sync up to the speedbar with the current buffer.
;;    
;;    - Files opened with the speedbar are displayed in the phw source window.
;;
...
;;    display in the methods-buffer of PHW!
;;
...
(defgroup phw-speedbar nil
  "Settings for the speedbar-integration of PHW."
  :group 'phw-general
  :prefix "phw-")

(defcustom phw-speedbar-before-activate-hook nil
  "*Hook running directly before PHW activates the integrated speedbar.
...
(defcustom phw-speedbar-buffer-sync 'basic
  "*Synchronize the speedbar-buffer of PHW automatically with current
edit buffer.
...
`phw-speedbar-buffer-sync-hook' is evaluated."
  :group 'phw-speedbar
  :type '(radio :tag "Synchronize PHWs speedbar buffer"
...
(defcustom phw-speedbar-buffer-sync-delay 'basic
  "*Time Emacs must be idle before the speedbar-buffer of PHW is synchronized.
...
(defcustom phw-speedbar-buffer-sync-hook nil
  "Hook run at the end of `phw-speedbar-buffer-sync'.
See documentation of `phw-speedbar-buffer-sync' for conditions when
...
- The speedbar-buffer is displayed in a visible window of the
  phw-frame \(so no check for visibilty of the speedbar-buffer in
...
Important note: If the option `phw-speedbar-buffer-sync' is not
nil the function `phw-speedbar-buffer-sync' is running either
every time Emacs is idle or even after every command \(see
`phw-speedbar-buffer-sync-delay'). So if the speedbar-buffer is
...
(defphw-advice-set phw-speedbar-adviced-functions 
  "These functions of speedbar are always adviced if PHW is active.")

(defconst phw-speedbar-buffer-name " SPEEDBAR"
  "Name of the PHW speedbar buffer.")

(defun phw-speedbar-buffer-selected ()
  (equal (current-buffer) (phw-buffer-obj phw-speedbar-buffer-name)))

(defphw-advice speedbar-click around phw-speedbar-adviced-functions
  "Makes the function compatible with PHW. If PHW is active and the window of
`phw-speedbar-buffer-name' is visible \(means a layouts uses the
speedbar-integration) and the clicked node in speedbar is a file then the
PHW-edit-window is selected at the end. So always the edit-window is selected
after clicking onto a filename in the speedbar."
  ...
  (let ((item (and (fboundp 'speedbar-line-file)
    ...
    (if (and phw-minor-mode
             ...
             (window-live-p (get-buffer-window phw-speedbar-buffer-name))
             (and item
                  (phw-file-exists-p item)
                  (not (phw-file-directory-p item))))
        (phw-select-edit-window))))


(defphw-advice speedbar-frame-mode around phw-speedbar-adviced-functions
  "During running speedbar within PHW this command is disabled!"
  (if phw-minor-mode
      (when (phw-interactive-p)
        (phw-info-message "speedbar-frame-mode takes no effect when running within PHW!"))
    ad-do-it))


(defphw-advice speedbar-get-focus around phw-speedbar-adviced-functions
  "During running speedbar within PHW this function behaves like follows:
Change window focus to or from the PHW-speedbar-window. If the selected window
is not speedbar-window, then the speedbar-window is selected. If the
speedbar-window is active, then select the edit-window."
  (if phw-minor-mode
      (if (phw-speedbar-buffer-selected)
          (phw-select-edit-window)
        (phw-speedbar-select-speedbar-window))
    ad-do-it))
...
(defphw-advice dframe-mouse-set-point around phw-speedbar-adviced-functions
...
;; it!
(defphw-advice dframe-select-attached-frame after phw-speedbar-adviced-functions
  "Run `phw-speedbar-dframe-select-attached-window' but only if
`dframe-after-select-attached-frame-hook' is not available."
  (unless (boundp 'dframe-after-select-attached-frame-hook)
    (phw-speedbar-dframe-select-attached-window)))

(defun phw-speedbar-dframe-select-attached-window ()
  (when (and phw-last-edit-window-with-point
             phw-last-source-buffer
             (window-live-p phw-last-edit-window-with-point)
             (equal (window-buffer phw-last-edit-window-with-point)
                    phw-last-source-buffer))
    (select-window phw-last-edit-window-with-point)
    (set-buffer phw-last-source-buffer)
;;     nil
    ))

(defun phw-speedbar-select-speedbar-window ()
  (ignore-errors
    (and (window-live-p (get-buffer-window phw-speedbar-buffer-name))
         (select-window (get-buffer-window phw-speedbar-buffer-name)))))

(defphw-window-dedicator-to-phw-buffer phw-set-speedbar-buffer
    phw-speedbar-buffer-name nil
  "Display in current window the speedbar-buffer and make window dedicated."
  (phw-speedbar-activate)
  (set-window-buffer (selected-window)
                     (get-buffer-create phw-speedbar-buffer-name))
  (unless phw-running-xemacs
    (set (make-local-variable 'auto-hscroll-mode) nil)))



(defvar phw-speedbar-verbosity-level-old nil)
(defvar phw-speedbar-select-frame-method-old nil)
(defvar phw-speedbar-update-flag-old -1)

(defun phw-speedbar-activate()
...
future this could break."
  ...
  (phw-enable-advices 'phw-speedbar-adviced-functions)
  
  (run-hooks 'phw-speedbar-before-activate-hook)

  (add-hook 'dframe-after-select-attached-frame-hook
            'phw-speedbar-dframe-select-attached-window)

  ;;disable automatic speedbar updates... let the PHW handle this with
  ;;its sync-mechanism
  (speedbar-disable-update)

  ;;always stay in the current frame
  ;; save the old value but only first time!
  (if (null phw-speedbar-select-frame-method-old)
      (setq phw-speedbar-select-frame-method-old speedbar-select-frame-method))
  (setq speedbar-select-frame-method 'attached)

  (when (not (buffer-live-p speedbar-buffer))
    (save-excursion
      (setq speedbar-buffer (get-buffer-create phw-speedbar-buffer-name))
...
;;         (setq auto-show-mode nil)
  ...
  ;;frame.  AKA the frame that PHW is running in.
  (setq speedbar-frame phw-frame)
  (setq dframe-attached-frame phw-frame)
  ...
  ;;much.
  ;; save the old value but only first time!
  (if (null phw-speedbar-verbosity-level-old)
      (setq phw-speedbar-verbosity-level-old speedbar-verbosity-level))
  (setq speedbar-verbosity-level 0)

  ;; save old update-flag but only the first time
  (if (equal phw-speedbar-update-flag-old -1)
      (setq phw-speedbar-update-flag-old speedbar-update-flag))
  (setq speedbar-update-flag nil)

  (phw-activate-phw-autocontrol-function phw-speedbar-buffer-sync-delay 
                                          'phw-speedbar-buffer-sync)

  ;;reset the selection variable
  (setq speedbar-last-selected-file nil))


(defun phw-speedbar-deactivate ()
  "Reset things as before activating speedbar by PHW"
  (phw-disable-advices 'phw-speedbar-adviced-functions)
  ...
  (if phw-speedbar-select-frame-method-old
      (setq speedbar-select-frame-method phw-speedbar-select-frame-method-old))
  (setq phw-speedbar-select-frame-method-old nil)

  (if phw-speedbar-verbosity-level-old
      (setq speedbar-verbosity-level phw-speedbar-verbosity-level-old))
  (setq phw-speedbar-verbosity-level-old nil)

  (if (not (equal phw-speedbar-update-flag-old -1))
      (setq speedbar-update-flag phw-speedbar-update-flag-old))

  (phw-stop-autocontrol/sync-function 'phw-speedbar-buffer-sync)
...
(defun phw-speedbar-active-p ()
  "Return not nil if speedbar is active and integrated in the `phw-frame'."
  (and (phw-buffer-obj phw-speedbar-buffer-name)
       (get-buffer-window (get-buffer phw-speedbar-buffer-name) phw-frame)))

(defun phw-speedbar-update-contents ()
  "Encapsulate updating the speedbar."
  (speedbar-update-contents))


(defphw-autocontrol/sync-function phw-speedbar-buffer-sync
    phw-speedbar-buffer-name phw-speedbar-buffer-sync t
  ...
  (let ((speedbar-default-directory
         (with-current-buffer visible-buffer
           (phw-fix-filename default-directory)))
        (phw-default-directory (phw-fix-filename default-directory)))
    (when (and (or (not (phw-string= speedbar-default-directory
                                     phw-default-directory))
      ...
      (run-hooks 'phw-speedbar-buffer-sync-hook))))

(defun phw-goto-window-speedbar ()
  "Make the PHW-speedbar window the current window.
This command does nothing if no integrated speedbar is visible in the
PHW-frame."
  (interactive)
  (phw-goto-phw-window phw-speedbar-buffer-name))

(defun phw-maximize-window-speedbar ()
  "Maximize the PHW-speedbar-window.
...
speedbar-window is not visible within the PHW-frame."
  (interactive)
  (phw-maximize-phw-buffer phw-speedbar-buffer-name t))
...
(defun phw-speedbar-decorate-tag (tag face)
...
(defun phw-speedbar-sb-tag-p (tag)
  "Return not nil if TAG is a semantic-tag generated from a speedbar tag."
  (phw--semantic--tag-get-property tag 'phw-speedbar-tag))

(require 'tree-buffer)
(require 'phw-face)
(defun phw-create-non-semantic-tree (node tag-list)
  "Add all tags of TAG-LIST with side-effects as children to NODE. TAG-LIST is
a list generated by `phw-get-tags-for-non-semantic-files'. TAG-LIST is of the
form:
    ...
    (dolist (tag tag-list)
      (typecase tag
        ...
        (speedbar-generic-list-tag
         ...
         (setq new-tag (phw--semantic-tag (car tag)
                                          (intern (car tag))))
         (phw--semantic--tag-set-overlay new-tag (make-vector 2 (cdr tag)))
         (phw--semantic--tag-put-property new-tag 'phw-speedbar-tag t)
         (phw-apply-user-filter-to-tags (list new-tag))
         (when (not (phw-tag-forbidden-display-p new-tag))
           (tree-node-new (phw-speedbar-decorate-tag tag phw-method-non-semantic-face)
                          0
                          new-tag
                          t
                          node)))
        (speedbar-generic-list-positioned-group
         ...
         (setq new-tag (phw--semantic-tag (car tag)
                                          (intern (car tag))))
         (phw--semantic--tag-set-overlay new-tag
                                         (make-vector 2 (car (cdr tag))))
         (phw--semantic--tag-put-property new-tag 'phw-speedbar-tag t)
         (phw-apply-user-filter-to-tags (list new-tag))
         (when (not (phw-tag-forbidden-display-p new-tag))             
           (phw-create-non-semantic-tree
            (setq new-node
                  (tree-node-new (phw-speedbar-decorate-tag tag phw-method-non-semantic-face)
                                 0
                                 new-tag
           ...
           (setf (tree-node->expanded new-node)
                 (member major-mode
                         phw-non-semantic-methods-initial-expand))))
        (speedbar-generic-list-group
         ...
         (setf (tree-node->expanded new-node)
               (member major-mode
                       phw-non-semantic-methods-initial-expand)))
        (otherwise
         (phw-error "phw-create-non-semantic-tree: malformed tag-list!")
         )))
    node))

(defun phw-get-tags-for-non-semantic-files ()
...
(silentcomp-provide 'phw-speedbar)

;;; phw-speedbar.el ends here
